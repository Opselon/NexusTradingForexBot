...
for i in range(4000):
    ...
    bars.append(
        SimpleNamespace(
            ...
            low=min(o, c) - rng.uniform(0.1, 0.6),
            close=c,
            tick_volume=100,
            is_complete=True,
        )
...
print(f"frame built {time.perf_counter() - tm:.0f}s rows={frame.height}", file=sys.stderr)
...
data = {
    ...
    "exact_match": all(abs(a - b) <= 1e-12 for a, b in zip(ds_liq, live_liq, strict=False)),
}
