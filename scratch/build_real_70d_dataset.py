...
import sys, json, time, pathlib; sys.path.insert(0, '.')
import polars as pl
...
df = df.with_columns(
    pl.col("time").cast(pl.Datetime("us")).cast(pl.Int64).alias("_us")
)
rows = []
for r in df.tail(1200).iter_rows(named=True):
    us = int(r["_us"])
    import datetime as _dt
    ts = _dt.datetime.fromtimestamp(us / 1_000_000, tz=_dt.timezone.utc)
...
try:
    ...
    print(json.dumps({"dataset_id": handle.get("dataset_id"),
                      "counts": handle.get("counts"),
                      "hash": handle.get("dataset_hash", "")}))
    did = handle.get("dataset_id")
    v = verify_70d_artifact(did)
    print(json.dumps({"verify": {k: v[k] for k in ("ok", "feature_count", "rows",
                                                   "schema_id_ok", "dimension_ok",
                                                   "schema_hash_ok", "all_finite",
                                                   "all_in_range") if k in v}}, indent=1))
