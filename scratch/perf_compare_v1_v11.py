...
def measure(fn, label):
    ...
    for i in range(55, N):
        win = bars[i - 55 + 1 : i + 1]
        b = rows[i]
        fv = engine.compute_from_bars(win, TickData(symbol="XAUUSD", timestamp=times[i], bid=b["close"], ask=b["close"]+0.2, volume=0))
        t0 = time.perf_counter()
        fn(win, decision_at=times[i], mid_price=b["close"], atr=fv.atr_m1)
        lat.append((time.perf_counter() - t0) * 1000.0)
    la = np.asarray(lat)
    print(f"{label:<10} p50={np.percentile(la,50):.3f} ms  p95={np.percentile(la,95):.3f} ms  max={la.max():.3f} ms  (n={len(la)})")
...
r2 = measure(lambda w, **kw: compute_liquidity_features_v1_1(w, **kw, params=LiquidityParams()), "v1.1")
print(f"\nlatency ratio v1.1/v1: p50 {r2[0]/r1[0]:.2f}x")

import json

with open("scratch/liq_perf_comparison.json", "w", encoding="utf-8") as fh:
    json.dump({"v1": {"p50": r1[0], "p95": r1[1], "max": r1[2]}, "v1_1": {"p50": r2[0], "p95": r2[1], "max": r2[2]}}, fh, indent=2)
