...
sh = [
    LiquidityPool(
        100.0, PoolSide.BSL, PoolSource.SWING_HIGH, 1, 1.0, t0, t0 + timedelta(minutes=5)
    ),
    LiquidityPool(
        105.0, PoolSide.BSL, PoolSource.SWING_HIGH, 1, 1.0, t0, t0 + timedelta(minutes=55)
    ),
]
...
st_v11, _ = detect_reactive_sweep_v1_1(
    [far_pool], bars, atr=1.0, decision_at=bars[-1].timestamp, relevance_atr=2.0
)
print(
    "far pool v1 state =",
    st_v1,
...
for k in range(10):
    col = A[:, k]
    print(
        f"  {names[k]:<12} mean={col.mean():.3f} med={np.median(col):.3f} sat%={(np.abs(col) >= 2.9999).mean() * 100:.1f} uniq={len(np.unique(np.round(col, 5)))}"
    )
