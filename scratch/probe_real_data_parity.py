...
).with_columns((pl.col("time_naive").cast(pl.Int64) * 1_000).alias("_us"))


def _utc_from_us(us: int) -> _dt.datetime:
    return _dt.datetime.fromtimestamp(us / 1_000_000, tz=_dt.UTC)
...
print(f"real frame built {time.perf_counter() - tm:.1f}s rows={frame.height}")
...
gov.compute_from_engine(
    bars=bars, mid_price=float(close), atr=float(fv.atr_m1), decision_at=bars[-1].timestamp
)
live_liq = list(gov.last_snapshot.features)
deltas = [abs(a - b) for a, b in zip(ds_liq, live_liq, strict=False)]
names = ["bsl", "ssl", "eqh", "eql", "htf", "internal", "external", "confl", "sweep", "disp"]
print("real LAST-BAR parity:")
for i, (n_, d, l) in enumerate(zip(names, ds_liq, live_liq, strict=False)):
...
for frac in [i / 25 for i in range(1, 26)]:
    ...
    g2.compute_from_engine(
        bars=sub, mid_price=float(close), atr=float(fv2.atr_m1), decision_at=sub[-1].timestamp
    )
