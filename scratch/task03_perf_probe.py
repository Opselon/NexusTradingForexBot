...
if __name__ == "__main__":
    ...
    def seventy():
        ...
        assemble_70d(base50=fv.to_tensor_input(), news10=[0.1] * 10,
                     liquidity10=[0.2] * 10, symbol="XAUUSD", timeframe="M1",
                     timestamp_utc=last.timestamp, news_available=True,
                     liquidity_available=True).validate(context="perf")

    conv = InferenceValidator(expected_schema_id="scalp_v3", expected_dimension=70,
                              expected_schema_hash="")
    vec = [0.0] * 50 + [0.1] * 10 + [0.2] * 10

    def validate():
        conv.validate(vec, news_status="FEATURE_AVAILABLE",
    ...
    def hook_snap():
        fv = engine.compute_from_bars(bars, tick)
        hook.compute_snapshot(completed_bars=bars, base50=list(fv.to_tensor_input()),
                              news_context=news_ctx,
                              timestamp_utc=datetime.now(UTC), context="perf")
