...
// ErrBoundary-class response: Python answered with an HTTP >= 400 carrying a
// valid v1 error envelope. This is a CONTRACT response, not a transport
// failure, so the circuit breaker stays closed. The envelope is preserved so
// a handler can replay Python's exact error body — byte-level error parity.
