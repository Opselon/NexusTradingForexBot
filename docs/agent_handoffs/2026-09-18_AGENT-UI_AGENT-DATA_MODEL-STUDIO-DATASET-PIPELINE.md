...
5. **Dataset Path Containment**: Every operator-supplied dataset path passes `_safe_dataset_path()` before any read — null bytes and `..` segments are rejected pre-resolution, the path is resolved to a real absolute path, and containment is enforced with `Path.is_relative_to` against the allowlist roots (`data/raw`, `data/processed`, `data/positions`, `data`; extendable via `NEXUS_MODEL_STUDIO_ROOTS`). This closes the CodeQL `py/path-injection` class **and** the string-prefix bypass (`data/rawx` starts with `data/raw`). The download write side additionally whitelist-sanitizes `symbol`/`source` before composing the output filename. Pinned by 4 tests in `tests/unit/test_model_studio_pipeline.py`.
