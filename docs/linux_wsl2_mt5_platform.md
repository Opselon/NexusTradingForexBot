...
| Direct Python `MetaTrader5` IPC | **UNSUPPORTED** — `initialize()` → `(-10005, 'IPC timeout')`; proven with `WINEDEBUG=+file` pipe forensics (client connect/write OK, terminal-side pipe server never replies). Reproduced on wine 9.0, 10.0 and 11.16 |
| Supported Nexus integration | **`RemoteMT5GatewayAdapter`** (HTTP + HMAC) — localhost RTT median 0.80 ms / p95 1.26 ms (200 calls) |
| Network topology caveat | **WSL2 → Windows host TCP/ICMP is blocked by default** (Hyper-V firewall). A gateway on the Windows host is unreachable from WSL until mirrored networking or a narrowly scoped allow rule exists. For tests, run engine + gateway **inside WSL** |
| Shell EOL requirement | `scripts/linux/*.sh` must be **LF** (pinned in `.gitattributes`, commit `8fdcbdb9`); CRLF copies fail in bash with `set: -\r: invalid option` **while exiting 0** — masked breakage |
...
first start (6184 → 6191 observed; the terminal manages its own build).
...
2. `python.exe -m pip` via get-pip, then `pip install MetaTrader5
   "numpy<2"`.
...
- Gateway-unavailable behaviour (verified): `connect()` → `False`,
