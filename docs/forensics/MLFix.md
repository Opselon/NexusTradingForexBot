...
- 2026-09-05 05:30 +03:30 -- Nexus-Main (promotion-gate evidence chain + promoted-champion truth): (1) OOS evidence produced on candidate c9982ddde175: chronological final-20% holdout (19,990 rows, 2026-07-27..2026-08-17) evaluated under the SEQUENTIAL contract (B,32,70); eval-sample-only slice (5,521 rows) = accuracy 0.3664 vs majority 0.5776, per-class precision/recall (NO_TRADE .698/.283, BUY .276/.531, SELL .251/.421), trade-class acc 0.4807, ECE15 0.0257, mean maxprob 0.386, entropy 1.086; unfiltered-rows variant (incl. non-evaluable label=0) recorded separately (acc 0.2363 / ECE 0.1538) to keep the eval semantics honest — evidence scratch/nsmain_oos_232534{,_eval_only}.json + probs npy. CONSISTENT with F2/F2b (~0.365-0.368): the corrected producer learns, but the 3.5-month dataset still yields NO exploitable edge (matches Gate-6 honest FAIL). (2) offline<->live parity: candidate loads and predicts on both ranks (1,70) fallback and (1,32,70) sequence, probs sum 1.0 — cold-start shape identity verified. (3) Determinism: independent second 34x10 run (pilot_70d_3class_20260905_000649) reproduced model_sha c9982ddde175 byte-for-byte (two seed-42 full runs identical; seed 43/44 dispersion runs were interrupted by host memory pressure — dispersion evidence remains OPEN). (4) PROMOTION EXECUTED by Agent-1 (Master Model Forensics) at 2026-09-05T01:24:08Z: pilot_70d_3class_20260905_000649 (same sha c9982ddde175) promoted to artifacts/models/scalp/XAUUSD/70d_liquidity/, replacing c8c0b5b06d4c (record: artifacts/forensics/agent1_promotion_record_20260905.json; full backup _champion_backup_20260905_012408 with meta+scaler+model). Reason: champion provenance incoherence (3-class meta vs 4-head tensor, scaler poison on news slots). Post-promotion disk truth verified by this lane: live tensor (3,32), input (128,70), dataset_id+sha bound to ds_70d_clean_m1_20260904, lineage CLEAN_HISTORICAL. RECOMMENDATION RECORDED: keep the new champion in SHADOW/PAPER until (a) >=1y M1 data (roadmap 9.1), (b) fold-wise OOS stability, (c) multi-seed dispersion complete. Engine was NOT running during any of this (no live exposure).
