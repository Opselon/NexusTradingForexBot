...
**ML-QA-018 closes the recount table: every in-gate benchmark/timing row is now REMEDIATED or residual-docstring-only.** The two rows above were the last OPEN entries; the only remaining untouched entry is `test_70d_bug106_incremental_phase19.py`, which stays DEFERRED (a `skipif` on a data file absent from git — needs a synthetic-frame fixture or an accepted-risk note before its timing asserts are reachable). The next workstream for this roster is a full re-scan of `tests/` (the scanner is regenerable in seconds) to catch shapes the original census did not classify, or the deferred entry.
