...
- [ML-QA-017](tasks/ML-QA-017.md) — Training-Env Worker Suite Process-Identity Determinism (roster candidate, parent's literal `os.getpid()` comparison replaced by transport cross-stamp continuity + injected `_pid` supplier, fixed-id + real-pid cross-check legs) `[P2 | DONE (PR pending) | AGENT-QA]`
- [ML-QA-018](tasks/ML-QA-018.md) — Benchmark Wall-Clock → CPU-Time Determinism, final two roster rows (`test_sample_weights.py` + `test_research_edge_hardening_20260909.py` perf_counter budgets -> shared `budget_cpu_ms`, calibrated 500 ms / ratio 6.0, `+ 1.0` waiver removed) `[P2 | DONE (PR pending) | AGENT-QA]`
