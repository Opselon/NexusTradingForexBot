...
| `ML-QA-012` | L: CI/CD | P2 | Push-Gate Wall-Clock Determinism: BUG-140 Outcome-Flush Race Suite (roster candidate #3, one frozen instant, CPU-time poll bounds) | **DONE** (PR #466, squash 2b707636) | `AGENT-QA` | NO | `ML-QA-004`, `ML-QA-009`, `ML-QA-011` | `PARALLEL_SAFE` |
| `ML-QA-013` | L: CI/CD | P2 | Push-Gate Monotonic-Clock Determinism: BUG-285 Overflow-Drain Cadence Suite (roster candidate #4, optional-argument `now` seam, both strict-`<` boundary edges provable) | **DONE** (PR pending) | `AGENT-QA` | NO | `ML-QA-004`, `ML-QA-009`, `ML-QA-012` | `PARALLEL_SAFE` |
...
| `ML-QA-016` | L: CI/CD | P2 | Hot-Reload Suite Process-Identity Determinism (roster candidate, `os.getpid()` literals replaced by object identity + injected `_pid` supplier, `mkdtemp` -> `tmp_path`) | **DONE** (PR pending) | `AGENT-QA` | NO | `ML-QA-004`, `ML-QA-003` | `PARALLEL_SAFE` |
| `ML-QA-017` | L: CI/CD | P2 | Training-Env Worker Suite Process-Identity Determinism (roster candidate, parent's literal `os.getpid()` comparison replaced by transport cross-stamp continuity + injected `_pid` supplier, fixed-id + real-pid cross-check legs, executed negative control proving the old assert's fork blind spot) | **DONE** (PR pending) | `AGENT-QA` | NO | `ML-QA-004`, `ML-QA-003`, `ML-QA-016` | `PARALLEL_SAFE` |
| `ML-QA-018` | L: CI/CD | P2 | Benchmark Wall-Clock -> CPU-Time Determinism, final two roster rows (`test_sample_weights.py` 50k SLA + `test_research_edge_hardening_20260909.py` sized-path linearity; perf_counter -> shared budget_cpu_ms, budgets calibrated against measured CPU cost 500 ms / ratio 6.0, additive `+ 1.0` waiver removed from the ratio assert) | **DONE** (PR pending) | `AGENT-QA` | NO | `ML-QA-004`, `ML-QA-003`, `ML-QA-014` | `PARALLEL_SAFE` |
