...
def test_no_future_leakage() -> None:
    ...
    tick_at = TickData(symbol="XAUUSD", timestamp=t0 + timedelta(minutes=99), bid=3309.9, ask=3310.2, volume=100)
    fv_100_from_100 = engine.compute_from_bars(bars[:100], tick_at)
    fv_100_from_200 = engine.compute_from_bars(bars[:100], tick_at)  # same window, extra future bars not passed
    ...
    trainer = pytest.importorskip("nexus_scalp.training.walk_forward_trainer").WalkForwardTrainer  # noqa: F811
    ...
    wft = trainer(num_folds=2, epochs_per_fold=1, min_rows_per_train_split=10, min_rows_per_test_split=5)
...
def test_gap_safe_sequences() -> None:
    ...
    res = builder.build(df, label_col="label", timestamp_col="timestamp", symbol_col="symbol", timeframe_col="timeframe", news_enabled=False)
    ...
    df2 = pl.DataFrame([
        {"timestamp": (t0 + timedelta(minutes=i)).isoformat(), "symbol": "EURUSD" if i == 20 else "XAUUSD", "timeframe": "M1", "feat_0": 0.0, "feat_1": 0.0, "label": 0}
        for i in range(40)
    ])
    res2 = SequenceBuilder(seq_len=8, max_gap_us=None).build(df2, label_col="label", timestamp_col="timestamp", symbol_col="symbol", timeframe_col="timeframe", news_enabled=False)
...
def test_paper_live_training_lineage() -> None:
    ...
    import pathlib as _p, atexit as _atexit, shutil as _sh
    ...
    try:
        repo.log_ledger_opened(ticket=101000, symbol="XAUUSD", direction="BUY", volume=0.1, entry_price=2000.0, timestamp_str="2026-09-03T12:00:00+00:00", entry_reason="TEST", account_source="PAPER")
        repo.log_ledger_closed(ticket=101000, symbol="XAUUSD", direction="BUY", volume=0.1, entry_price=2000.0, exit_price=2500.0, status="CLOSED", pnl=50000.0, commission=0.0, swap=0.0, duration_sec=10.0, timestamp_str="2026-09-03T12:01:00+00:00", exit_mechanism="MANUAL_CLOSE", account_source="PAPER")
        ...
        repo.log_ledger_opened(ticket=152569700000, symbol="XAUUSD", direction="BUY", volume=0.1, entry_price=4300.0, timestamp_str="2026-09-03T12:05:00+00:00", entry_reason="TEST", account_source="LIVE")
        repo.log_ledger_closed(ticket=152569700000, symbol="XAUUSD", direction="BUY", volume=0.1, entry_price=4300.0, exit_price=4304.0, status="CLOSED", pnl=50.0, commission=0.0, swap=0.0, duration_sec=10.0, timestamp_str="2026-09-03T12:06:00+00:00", exit_mechanism="SYSTEM_CLOSE", account_source="LIVE")
        repo._queue.join()
        ...
        mp = ModelProvenance(model_id="primary_scalp_scalp_v3_70d", model_version="1.0.0", feature_schema_id="scalp_v3", feature_dimension=70)
        ...
        from nexus_scalp.labeling.triple_barrier import TripleBarrierLabeler as _LBL
        assert _LBL(embargo_bars=15).embargo_bars == 15
