...
def test_market_radar_live_state_integration():
    ...
    class _FakeEngine:
        def __init__(self):
            ...
            self.adapter = SimpleNamespace(is_connected=lambda: True)
            self.compute_live_freshness = lambda: {"overall": "UNKNOWN"}
...
def test_radar_carried_by_sse_snapshot_contract():
    """BUG-138 follow-up: Market Radar must travel over SSE (/api/ticks/stream),
    which emits get_system_state() directly. The SSE layer is a thin serializer of
    that canonical snapshot (server.py event_generator -> get_system_state()), so we
    assert the radar section is embedded in every endpoint fed by that snapshot —
    matching the repo's established SSE-contract test pattern (see
    test_liquidity_runtime_integration_phase18.py TEST-70D-16/17). Regression guard
    so REST, SSE and /api/status never diverge on the radar section (single source)."""
    from fastapi.testclient import TestClient
    ...
    # /api/live/state unpacks get_system_state() including radar.
    live = client.get("/api/live/state").json()
    assert live["radar"] is not None, "/api/live/state radar missing"
    assert live["radar"]["state"] == "SETUP_READY"
    assert live["radar"]["best_setup"]["setup_type"] == "LIQUIDITY_SWEEP"
    assert live["radar"]["decision_reason"] == "BLOCKED_BY_GUARDIAN_UNSAFE_REGIME"

    # /api/status uses the SAME canonical snapshot the SSE stream emits.
    status = client.get("/api/status").json()
    assert status.get("radar") is not None, "/api/status (SSE source) radar missing"
    assert status["radar"]["state"] == "SETUP_READY"
    print("SSE snapshot contract radar regression test PASSED successfully.")
