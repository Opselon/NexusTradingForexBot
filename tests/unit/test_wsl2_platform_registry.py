...
3. The WSL2 platform entry in docs/RELEASE.md §3 must keep the fail-closed
   claims: direct Python MetaTrader5 IPC = unsupported, supported path =
   RemoteMT5GatewayAdapter, Xvfb required, network caveat present.
"""
...
class TestLinuxScriptEol:
    def test_runtime_script_blob_is_lf(self):
        ...
        `set: -\\r: invalid option` while the script still exits 0 on some
        ...
        assert b"\r\n" not in data, (
            ...
            "bash on Linux fails with confusing '$\\r' errors. Re-commit with LF."
        )
...
class TestWsl2PlatformRegistryEntry:
    def test_release_md_documents_wsl2_entry_with_fail_closed_claims(self):
        """docs/RELEASE.md §3 must carry the WSL2 entry with the correct,
        fail-closed capability claims (no Wine-IPC-as-supported drift)."""
        text = (REPO / "docs" / "RELEASE.md").read_text(encoding="utf-8")
        assert "Linux (WSL2" in text, "WSL2 platform row missing from RELEASE.md §3"
    ...
    def test_platform_doc_rejects_wine_ipc_and_requires_xvfb(self):
        ...
        assert "firewalled" in text or "blocked by default" in text, (
            "WSL2→Windows-host network caveat missing"
        )
    ...
    def test_doctor_script_never_gains_live_credential_defaults(self):
        ...
        # the credential check must remain fail-closed (a FAIL, not a warning)
