...
def log_root(tmp_path: Path):
    """Fixture: configure logging into a temp dir; yield the dir + a logger."""
...
def test_redact_sensitive_fields() -> None:
    ...
    # trusted structural keys are never scrubbed (long alnum event names)
...
def test_error_routes_to_error_file_with_stack(log_root) -> None:
    ...
    assert _read_events(root, "info") == []
    today = datetime.now().strftime("%Y-%m-%d"); content = (root / "error" / today[:4] / today[5:7] / f"{today}.log").read_text(encoding="utf-8")
...
def test_structured_fields_present(log_root) -> None:
    ...
    time.sleep(0.1)
    today = datetime.now().strftime("%Y-%m-%d"); content = (root / "info" / today[:4] / today[5:7] / f"{today}.log").read_text(encoding="utf-8")
    ...
    assert "category=STRATEGY" in content  # auto-category from event map
...
def test_level_match_filter_rejects_other_levels() -> None:
    filt = _LevelMatchFilter(logging.INFO)
    assert filt.filter(_record(logging.INFO))
    assert not filt.filter(_record(logging.WARNING))
