  ...
  return (
    <Drawer title={t("incidents.drawer.title", "Incident {id}", { id: incidentId })} onClose={onClose}>
      <div style={{ display: "flex", gap: 6, marginBottom: 10, flexWrap: "wrap" }}>
        {(["detail", "timeline", "traces", "report"] as const).map((id) => (
      ...
      {detailQ.isPending ? (
        ...
        <div style={{ display: "grid", gap: 10 }}>
          ...
          <Panel title={t("incidents.drawer.where", "Where it happened")} tight>
            <dl className="kv">
              <InfoRow
                label={t("incidents.drawer.component_op", "component / operation")}
                value={`${inc.component ?? "—"} / ${inc.operation ?? "—"}`}
              />
          ...
          <Panel title={t("incidents.drawer.impact", "Impact (backend analyzer)")} tight>
            ...
            <div className="tiny muted" style={{ marginTop: 6 }}>
              {t(
                ...
                { records: arr(inc.affected_records).length, models: arr(inc.affected_models).length, tags: (inc.tags ?? []).join(", ") || "—" },
              )}
            </div>
          </Panel>
            ...
            >
              {arr(inc.timeline)
                ...
                .map((row: Row, i: number) => (
                  <tr key={i}>
                    <td className="tiny">{formatDateTime(str(row.timestamp))}</td>
                    <td className="small">{str(row.event) ?? str(row.event_type) ?? "—"}</td>
                    <td className="tiny muted">{str(row.detail) ?? str(row.message) ?? ""}</td>
                  </tr>
                ))}
            </DataTable>
          )}
        ...
        <Panel title={t("incidents.drawer.traces", "Value traces (how each number got here)")} tight>
          {arr(inc.value_traces).length === 0 ? (
            ...
            <div style={{ display: "grid", gap: 8 }}>
              {arr(inc.value_traces).map((row: Row, i: number) => (
                <div key={i} style={{ border: "1px solid var(--border)", borderRadius: 6, padding: "6px 8px" }}>
                  <div className="small">
                    <b>{str(row.field) ?? t("incidents.drawer.field_fallback", "field")}</b> · <span className="muted tiny">{str(row.source) ?? "—"}</span>
                  </div>
                  <JsonBlock value={row.hops ?? row} maxChars={1200} />
                </div>
              ))}
            </div>
          )}
