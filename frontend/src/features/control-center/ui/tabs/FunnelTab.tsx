...
export function FunnelTab({ hours }: { hours: number | undefined }) {
  ...
  // perf: localized labels derive once per payload + dictionary, not per render
  // (the parent page re-renders on every summary tick with this query idle).
  const labeledActionRows = useMemo(() => actionRows.map((r) => ({ ...r, label: actionLabel(t, r.label) })), [actionRows, t]);
  const empty = actionRows.length === 0 && stageRows.length === 0 && gateRows.length === 0;
  ...
  return (
    <Panel title={t("control-center.panel.funnel", "Terminal-stage funnel")}>
      {funnelQ.isPending ? (
        ...
        /* §9: a failed GET is an error, never an empty funnel — backend words + retry */
        <ErrorState
          message={funnelQ.error instanceof Error ? funnelQ.error.message : t("control-center.err.funnel", "funnel endpoint failed")}
          ...
          {empty ? (
            ...
            <div className="grid cols-3">
              <div>
                ...
                <DistBars rows={labeledActionRows} tone="var(--green)" />
              </div>
              <div>
