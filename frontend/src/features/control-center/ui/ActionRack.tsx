...
import { modeLabel, type GlowTone } from "./tones";
        ...
        <div className="ctl-rack">
          <div className="ctl-rack-group">
            <div className="ctl-rack-head">
              ...
              <span className={`ctl-chip ctl-chip--${engineChipTone}`}>
                {engineChip === "ON"
                  ? t("control-center.engine.on", "ON")
                  : engineChip === "OFF"
                    ? t("control-center.engine.off", "OFF")
                    : t("control-center.engine.unknown_chip", "UNKNOWN")}
              </span>
            </div>
            <div className="ctl-rack-actions">
              <button
                ...
                title={engineCmd.state.running ? t("control-center.rack.in_flight", "engine command already in flight") : undefined}
                onClick={() => setStartConfirm(true)}
              >
                {engineCmd.state.running ? t("control-center.rack.sending", "sending…") : t("control-center.action.start_engine", "▶ Start engine")}
              </button>
              ...
              >
                {engineCmd.state.running ? t("control-center.rack.sending", "sending…") : t("control-center.action.stop_engine", "■ Stop engine (kill switch)")}
              </button>
              <span className="ctl-guard" title={t("control-center.rack.confirm_hint", "opens the existing confirmation dialog")}>
                {t("control-center.rack.requires_confirm", "⚠ requires confirm")}
              </span>
            </div>
            <div className="ctl-rack-note">{t("control-center.rack.enable_rules", "Enable rules unchanged: Start while STOPPED, Stop while RUNNING — the snapshot decides, chips only re-state it.")}</div>
          </div>
          ...
          <div className="ctl-rack-group">
            <div className="ctl-rack-head">
              ...
              <span className="ctl-sep" aria-hidden="true" />
              <span className={`ctl-chip ctl-chip--${modeTone}`}>{modeLabel(t, mode)}</span>
              {armed && (
                <span className="ctl-chip ctl-chip--armed">
                  {t("control-center.rack.armed", "ARMED")} · {modeLabel(t, modeTarget)}
                </span>
              )}
            </div>
            <div className="ctl-rack-actions">
              ...
              <button
                ...
                title={modeCmd.state.running ? t("control-center.rack.mode_in_flight", "mode switch already in flight") : undefined}
                onClick={() => void applyMode()}
              >
                {modeCmd.state.running ? t("config.apply.applying", "applying…") : t("control-center.action.apply", "apply")}
              </button>
              {typedGuard && (
            ...
            <div className="ctl-rack-note">
              {t(
                "control-center.panel.engine_commands_note",
                "Commands go to /api/engine/toggle + /api/engine/mode (BUG-148 hot-swap path). The UI never shows a locally-changed state — the next authoritative snapshot decides.",
              )}
            </div>
          </div>
        </div>
      ...
      {(startConfirm || stopConfirm) && (
        ...
        >
          <div className="small">
            {stopConfirm
              ? t(
                  "control-center.confirm.stop_body",
          ...
          <div className="confirm-box">
            ...
            <div className="row">
              ...
              <button
                ...
                disabled={liveConfirm !== LIVE_CONFIRM_TEXT || modeCmd.state.running}
                title={modeCmd.state.running ? t("control-center.rack.mode_in_flight", "mode switch already in flight") : undefined}
                onClick={() => void applyMode()}
              >
                {modeCmd.state.running ? t("control-center.rack.applying", "applying…") : t("control-center.live.switch", "switch to LIVE")}
              </button>
            </div>
          </div>
        </ConfirmModal>
