...
export function TraceTab() {
  ...
  const timeline = trace.data ? traceTimeline(trace.data, t) : [];
  const rowCount = (trace.data?.signal?.length ?? 0) + (trace.data?.orders?.length ?? 0);

  return (
    <Panel title={t("debug.trace.title", "Execution trace ({path})", { path: "/api/debug/trace/{execution_id}" })} accent right={<span className="timestamp-note">{t("debug.trace.right", "read-only join: audit_signals + audit_orders")}</span>}>
        ...
        {!submitted ? (
          ...
          <>
            <div className="timestamp-note" style={{ marginBottom: 6 }}>
              {t("debug.trace.summary", "{id} · {signal} signal row(s) · {order} order row(s)", { id: submitted, signal: trace.data.signal?.length ?? 0, order: trace.data.orders?.length ?? 0 })}
              {trace.data.db_path ? t("debug.trace.db_path", " · db {db}", { db: trace.data.db_path }) : ""}
            </div>
            {rowCount === 0 ? (
              <>
                <div className="l3-note warn">
                  {t("debug.trace.rows_empty", "Rows empty{reason}. The id is valid; the join found nothing to place on the timeline.", { reason: trace.data.reason ? t("debug.trace.rows_reason", " — backend reason: {r}", { r: String(trace.data.reason) }) : "" })}
                </div>
                <EmptyState
                  message={t("debug.trace.nothing", "Nothing to trace for this execution id.")}
                  hint={t("debug.trace.nothing_hint", "Try an id from a window where the audit DB was attached.")}
                />
              </>
            ) : (
