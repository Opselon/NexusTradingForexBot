        ...
        right={
          <>
            <FreshnessCaption
              ...
              intervalMs={30_000}
              note={manage.data?.password_set ? t("database.manage.pw_stored", "pg password stored") : t("database.manage.pw_missing", "pg password missing")}
              stale={false}
            />
        ...
        {!md ? (
          ...
          <>
            ...
            <div className="dbc-kpis" style={{ marginBottom: 12 }}>
              <MetricCard label={t("database.status.active_provider", "active provider")} value={<span className="inline-mono">{md.provider ?? "—"}</span>} sub={t("database.status.active_provider_sub", "applies on next restart")} />
              <MetricCard label={t("database.status.overall_health", "overall health")} value={<StatusBadge status={md.overall ?? "UNKNOWN"} />} />
              <MetricCard
              ...
              />
              <MetricCard
                label={t("database.manage.group_provider", "psycopg")}
            ...
            {preview.data?.body?.preview && (
              <div style={{ marginTop: 10 }}>
                <div className="dbc-section-title">
                  {t("database.manage.preview_title", "dry-run preview ({tables} tables · {size})", { tables: (preview.data.body.preview.tables ?? []).length, size: formatBytes(preview.data.body.preview.estimated_volume_bytes) })}
                </div>
            ...
            {(migrating || !jobDone) && (progress.data || migrate.data) && (
              <div className="dbc-progress" style={{ marginTop: 12 }} role="progressbar" aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100}>
                <div className="lab">
                  <span>
                    {jobDone
                      ...
                      : t("database.manage.running", "running · {table} · {copied}/{total} rows", { table: progress.data?.current_table || t("database.manage.preparing", "preparing"), copied: progress.data?.rows_copied ?? 0, total: progress.data?.total_rows ?? "?" })}
                  </span>
                  <span>{pct}%</span>
                </div>
                <div className="track">
                ...
                <div className="dbc-row between">
                  <span className="dbc-sub">{t("database.manage.progress_stream", "numbers stream from /api/db/manage/progress")}</span>
                  <span>
            ...
            {lastReport && (
              <div style={{ marginTop: 10 }}>
                ...
                {lastReport.errors && lastReport.errors.length > 0 && (
                  <div className="dbc-note bad">{t("database.manage.errors", "errors: {errors}", { errors: lastReport.errors.slice(0, 5).join(" · ") })}</div>
                )}
                {lastReport.warnings && lastReport.warnings.length > 0 && (
                  <div className="dbc-note warn">{t("database.manage.warnings", "warnings: {warnings}", { warnings: lastReport.warnings.slice(0, 5).join(" · ") })}</div>
                )}
              </div>
            )}
          </>
      ...
      {guard && (
        <TypedConfirmModal
          ...
          body={
            guard === "backup" ? (
              <>
                {t("database.confirm.backup_body_pre", "Writes")} <span className="inline-mono">artifacts/backups/audit_backup_&lt;ts&gt;.db</span> {t("database.confirm.backup_body_mid", "using the streaming SQLite backup API (consistent snapshot; the engine keeps running). The backend result decides the verdict.")}
              </>
            ) : (
              <>
