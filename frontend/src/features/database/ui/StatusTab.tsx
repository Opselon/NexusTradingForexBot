...
function KpiStrip({
  ...
  // Each derivation is a pure function of one payload;
  ...
  return (
    <div className="dbc-kpis">
      <MetricCard
        label={mismatch ? t("database.status.label_configured_effective", "configured → effective") : t("database.status.active_provider", "active provider")}
        value={
          mismatch && truth ? (
        ...
        sub={
          mismatch
            ...
            : driverMissing
              ...
              : t("database.status.restart_sub", "switch applies on next restart")
        }
        tone={mismatch || driverMissing ? "neg" : undefined}
      />
      <MetricCard label={t("database.status.overall_health", "overall health")} value={<StatusBadge status={overall} />} sub="db/manage/status" />
      <MetricCard
        label={t("database.status.pending_migrations", "pending migrations")}
        value={pend.pending ?? "—"}
        tone={pend.pending && pend.pending > 0 ? "neg" : "dim"}
        sub={pend.behind.length ? t("database.status.behind_schema", "{domains} behind schema", { domains: pend.behind.join(", ") }) : t("database.status.all_at_expected", "all domains at expected schema")}
      />
      <MetricCard
...
function DomainTable({ status, t }: { status: DbStatus | undefined; t: TFunc }) {
  const rows = useMemo(() => (status ? domainRows(status.databases) : []), [status]);
  if (!status) return <Skeleton count={3} />;
  if (status.available === false) {
    const fallback = t("database.status.no_state_published", "no state published");
    const msg = typeof status.error === "object" ? status.error?.message ?? fallback : String(status.error ?? fallback);
    return <div className="dbc-note bad">{t("database.status.schema_unavailable", "Schema state unavailable: {msg}", { msg })}</div>;
  }
  if (rows.length === 0) {
    return <EmptyState message={t("database.status.domains_empty", "No database domains reported.")} hint={t("database.status.domains_empty_hint", "The backend has not published schema/migration state for any domain.")} />;
  }
  return (
    <DataTable
      headers={[
...
function HygieneBody({ hygiene, t }: { hygiene: DbHygiene; t: TFunc }) {
  ...
  if (!worker && storage.length === 0 && plans.length === 0) {
    return <EmptyState message={t("database.status.hygiene_empty_body", "Hygiene worker reported no status, plans or storage.")} hint={t("database.status.hygiene_raw_hint", "Raw payload is below.")} />;
  }
  return (
    <>
      {worker && (
        <div className="dbc-kpis" style={{ marginBottom: 4 }}>
          <MetricCard label={t("database.status.worker_status", "worker state")} value={<StatusBadge status={worker.state} />} sub={t("database.status.mode_of", "mode {mode}", { mode: worker.mode })} />
          <MetricCard label={t("database.status.execution_mode", "execution mode")} value={<span className="inline-mono">{worker.executionMode}</span>} sub={t("database.status.execution_mode_sub", "scope of the sweep")} />
          <MetricCard
            label={t("database.status.cycle_label", "cycle")}
            value={worker.cycle ?? "—"}
            sub={worker.lastFailure ? t("database.status.last_failure", "last failure {at}", { at: worker.lastFailure }) : t("database.status.no_failure", "no recorded failure")}
          />
          <MetricCard
      ...
      {storage.length > 0 && (
        <>
          ...
          <DataTable headers={[{ label: t("database.th.database", "DATABASE") }, { label: t("database.th.size", "SIZE"), num: true }, { label: t("database.th.wal", "WAL"), num: true }]}>
            {storage.map((s) => (
              <tr key={s.database}>
          ...
          >
            {plans.map((p) => (
              <tr key={p.database}>
                ...
                <td className="num">
                  {p.duplicates ?? "—"}
                  {typeof p.exactDuplicates === "number" ? t("database.status.exact_suffix", " ({n} exact)", { n: p.exactDuplicates }) : ""}
                </td>
  ...
  return (
    <div className="dbc-stack">
        ...
        {!md ? (
          manage.isError ? (
            <div className="dbc-note bad">
              {t("database.status.provider_unavailable", "Provider state unavailable: {msg}", { msg: manage.error instanceof Error ? manage.error.message : t("database.status.backend_failed", "backend request failed") })}
            </div>
          ) : (
            <Skeleton count={3} />
          )
          ...
          <>
            ...
            <div className="dbc-row between">
              <div className="dbc-chips">
                {(md.supported_providers ?? []).map((p) => (
                  <span
                    ...
                    title={p === md.provider ? t("database.status.chip_active_title", "active provider (switch applies on next restart)") : t("database.status.chip_switch_title", "switch to {p} on the Manage tab", { p })}
                  >
              ...
              <span className="dbc-sub">
                {t("database.status.pg_password_label", "pg password")} {md.password_set ? t("database.status.pw_set_store", "SET (OS SecretStore)") : t("database.status.pw_missing", "MISSING")}
            ...
            <DomainTable status={status.data} t={t} />
            {md.domains && (
      ...
      <QuerySection<DbStatus>
        ...
        right={
          <>
            <FreshnessCaption fetchedAtMs={status.dataUpdatedAt || null} intervalMs={30_000} note={t("database.status.readonly_note", "read-only; the API never runs migrations")} stale={poll.paused} />
