 ...
 *
 * Status notes resolve at render (lazy (t) => string closures re-translate on a
 * language switch; plain strings keep the language they were built with).
 */
...
/** One feed note: lazy closures re-translate on render, plain strings keep the
 *  language they were built with (verdict helpers applied at the call site). */
type FeedNote = { err: boolean; text: NoteText };
    ...
    >
      ...
      {selected && (
        <ArticleDrawer
          ...
          busy={analyze.isPending}
          analyzeNote={noteText(note?.text, t)}
