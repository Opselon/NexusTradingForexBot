  ...
  return (
      ...
      right={
        <>
          <select
            ...
            aria-label={t("news.keywords.category_label", "category")}
          >
    ...
    >
      <div className="grid cols-4">
        <MetricCard
          label={t("news.keywords.metric_dataset", "dataset keywords")}
          value={data?.dataset?.total_keywords ?? "—"}
          tone="dim"
          sub={`v${data?.dataset?.version ?? "—"}`}
        />
        <MetricCard
          label={t("news.keywords.metric_scanned", "articles scanned")}
          value={cov?.articles_scanned ?? "—"}
          tone="dim"
          sub={t("news.keywords.metric_scanned_sub", "last 500 canonical")}
        />
        <MetricCard
        ...
        <MetricCard
          ...
          sub={t(
            ...
            "neutral {n} · bullish / bearish keywords",
            { n: dir.NEUTRAL ?? 0 },
          )}
        />
      </div>
      ...
      {keywords.isPending ? (
        ...
        <EmptyState
          ...
          hint={t(
            ...
            "Fetch news to populate — the scan covers the last 500 canonical articles.",
          )}
        />
      ) : (
        <>
          ...
          >
            {tops.map((k) => (
              <tr key={k.keyword}>
                ...
                <td>{k.category ?? "—"}</td>
                <td>
                  <span className={`news-dir ${(k.direction_bias ?? "NEUTRAL").toUpperCase()}`}>
                    {dirWord(t, k.direction_bias ?? "NEUTRAL")}
                  </span>
                </td>
          ...
          <div style={{ marginTop: 10 }}>
            <HeatBar
              items={tops.slice(0, 8).map((k) => ({
                ...
                title: t("news.keywords.weight_title", "{category} · weight {w}", {
                  category: k.category ?? "",
                  w: k.weight ?? "—",
                }),
              }))}
          ...
          {listing.length > 0 && (
            <div className="tiny faint" style={{ marginTop: 8 }}>
              {t(
                ...
                "{n} dataset keywords match the current filter (top {top} shown above).",
                { n: listing.length, top: tops.length },
              )}
            </div>
          )}
        </>
