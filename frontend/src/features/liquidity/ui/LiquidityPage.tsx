    ...
    <div className="liq-page">
      <header className="liq-hero">
        <div className="liq-hero-main">
          <div className="liq-kicker">
            ...
            {t("ux.sidebar.features.market", "MARKET & RESEARCH")}
          </div>
          <h1 className="liq-title">
            <span className="word">{t("nav.feature.liquidity", "Liquidity")}</span>
          </h1>
          <p className="liq-desc">
            {t(
              "liquidity.page.desc",
              "Liquidity intelligence governor + MSLIE market structure — the ten-value runtime snapshot, the calculation/source state split, and the live liquidity-map / sweep perception. All states below are read from the backend; the console never infers a sweep that was not recorded.",
            )}
          </p>
        </div>
        <div className="liq-hero-side">
          <div className="liq-side-pills">
            <span
              ...
              title={t("liquidity.hero.governor_title", "backend governor state from /api/liquidity/state")}
            >
              <span className="d" aria-hidden="true" />
              {s
                ? enabled
                  ? t("liquidity.hero.governor_enabled", "governor enabled")
                  : t("liquidity.hero.governor_disabled", "governor disabled")
                : t("liquidity.hero.governor_unknown", "governor unknown")}
            </span>
            <span
              className="liq-side-pill"
              title={t(
                "liquidity.hero.calc_title",
                "calculation_status — did the last compute attempt succeed (BUG-110: orthogonal to source)",
              )}
            >
              {t("liquidity.hero.calc", "calc")} <StatusPill status={s?.calculation_status ?? "NOT_RUN"} />
            </span>
            <span
              className="liq-side-pill"
              title={t(
                "liquidity.hero.source_title",
                "source_status — where the input data came from (BUG-110: orthogonal to calculation)",
              )}
            >
              {t("liquidity.hero.source", "src")} <StatusPill status={s?.source_status ?? s?.source ?? "UNKNOWN"} />
            </span>
          </div>
          <div className="liq-ep-chips" role="list" aria-label={t("liquidity.hero.ep_a11y", "Liquidity provenance endpoints")}>
      ...
      <div>
        <div className="liq-sec-label">
          <span className="liq-sec-i">01</span> {t("liquidity.section.governor", "Governor state")}
          <span className="liq-sec-note">{t("liquidity.section.governor_note", "calculation vs source kept separate (BUG-110)")}</span>
        </div>
        <div className="liq-state-grid">
          <MetricCard
            ...
            sub={t("liquidity.card.governor_sub", "source {src} · algo {algo}", {
              src: s?.source ?? "—",
              algo: s?.algorithm_version ?? "—",
            })}
      ...
      <div>
        <div className="liq-sec-label">
          <span className="liq-sec-i">02</span> {t("liquidity.section.snapshot", "Runtime snapshot")}
          <span className="liq-sec-note">{t("liquidity.section.snapshot_note", "ten backend values + governor pools")}</span>
        </div>
          ...
          <Panel title={t("liquidity.panel.pools", "Liquidity pools (governor snapshot)")} tight>
            {pools.length === 0 ? (
              stateQ.isPending ? (
                ...
                <EmptyState
                  message={t(
                    "liquidity.empty.pools",
                    "No pools in the current snapshot (engine not running or features disabled).",
                  )}
                />
              )
            ) : (
      ...
      <div>
        <div className="liq-sec-label">
          <span className="liq-sec-i">03</span> {t("liquidity.section.mslie", "MSLIE sweep / liquidity-map view")}
          <span className="liq-sec-note">
            {t("liquidity.section.mslie_note", "heat scaled by backend probability_as_target")}
          </span>
        </div>
        <Panel
            ...
            <div className="grid cols-2">
              <div>
                <div className="section-title">
                  {t("liquidity.section.map", "liquidity map (heat list, ranked by target probability)")}
                </div>
                {zones.length === 0 ? (
                  <EmptyState
                    message={t("liquidity.empty.zones", "No liquidity zones mapped yet (engine STANDBY until bars arrive).")}
                  />
                ) : (
                  <DataTable
                    headers={[
                            ...
                            <span
                              ...
                              aria-label={
                                z.probability === null
                                  ...
                                  : t("liquidity.heat.prob", "probability {p} percent", {
                                      p: (z.probability * 100).toFixed(0),
                                    })
                              }
                            >
                              <i
              ...
              <div>
                ...
                {mslieQ.data?.last_sweep ? (
                  <dl className="kv liq-sweep-dl">
                    ...
                    <InfoRow
                      label={t("liquidity.kv.after_state", "after-event state")}
                      value={<StatusPill status={str(sweep.after_event_state)} />}
                    />
      ...
      <div>
        <div className="liq-sec-label">
          <span className="liq-sec-i">04</span> {t("liquidity.section.model_input", "Model input")}
          <span className="liq-sec-note">{t("liquidity.section.model_input_note", "MSLIE feature vector, read-only")}</span>
        </div>
      ...
      {confirmTo !== null && (
        <ConfirmModal
          title={
            confirmTo
              ? t("liquidity.confirm.enable_title", "Enable Liquidity Intelligence")
              : t("liquidity.confirm.disable_title", "Disable Liquidity Intelligence")
          }
          danger={!confirmTo}
