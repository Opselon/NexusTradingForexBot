    ...
    <div className="liq-page">
      <header className="liq-hero">
        <div className="liq-hero-main">
          <div className="liq-kicker">
            ...
            {t("liquidity.page.kicker", "MARKET & RESEARCH")}
          </div>
          <h1 className="liq-title">
            <span className="word">{t("liquidity.page.title", "Liquidity")}</span>
          </h1>
        ...
        <div className="liq-hero-side">
          ...
          <div
            className="liq-ep-chips"
            role="list"
            aria-label={t("liquidity.hero.ep_a11y", "Liquidity provenance endpoints")}
          >
      ...
      <Panel
        ...
        right={
          <button
            ...
            title={cmd.state.running ? t("liquidity.btn.toggle_inflight", "toggle already in flight") : undefined}
            onClick={() => setConfirmTo(!enabled)}
          >
            {cmd.state.running
              ? enabled
                ? t("liquidity.btn.disabling", "disabling…")
                : t("liquidity.btn.enabling", "enabling…")
              : enabled
      ...
      <div>
        <div className="liq-sec-label">
          ...
          <span className="liq-sec-note">{t("liquidity.section.mslie_note", "heat scaled by backend probability_as_target")}</span>
        </div>
        <Panel
                ...
                <div>
                  ...
                  {mslieQ.data?.last_sweep ? (
                    <dl className="kv liq-sweep-dl">
                      ...
                      <InfoRow label={t("liquidity.kv.price_pool", "price / pool")} value={`${formatPrice(num(sweep.price), 2)} / ${formatPrice(num(sweep.pool_price), 2)}`} />
                      <InfoRow label={t("ux.signal.confidence", "confidence")} value={num(sweep.confidence) === null ? "—" : `${formatNumber(num(sweep.confidence)!, 1)}%`} />
