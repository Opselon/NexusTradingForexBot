  ...
  return (
    <Drawer title={t("ai-analysis.decision.title", "Decision {id}…", { id: decisionId.slice(0, 16) })} onClose={onClose}>
      <div className="grid cols-3" style={{ marginBottom: 12 }}>
        <MetricCard label={t("ai-analysis.decision.action", "Action")} value={d?.action ? actionLabel(t, d.action) : "—"} tone={actionTone(d?.action) === "buy" ? "pos" : actionTone(d?.action) === "sell" ? "neg" : "dim"} />
        ...
        <Panel title={t("ai-analysis.decision.levels", "Levels (recorded or none)")} tight>
          <dl className="kv" style={{ marginBottom: 10 }}>
            <InfoRow label={t("ai-analysis.decision.proposed_entry", "proposed entry")} value={formatPrice(d.proposed_entry, 2)} />
                        ...
                        <InfoRow label={t("ai-analysis.decision.execution_mode", "execution mode")} value={d.execution_mode ? modeLabel(t, d.execution_mode) : "—"} />
                        <InfoRow label={t("ai-analysis.decision.regime", "regime")} value={d.regime ?? "—"} />
          </dl>
          <ProbBar
            rows={[
              { label: t("ai-analysis.decision.p_confidence", "P (confidence)"), value: confidence01(d.confidence), tone: actionTone(d.action) },
      ...
      <Panel title={t("ai-analysis.decision.gate_trace", "Gate trace")} tight>
        {gatesQ.isPending ? (
          ...
          (() => {
            ...
            if (gates.length === 0) return <EmptyState message={t("ai-analysis.decision.gates_empty", "No gate trace recorded for this decision.")} />;
                ...
                <div className="aa-gate-sum">
                  ...
                  {firstFail && (
                    <span className="tiny aa-firstfail" title={str(firstFail.value) ?? ""}>
                      {t("ai-analysis.decision.first_failure", "first failure: {gate}", { gate: firstFail.gate })}
                    </span>
                  )}
                </div>
                <GateStepper
      ...
      <div className="tiny muted" style={{ marginTop: 8 }}>
        {t("ai-analysis.decision.pills_note_pre", "status pills mirror backend values only;")} <StatusPill status={d?.execution_mode ?? "—"} /> {t("ai-analysis.decision.pills_note_post", "is the recorded execution mode.")}
      </div>
    </Drawer>
  );
}
