    ...
    >
      {modeQuery.isPending ? (
        ...
        <div className="l3-note bad">
          ...
          <button className="btn small" onClick={() => void modeQuery.refetch()}>{t("common.retry", "Retry")}</button>
        </div>
      ) : (
              ...
              <div className="cfg-preview">
                ...
                {!previewBusy && !preview.isError && pv && pv.validation.valid && pv.impact.touches.length > 0 && (
                  <div className="cfg-impact">
                    ...
                    {pv.impact.touches.map((touch) => (
                      <span key={touch} className="cfg-impact-chip">{touch}</span>
                    ))}
                  </div>
                )}
              </div>
...
function RuntimeConfigForm() {
  ...
  const errors = useMemo(() => validateFields(specs, values, DEFAULT_RULES, t), [specs, values, t]);
    ...
    >
      <div className="l3-form">
        <FieldRow label={spec.label} hint={spec.key} error={path === "" ? null : err}>
          <TextField value={path} onChange={setPath} error={err} placeholder={t("config.swap.ph", "artifacts/model.pt")} spec={spec.label} />
        </FieldRow>
      </div>
      <div className="l3-toolbar cfg-actions">
      ...
      {status.isPending ? (
        ...
        <div className="l3-form">
          ...
          <FieldRow label={specs[0]!.label!} hint={t("config.tg.token_hint_text", "leave empty to keep the stored secret — it never round-trips in plaintext (BUG-072)")} error={firstError(errors, "bot_token")}>
            <TextField value={token} onChange={setToken} error={firstError(errors, "bot_token")} placeholder={st?.masked_token || "123456:ABC-DEF…"} spec={specs[0]!.label!} />
          </FieldRow>
