  ...
  const runReplayToggle = async (active: boolean): Promise<void> => {
    ...
    try {
      ...
      setReplayCmd({
        ...
        msg: res.message ??
          (refused
            ? t("dash.replay.refused", "Backend refused the replay toggle.")
            : active
              ? t("dash.replay.engaged_msg", "replay engaged.")
              : t("dash.replay.released_msg", "replay released.")),
        ok: !refused,
      });
    ...
    <div>
      ...
      <div className="grid cols-4">
        ...
        <MetricCard
          ...
          sub={
            <>
              {t("dash.card.mode_sub", "data_source: {v}", { v: snapshot.data_source ?? "—" })}
              {snapshot.mode_source_mismatch ? t("dash.card.mismatch", " · MISMATCH!") : ""}
            </>
          }
        />
        <MetricCard
      ...
      <div className="l4-toolbar l4-section-gap" aria-label={t("dash.prov.aria", "value provenance")}>
        ...
        <AgeNote label={t("dash.age.tick", "tick age")} ageSec={snapshot.diagnostics.tick_age_sec} suffix={snapshot.tick_stale ? t("dash.age.stale_suffix", "STALE") : undefined} />
        ...
        <span style={{ marginInlineStart: "auto" }} className="l4-chip-row">
          <InfoChip
            ...
            v={replaying === null ? "—" : replaying ? t("dash.replay.engaged", "ENGAGED") : t("dash.replay.off", "OFF")}
            tone={replaying ? "warn" : ""}
          />
        </span>
      </div>
          ...
          right={
            <>
              <span className="timestamp-note">
                {chartBars.length
                  ? t("dash.chart.forming_bars", "{n} forming · {m} bars", { n: chartBars.filter((b) => b.is_complete === false).length, m: chartBars.length })
          ...
          <div style={{ padding: 10 }}>
            ...
            {snapshot.tick_stale && (
              <div className="confirm-box" style={{ borderColor: "rgba(235,161,63,0.5)" }}>
                <span>
                  {t("dash.chart.stale_notice", "Backend marks the tick stream stale (freshness {f}) — prices and candles above may be frozen at the last real tick.", {
                    f: snapshot.tick_freshness_ms === null ? "—" : `${(snapshot.tick_freshness_ms / 1000).toFixed(1)}s`,
                  })}
                </span>
              </div>
            )}
            ...
            <div>
              ...
              <div className="meta">
                <span className={`l4-chip ${snapshot.probs.available ? "good" : "warn"}`}>
                  {snapshot.probs.available ? t("dash.pred.probs_live", "probs LIVE") : t("dash.pred.probs_unavailable", "probs UNAVAILABLE")}
                </span>
                <span className="l4-chip">
                  {t("dash.pred.inference", "inference {t}", { t: snapshot.probs.inference_timestamp ? formatTime(snapshot.probs.inference_timestamp) : "—" })}
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="grid cols-2 l4-section-gap">
        ...
        <Panel
          ...
          right={<TriBadge value={replaying} on={t("dash.replay.engaged", "ENGAGED")} off={t("dash.replay.off", "OFF")} />}
        >
          <div className="l4-transport">
            <button
      ...
      <div className="grid cols-2">
        ...
        <Panel title={t("nav.page.ml", "ML / 70D")} right={<span className="l4-chip">{snapshot.model.feature_schema_id ?? t("dash.ml.schema_none", "schema —")}</span>}>
          <dl className="kv">
            ...
            <dd dir="ltr">{snapshot.model.latency_ms === null ? "—" : `${snapshot.model.latency_ms.toFixed(1)} ms`}</dd>
      ...
      <div className="grid cols-2">
        <Panel title={t("dash.pos.title", "Open positions ({n})", { n: positions.length })} tight>
          {positions.length === 0 ? (
            ...
            <DataTable
              headers={[
                { label: t("trading.th.ticket", "Ticket") },
                { label: t("trading.th.symbol", "Symbol") },
                { label: t("dash.pos.side", "Side") },
                { label: t("trading.th.volume", "Volume"), num: true },
        ...
        <Panel title={t("dash.mt5.title", "MT5 status detail")} tight>
          {mt5Query.isPending ? (
            ...
            <dl className="kv" style={{ padding: "10px 14px" }}>
              ...
              <dt>{t("dash.mt5.pending", "pending orders (broker)")}</dt>
      ...
      {stopConfirm && (
        ...
        >
          <div>
            ...
            <div className="small muted" style={{ marginTop: 8 }}>{t("trading.confirm.stop_recovery", "Recovery: Start engine re-attaches the loop; the backend refuses the command if the runtime state forbids it.")}</div>
          </div>
        </ConfirmModal>
      )}
      {replayConfirm !== null && (
        ...
        >
          <div>
            {replayConfirm ? (
              <>
                <b>{t("trading.confirm.impact", "Impact:")}</b> {t("dash.replay.enter_body", "the engine switches to the historical replay stream on the simulation boundary. The backend refuses this while mode=LIVE.")}
              ...
              <>
                <b>{t("trading.confirm.impact", "Impact:")}</b> {t("dash.replay.leave_body", "leaves replay; the pre-replay mode is restored by the backend and live ticks resume.")}
