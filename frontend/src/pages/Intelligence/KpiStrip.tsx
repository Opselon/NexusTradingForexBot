...
function ScoreRow({ label, value, tone }: { label: string; value: number | null | undefined; tone: "bull" | "bear" | "accent" }) {
  ...
  return (
    <div className={`ix-kpi-srow${v === null ? " is-empty" : ""}`}>
      ...
      <span className="track" role="img" aria-label={`${label}: ${v === null ? "unavailable" : v}`}>
        {frac !== null && <i className={tone} style={{ inlineSize: `${frac * 100}%` }} />}
      </span>
      <span className="num" title={v === null ? `${label}: not sent by the backend` : `${label} = ${v} (verbatim backend 0–1 field)`}>
        {raw(v)}
      </span>
    </div>
  );
...
<div className="ix-kpi-wrap">
  <div className="grid cols-4 ix-kpis">
    ...
    <article className="ix-kpi-card ix-kpi--state">
      <div className="ix-kpi-head">
        ...
        <span className="ix-kpi-lab">News state (backend)</span>
        {stale && (
          <span className="ix-kpi-flag" title="stale: true — the backend marks this context stale (verbatim flag)">
            STALE
          </span>
        )}
      </div>
      <div className={`ix-kpi-val${loading || !available ? " is-dim" : breaking ? " is-neg" : ""}`}>
        {loading ? "…" : available ? (ns?.state ?? "—") : "UNAVAILABLE"}
      </div>
      <div className="ix-kpi-sub">
        {loading
          ? "awaiting first backend payload"
          : !available
            ? "news subsystem disabled or offline"
            : stale
              ? "⚠ backend marks context STALE"
              : `freshness ${raw(ns?.freshness)}`}
      </div>
    </article>
    ...
    <article className="ix-kpi-card ix-kpi--sentiment">
      <div className="ix-kpi-head">
        <span className="ix-kpi-ico" aria-hidden="true">▲▼</span>
        <span className="ix-kpi-lab">Bullish / Bearish</span>
      </div>
      <div className={`ix-kpi-val${loading || !available ? " is-dim" : ""}`}>
        {loading ? "…" : available ? `${pct(ns?.bullish_score)} / ${pct(ns?.bearish_score)}` : "—"}
      </div>
      <div className="ix-kpi-sub">
        <ScoreRow label="BULL" tone="bull" value={available ? ns?.bullish_score : null} />
        <ScoreRow label="BEAR" tone="bear" value={available ? ns?.bearish_score : null} />
        <span className="ix-kpi-cap">backend-scored sentiment (display only)</span>
      </div>
    </article>
    ...
    <article className="ix-kpi-card ix-kpi--confidence">
      <div className="ix-kpi-head">
        <span className="ix-kpi-ico" aria-hidden="true">◐</span>
        <span className="ix-kpi-lab">Context confidence</span>
      </div>
      <div className={`ix-kpi-val${loading || pct(ns?.confidence) === "—" ? " is-dim" : ""}`}>
        {loading ? "…" : pct(ns?.confidence)}
      </div>
      <div className="ix-kpi-sub">
        <ScoreRow label="confidence" tone="accent" value={ns?.confidence} />
        <ScoreRow label="XAUUSD rel." tone="accent" value={ns?.xauusd_relevance} />
        <span className="ix-kpi-cap">raw 0–1 shown beside every bar</span>
      </div>
    </article>
    ...
    <article className={`ix-kpi-card ix-kpi--events${eventPulse ? " has-pulse" : ""}`}>
      <div className="ix-kpi-head">
        <span className="ix-kpi-ico" aria-hidden="true">◉</span>
        <span className="ix-kpi-lab">Active events</span>
      </div>
      <div className={`ix-kpi-val${loading || (ns?.active_event_count ?? null) === null ? " is-dim" : ""}`}>
        {loading ? "…" : (ns?.active_event_count ?? "—")}
      </div>
      <div className="ix-kpi-sub">
        <span>context time {ns?.timestamp ? formatDateTime(ns.timestamp) : "—"}</span>
        {!loading && !available && <span className="ix-kpi-cap">news subsystem disabled or offline</span>}
      </div>
    </article>
  </div>
  ...
  {fetchedAgo !== null && (
    <div
      ...
      title="Client-side cache age of the news-state query (react-query dataUpdatedAt) — not a backend payload field."
    >
