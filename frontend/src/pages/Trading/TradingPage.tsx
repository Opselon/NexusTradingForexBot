...
export default function TradingPage({ snapshot, nowMs }: Props) {
  ...
  /* Display-only impact copy — built during render so it follows t(); the
     mode VALUES (PAPER/SHADOW/LIVE) stay untranslated enum identifiers. */
  ...
  return (
    <div>
      ...
      <div className="grid cols-2">
        ...
        >
          <dl className="kv">
            ...
            <dd>{snapshot.spread === null ? "—" : <span dir="ltr">{`${formatNumber(snapshot.spread)} pts`}</span>}</dd>
          ...
          {mt5Query.data?.orders && mt5Query.data.orders.length > 0 ? (
            <div className="table-wrap">
              <table className="data-table">
                <thead>
                  <tr><th>{t("trading.th.ticket", "Ticket")}</th><th>{t("trading.th.type", "Type")}</th><th>{t("trading.th.volume", "Volume")}</th><th>{t("trading.th.price", "Price")}</th><th>{t("trading.th.state", "State")}</th><th>{t("trading.th.setup", "Setup")}</th></tr>
                </thead>
                <tbody>
            ...
            <div className="l4-toolbar" style={{ padding: "8px 12px 0" }}>
              {ordersQuery.data?.latency ? (
                <>
                  ...
                  <InfoChip k="p50" v={<span dir="ltr">{`${formatNumber(ordersQuery.data.latency.p50_ms, 1)} ms`}</span>} tone="accent" />
                  <InfoChip k="p95" v={<span dir="ltr">{`${formatNumber(ordersQuery.data.latency.p95_ms, 1)} ms`}</span>} />
                  <InfoChip k="p99" v={<span dir="ltr">{`${formatNumber(ordersQuery.data.latency.p99_ms, 1)} ms`}</span>} />
                </>
              ) : (
          ...
          <SortableTable
            columns={[
              ...
              },
              {
                ...
                render: (r) =>
                  r.broker ? (
                    <span style={{ display: "inline-flex", gap: 6, alignItems: "center" }} dir="ltr">
            ...
            <div className="grid cols-2">
              ...
              <div>
                ...
                <dl className="kv">
                  ...
                  <dd dir="ltr">{bos.length ? bos.slice(-6).map((l) => `${String(l.type ?? "BOS").split("_")[0]}@${formatPrice(num(l.price), digits)}`).join(" · ") : "—"}</dd>
                  <dt>{t("trading.smc.equilibrium", "equilibrium")}</dt>
                  <dd dir="ltr">{mids.length ? mids.map((m) => `${formatPrice(num(m.price), digits)} (${String(m.label ?? "50%")})`).join(" · ") : "—"}</dd>
                  <dt>{t("trading.smc.liquidity", "liquidity sweeps")}</dt>
                  <dd dir="ltr">{liq.length ? liq.slice(-6).map((m) => `${String(m.type ?? "").includes("BUY") ? "BSL" : "SSL"}@${formatPrice(num(m.price), digits)}`).join(" · ") : "—"}</dd>
                  <dt>{t("trading.smc.algo", "algo config")}</dt>
                  <dd className="small" dir="ltr">
        ...
        >
          {(d) => (
            <div className="table-wrap">
              <table className="data-table">
                <thead>
                  <tr><th>#</th><th>{t("trading.th.order_id", "Order id")}</th><th>{t("trading.th.symbol", "Symbol")}</th><th>{t("trading.th.type", "Type")}</th><th>{t("trading.th.volume", "Volume")}</th><th>{t("trading.th.price", "Price")}</th><th>{t("trading.th.status", "Status")}</th><th>{t("trading.th.executed", "Executed")}</th></tr>
                </thead>
                <tbody>
                  {d.items.map((r, i) => (
      ...
      {stopConfirm && (
        <ConfirmModal
          title={t("ux.confirm.title", "Confirm action") + " — " + t("trading.confirm.stop_suffix", "STOP ENGINE")}
