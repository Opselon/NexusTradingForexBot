...
function VerdictChip({ disagreement, valid }: { disagreement: string; valid?: boolean }) {
  ...
  return (
    <span className="tiny" style={{ color: "var(--rose)" }} title={t("ml.verdict.tip", "No shadow inference ran for this tick — the shadow model was not attached or the 70D vector was rejected. This is not a trade decision.")}>
      {disagreement}
    </span>
  );
}
    ...
    <div>
      <div className="grid cols-4">
        ...
        <MetricCard
          ...
          tone="dim"
          sub={snapshot?.model.latency_breakdown ? t("ml.kpi.latency_sub", "fwd {a} · feat {b} · e2e {c}", { a: snapshot.model.latency_breakdown.model_forward_ms ?? "—", b: snapshot.model.latency_breakdown.feature_ms ?? "—", c: snapshot.model.latency_breakdown.e2e_ms ?? "—" }) : t("ml.kpi.latency_fallback", "backend-measured only")}
        />
      </div>
      ...
      <div className="grid cols-2" style={{ marginTop: 14 }}>
        ...
        <Panel title={t("ml.panel.identity", "Artifact identity (manifest)")} right={<span className="timestamp-note">/api/v1/model/identity</span>}>
          {identityQuery.isPending ? (
            ...
            <EmptyState message={identityQuery.data.reason ?? "No model bundle loaded."} hint={t("ml.identity.empty_hint", "Identity is absent, not invalid — the backend said so.")} />
          ) : identityQuery.data ? (
            <dl className="kv">
              <dt>model_id</dt>
