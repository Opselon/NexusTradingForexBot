...
## CHG-0059 - Backtest/replay deep forensic + determinism/causality/friction hardening (2026-09-05, Agent 15)

- AGENT: Agent 15 (Nexus-Main orchestrated) | ROLE: Research / Backtesting / Replay / MT5-data Forensics
- TASK: user brief 2026-09-05 - analyze/trace/reproduce/fix/validate the complete backtest + historical-replay system; verify MT5 Python API semantics (copy_ticks_range, copy_rates_range, symbol_info, order_calc_profit/margin), UTC handling, boundary inclusivity, determinism, lookahead/causality, friction realism (spread/slippage/latency/fees), direction-aware bid/ask fills, tick SL/TP first-touch, order_send hard safety boundary, replay parity, funnel reconciliation, honest metrics, provenance.
...
## CHG-0061 - Dataset subsystem deep forensic + acquisition/immutability/provenance hardening (2026-09-05, Agent 14)

- AGENT: Agent 14 (Nexus-Main orchestrated) | ROLE: Dataset Engine / Tick Acquisition / Provenance / Immutability Forensics
- TASK: user brief 2026-09-05 - deep forensic + development + root-cause fix of the COMPLETE dataset chain: raw MT5 tick/bar data -> acquisition (copy_ticks_range COPY_TICKS_ALL probed surface) -> validation -> dedup -> normalization -> dataset creation -> fingerprint -> immutable storage -> dataset id -> manifest -> experiment/training. Prove "what exact bytes produced this result" is answerable for every downstream research result.
- SCOPE (expected): src/nexus_scalp/research/mt5_tick_dataset.py (acquisition boundary), src/nexus_scalp/research/dataset.py (research dataset builder), src/nexus_scalp/model_generation/dataset_factory.py + artifact_store.py (manifest/persistence), src/nexus_scalp/adapters/mt5/mt5_adapter.py (get_tick_history/get_rate_history verification only), tests/unit/test_agent14_* (new regression suites), agents registries, docs/agent_handoffs/
- OBJECTIVES: (1) BUG-188 post-fix acquisition semantics proven with regression reproduction (FAIL BEFORE / PASS AFTER); (2) tick-level dedup semantics (exact dups, boundary timestamps, same-timestamp distinct identities never merged); (3) fingerprint = content-addressed deterministic (mutate-one-byte changes identity, restore restores identity); (4) fingerprinted-dataset immutability (overwrite/append/silently-repair/reorder paths all blocked); (5) manifest describes the actual stored artifact; (6) provenance v2 fields honest (NOT_RECORDED, never invented) and immutable after fingerprinting; (7) cache-hit idempotence (repeat/overlap/subset/interrupted/corrupt-cache); (8) partial/failed acquisition never masquerades as complete (fault injection); (9) executable quality gates (malformed/missing-ts/invalid-OHLC/dups/out-of-order/impossible-values rejected loudly); (10) temporal boundaries exact (inclusive/exclusive, no edge tick lost/duplicated); (11) offline-after-acquisition guarantee (poisoned MetaTrader5 module); (12) serialization determinism; (13) corruption detection (truncated/altered/missing/swapped -> DETECT/REJECT); (14) path safety; (15) concurrency; (16) real bounded historical execution + idempotent re-run.
