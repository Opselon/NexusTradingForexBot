...
## BUG-231 - StrategyFactory.evaluate() NO_GO path reported candidates[0] (dict-iteration order), not the most informative rejection (2026-09-03, Hermes subagent fix-or-clear; FIXED here)
- SEVERITY: P3 (observability/sample-metadata quality only; zero GO-selection or trading behavior change)
- SURFACE: src/nexus_scalp/model_generation/strategy_factory.py (StrategyFactory.evaluate, all-NO_GO return line)
- ROOT CAUSE (code-trace): candidates are collected in self.strategies dict order filtered by setup-type compatibility. On the all-NO_GO path the legacy code returned candidates[0] - whichever compatible strategy happens to be FIRST in HUNTER_STRATEGIES insertion order. For LIQUIDITY_SWEEP (compatible: hunter_sweep_v1, hunter_london_v1, hunter_smc_v1) the reported reasons could be a thin 2-reason rejection (SPREAD_TOO_WIDE+REGIME_NOT_OK from hunter_sweep_v1) while the later-registered hunter_london_v1 carried the richer 3-reason blocker (same 2 + SESSION_GATE). Tie order was equally dict-order-dependent: reversed registry flipped the reported strategy_id.
- FIX (NO_GO reporting only): return min(candidates, key=lambda c: (-len(c.reasons), c.strategy_id)) - the NO_GO with the MOST reasons (richest explanation), tie-broken by lowest strategy_id for determinism. Docstring documents the NO_GO selection contract. GO selection (highest RR) untouched; single-candidate and NO_COMPATIBLE_STRATEGY paths byte-identical.
- REGRESSION TESTS: tests/unit/test_strategy_factory_no_go_richest_reasons.py (7 tests: richest-wins-over-first, true-3-way tie-break to lexicographically-lowest, dict-order independence via reversed registry, thin-vs-rich shape, GO path unchanged, NO_COMPATIBLE path unchanged). RED verified against unpatched main checkout (4 failed as predicted); GREEN with the patch (18/18 across the 4 hunter/factory suites).
- EVIDENCE: ruff check + ruff format --check clean (2 files); mypy strategy_factory.py clean; py_compile OK; 18 passed RC=0.
- NOT-CHANGED: GO selection, thresholds, registry entries, _evaluate_one gates, sample_maker integration (reasons are metadata-only via analyze_row/attach_hunter_metadata).
