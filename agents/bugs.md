...
## BUG-222 - innerHTML sinks interpolated API-sourced reason/status/error strings without esc() (2026-09-03, Nexus-Fleet-Orchestrator UIX program; FIXED here)
...
- FIX (9 byte-safe edits, CRLF-preserving): route each through esc() (HTML-entity escape incl. quotes). No layout/behavior change; strings render identically for benign input.
- DELIBERATE NON-CHANGES (residual risk, disclosed): remaining unescaped sinks interpolate numeric values or internal-only enums (app.js:434 feature names from local feature engine — low risk, owner Nexus-UIX-Contract to sweep in Wave 3; app.js:6318 telegram counters are numbers). UX-stale badge in Web/ux.js:227 already escapes correctly.
...
## BUG-223 - AuditRepository implicit default writes unit-test orders into the PRODUCTION audit ledger under pytest (2026-09-03, Hermes-Integration, TASK-TDF phase-2 discovery; FIXED here)
...
- FIX (seam seal, both halves):
  1. AuditRepository honors NEXUS_AUDIT_DB for the IMPLICIT default ONLY (`db_url == _DEFAULT_AUDIT_DB_URL` branch); explicit db_url/config callers can never be hijacked by the env var. BUG-149 workspace anchoring preserved when no override is set (packaged/CLI unchanged); BUG-156 memory-URI guard untouched.
  2. tests/conftest.py autouse fixture `_isolate_implicit_audit_db` sets NEXUS_AUDIT_DB to a per-run temp file, mirroring the existing NEXUS_SETTINGS_DB isolation pattern.
...
## BUG-225 - LIVE champion artifact is a byte-identical fresh random init: permanent NO_TRADE via mathematically unreachable confidence gate (2026-09-03, Nexus-Fleet-Orchestrator AI-decision autopilot; detection landed, runtime repair pending)
...
- SYMPTOM (live, port 8080 PAPER): NO_TRADE 98%+ with INSUFFICIENT_CONFIDENCE reasons frozen at "Model Confidence (~0.33) < Effective Threshold (0.40-0.60)"; model probabilities near-uniform (buy~0.25/sell~0.25/no_trade~0.26, implied WAIT ~0.22); 0 of 1,452 post-repair audit_signals reach even the 0.40 base gate.
- REPRODUCTION (executed probes, scratch/ns_bug225_* lineage): (1) 200 real-M1-window 70D vectors through the live champion: argmax NO_TRADE 200/200, normalized directional conf mean 0.3241 max 0.3347, 0/200 pass 0.40; the 50d_main bundle on the SAME vectors: argmax BUY 200/200, mean 0.3898, max 0.4078 (44 pass) — the live bundle alone is degenerate. (2) torch.load the champion and compare to ScalpNet(num_features=70, num_classes=4) minted under torch.manual_seed(42): BYTE-IDENTICAL across all 31 tensors. Same verdict for artifacts v1.0.0/model.pt and EURUSD/v1.0.0/model.pt; 50d_main and 70d_news diverge (trained).
- ROOT CAUSE (chain): the engine pins the process-global torch RNG in WalkForwardTrainer.__init__ (_set_seed(42), verified: trainer-init then ScalpNet mint == the live artifact EXACTLY). Every fresh-weights mint path (cold-start bootstrap when the artifact file was missing, force_fresh, _reinitialize_collapsed_model) therefore produces THE SAME byte-identical weights. Once minted, the WalkForwardTrainer quality gate correctly REJECTS every online fine-tune (17+ rejected 2026-09-02, e.g. "SELL class recall collapsed to 0.0%") and rolls back to baseline — but the baseline IS the fresh init, and LiveEngine persists the returned model unconditionally ("ASYNC RETRAIN SUCCESS" + atomic save after accepted=False), so the untrained state is self-perpetuating. Structural gates all PASS on it (BUG-141 width guard: 70==70; class-head probe: 4 classes; BUG-166: registry fingerprint 763a25f61fe6b7d3 == disk hash; CHAMPION VERIFIED at every boot since 2026-09-02T00:39) because the corruption is SEMANTIC, not structural.
...
- DETECTION (landed this fix): integrity.detect_untrained_fresh_init(path, dim) — byte-compares the checkpoint to the canonical seed-42 fresh init (exact, causal, cheap); forensic check CHECK-MDL-02 check_model_semantic_health() wired into the deploy-gate Model group between CHECK-MDL-01 and CHECK-MDL-03; CRITICAL + detail=UNTRAINED_CHAMPION_ARTIFACT when byte-equal, UNKNOWN when no artifact/torch unavailable, PASS with canary detail otherwise.
...
- RUNTIME REPAIR (handoff, model owner + user consent): the champion must be REPLACED with a trained artifact — canonical path is the research pipeline (three_model.train_variant("70d_liquidity", smoke=True) family, BUG-141 recovery recipe) or a governed promotion of a validated candidate; a bare engine restart does NOT repair it (the fresh weights are reloaded and the quality gate will keep rejecting fine-tunes against a degenerate baseline). Running engine (PID from 2026-09-02 19:49) keeps the artifact in memory until restart — do NOT restart without the repair artifact ready (user owns engine lifecycle).
- RESIDUAL RISKS: (a) v1.0.0/model.pt (50D) + EURUSD/v1.0.0/model.pt are also fresh inits — same canary now flags them; (b) WalkForwardTrainer rejection reasons during the contaminated window (BUY/SELL recall collapse) are partly self-fulfilling: the labels in the rolling buffer came from paper fills, so repair should retrain from a CLEAN research dataset, not the live buffer; (c) the seeded-RNG determinism that makes this canary exact is an implementation property — if seed handling changes, detect_untrained_fresh_init must be revisited (docstring notes this).
- EVIDENCE ARTIFACTS: audit_signals post-2026-09-02 rows (1,452 with probs, 0 pass 0.40); provenance registry rows (champion fingerprint 763a25f61fe6b7d3 registered 2026-09-02T00:42 CHAMPION / 22:34 CHAMPION-50d + 23:21 CANDIDATE-70d); logs/info/2026/09/2026-09-02.log (accepted=False x17, accepted=True x1 at 04:02 -> 8a5fc653858abf17, later reverts to 763a25f6 on 08:13 retrain — the engine was serving the fresh init in memory across the 04:02 accepted write, re-persisting it at every subsequent retrain).
- COMMITS: this fix commit (Nexus-Fleet-Orchestrator: BUG-225 ...). Ledger row: this row.

## BUG-227 - Strategy-layer mutation gap: 20+ decision constants have ZERO regression pins; temporal/stability suite absent from critical_suite.txt (2026-09-03, Nexus-Strategy-Swarm mutation census)
...
- CENSUS (grep-verified tests/ refs per constant, 0 = any mutation silently survives):
  ...
  - regime_classifier.py: ofi_trend_threshold=0, switch_prob_margin=0, spread_chop_enter_usd=0 as a NAME (covered only transitively via test_regime_calibration_bug132 value checks), _REGIME_ACTIVITY escalation map=0.
  - setup_detector.py: HUNTER_MIN_QUALITY=0, per-detector quality weights=0.
  - mslie/: SWEEP_WINDOW_BARS=0, MIN_POOL_DISTANCE_ATR=0, CONFIDENCE_FLOOR=0.
  - stability_controller.py: entry_min_margin/hard_reversal_margin/entry_confirm_bars each pinned by exactly 1 test file (test_temporal_liquidity_phase20.py: test_buy_confirm_then_sell_weak_no_flip / test_strong_opposite_confirms / test_candidate_timeout covers HARD_REVERSAL + candidate-age expiry; restart/schema-reset covered) - thin but present.
- PIPELINE GAP: tests/unit/test_temporal_liquidity_phase20.py (stability controller + temporal features) is NOT listed in tests/critical_suite.txt - the decision-stability state machine has zero CI-gate protection.
- VERIFIED-NOT-GAPS: confidence_threshold (29 test files), cooldown_seconds (4), range_confidence_penalty (3) - adequately pinned.
- REMEDIATION PLAN (strategy waves): Wave A = add test_temporal_liquidity_phase20.py to critical_suite.txt + pin HUNTER_MIN_QUALITY boundary; Wave B = policy flip-penalty/memory + throttle windows; Wave C = mslie sweep-window expiry + regime escalation map. Each pin = 1 test asserting the behavioral effect of the constant, not its literal value, to avoid overfitting pins.
...
## BUG-228 - Quality-gate misfires on zero-improvement fine-tunes: unchanged baseline reported as red [QUALITY GATE REJECTION] + phantom atomic revert; raw ANSI print bypasses structured logger (2026-09-03, Nexus-Main)
...
## BUG-226 - PAPER provenance contamination of the canonical accounting metrics: synthetic -75,341.78 trade + 10,000.0 seed-plateau snapshots produced fake -$82k net and -74.7% drawdown on the LIVE dashboard (2026-09-03, Nexus-Main orchestration; root cause PROVEN, fix landed)
...
- EVIDENCE (read-only probes over artifacts/audit.db, 2026-09-03):
  * 15 ledger rows with tickets 1001..100015 (paper-adapter ticket space, MT5 real tickets are >= 1e11), incl. ticket 100002 SELL_LIMIT entry 2000.08 (= the PaperMT5Adapter metal seed price) -> exit 4430.46 (real market), net -$75,341.78, exit MANUAL_CLOSE exit_reason_source=BROKER_DEAL_REASON, account_balance_after 27851.23 (REAL post-close balance recorded at close time).
  * 647 account snapshots with balance==equity==margin_free==10000.0 exactly (paper seed), spanning 2026-09-02T16:19 to 2026-09-03T07:41; equity curve min 10000 vs real peak 39601 -> fake -74.7% drawdown.
  * REAL-only ledger (139 dedup orders): net -$2,766.25, win-rate 38.8% decided, avg win $21.35 / avg loss -$57.46 - bad but NOT -$82k; the user-visible catastrophe was 92% paper contamination.
  * Runtime mode is LIVE (api/v1/runtime/mode effective_mode=LIVE, commit e59b9cb5), so the paper rows came from a PAPER session sharing artifacts/audit.db (paper boot path engine_boot.py BUG-148 writes to the canonical workspace tree; hot-swap set_execution_mode likewise).
...
- FIX (tag at write, exclude at read; raw evidence NEVER rewritten - contract s47):
  1. audit_repository: new idempotent columns audit_ledger.account_source / audit_account_snapshots.account_source; log_ledger_opened/log_ledger_closed/log_account_snapshot accept and persist account_source; AuditRepository.current_account_source (default LIVE) is read at write time.
  2. order_manager: _ledger_account_source() probes the bound adapter (PaperMT5Adapter -> PAPER else LIVE); applied on opened + both closed call sites.
  3. live_engine: boot seeds audit.current_account_source from the EFFECTIVE mode (_boot_account_source); set_execution_mode hot-swap re-tags on every adapter swap (PAPER/LIVE both directions); PaperMT5Adapter carries current_account_source='PAPER'; IMT5Port documents the metadata attribute.
  4. accounting/core.load_trades: excludes (a) account_source='PAPER' rows, (b) legacy untagged rows in the paper ticket space [1e5,1e11), (c) physically-impossible seed-price hybrids (entry 1999-2001 AND exit > 4000). load_snapshots: excludes exact 10000/10000/10000 seed plateau + PAPER-tagged rows.
...
- VALIDATION: filter applied to a COPY of production audit.db -> trades 343/$-82,057.73 => 329/$-6,668.26; snapshots min equity 10,000 => 26,223.90; focused accounting+order-manager+integration suites green; ruff+mypy clean.
- NOT-CHANGED: ledger rows themselves, experience/outcome rows, risk engine sizing math, execution paths. Exposure/Equity 1991% panel is a separate UI/risk-plan display concern (risk per trade $232 vs equity $11.6k denominator) - tracked for follow-up, not silently "fixed" here.
- RISKS: rows written before deployment carry account_source='' and are classified by ticket-space + price-signature heuristics; future real broker migrations to instruments trading near 2000 with exits >4000 would trip heuristic (c) - acceptable for XAUUSD and removable once all rows are provenance-tagged.

## BUG-229 - Default NO_TRADE reason codes misattributed a routine no-candidate tick to the market regime: REGIME_*/RANGE_BOUND_SIDEWAYS/NEUTRAL_MARKET were the 'no rule fired' default labels, not regime rejections (2026-09-03, Hermes-Coder TASK-ID NX-TDFQ3-RELBL, TASK-TDF Q3 census; FIXED here)
...
## BUG-231 - Pending-order dispatch is NOT cause-aware: wrong 10016 retcode labels + blind 3x resend of a structurally-invalid SELL_LIMIT request (stale PAPER signal geometry survived PAPER→LIVE hot-swap) (2026-09-03, Nexus-Recovery / Nexus-Main orchestrator)
...
- SYMPTOM (production log 2026-09-03 11:17:13 + 11:35:09-11:35:11 +03:30): SELL_LIMIT XAUUSD volume=0.97/0.96 price=2000.08 rejected 3x by the REAL broker with retcode 10016 at 25ms intervals (identical request), then "Pending order dispatch rejected by broker server", TERMINAL_OUTCOME state=REJECTED_UNFILLED ticket=0. User-verified live market at the same moment: Bid 4442.65 / Ask 4443.14 — the SELL_LIMIT price 2000.08 was on the WRONG SIDE of the market by ~2442 USD, i.e. the request was structurally invalid and could never fill, yet it was resent three times and the log mislabeled the rejection as "TRADE_DISABLED / FREEZE_LEVEL".
- ROOT CAUSE (two layers + one originating upstream cause):
  (1) WRONG RETCODE LABELS: _translate_retcode (mt5_adapter.py) mapped 10016 to "TRADE_DISABLED / FREEZE_LEVEL - Order modification frozen by broker"; diagnostics.py RETCODE_LABELS mapped 10016→TRADE_DISABLED_OR_FREEZE_LEVEL, 10017→MARKET_CLOSED, 10018→NO_MONEY, 10019→PRICE_CHANGED, 10020→PRICE_OFF, 10021→NO_CHANGES (offset-by-one drift from 10017 onward). Official MQL5 trade-server return codes: 10013 TRADE_RETCODE_INVALID, 10014 INVALID_VOLUME, 10015 INVALID_PRICE, **10016 INVALID_STOPS ("Invalid stops in the request")**, 10017 TRADE_DISABLED, 10018 MARKET_CLOSED, 10019 NO_MONEY, 10020 PRICE_CHANGED, 10021 PRICE_OFF, 10024 TOO_MANY_REQUESTS, 10029 FROZEN, 10030 UNSUPPORTED_FILLING, 10031 NO_CONNECTION. The mislabel caused the operator (and logs) to chase "frozen order" hypotheses while the real defect was invalid stops geometry.
  (2) BLIND RETRY: place_pending_order's fast-act loop retried EVERY retcode 3x at 25ms, re-deriving only the ENTRY price from the live tick between attempts but NEVER re-validating or re-deriving SL/TP. For the observed request the SL (2001.68) and TP (1983.32) stayed ~2000-relative while the market was 4442 — after entry repair the stops were on the wrong side / absurd distances, guaranteeing repeated 10016 (INVALID_STOPS). Retrying a structurally-invalid request is not recovery; it is broker spam + log noise + wasted latency.
  (3) ORIGINATING UPSTREAM CAUSE (evidence, runtime fix HANDOFF to live_engine owner — out of scope here): audit.db audit_signals rows 2026-09-03T07:41–08:05 UTC show proposed_entry=2000.08 / sl=2001.68 / tp=1983.32 with the PAPER adapter's FIXED spread signature spread_usd=0.20 (paper_adapter.py hardcodes spread=0.20 for 2-digit metals and a 2000.00 metals baseline; real-market rows in the same minutes carry spread 0.08–0.89 at ~4425–4442). At 11:11:29+03:30 the operator hot-switched PAPER→LIVE via WEB_UI ("[MODE] HOT_SWAP_REQUESTED source=WEB_UI old=PAPER new=LIVE"). set_execution_mode swaps the adapter (live_engine.py) but does NOT re-seed the aggregator/feature pipeline nor re-validate the in-flight proposal, so the stale PAPER-geometry signal was dispatched into the real broker. The adapter-level recovery in this fix converts that dispatch failure into a safe, immediate ABORT; the engine-level guarantee (kill/replace stale proposals on mode swap) remains OPEN for the live_engine owner.
- FIX (this commit, adapter-scoped):
  * PENDING_RETCODE_MAP with the correct official labels (asserts pin 10016→INVALID_STOPS, 10017→TRADE_DISABLED, 10018→MARKET_CLOSED); _translate_retcode and diagnostics.RETCODE_LABELS both corrected (SEMANTIC CHANGE of label values only; signatures unchanged).
  * _classify_pending_retcode: TRANSIENT {10004,10012,10020,10021,10031} / REPAIRABLE {10013,10015,10016} / HARD_REJECT (everything else, incl. unknown codes — unknown retcodes are never retried).
  * _validate_pending_request: pre-dispatch structural validation against live symbol_info+tick — tick-size alignment, correct side (SELL_LIMIT/SELL_STOP above bid, BUY_LIMIT/BUY_STOP below ask), stops-level minimum distance (spread floor when stops_level=0), SL/TP side+distance geometry, freeze-level distance, volume min/max/step.
  * _repair_pending_request: ONE validated repair — aligns entry to tick size, pushes to the legal side with min gap, re-derives SL/TP ONLY while preserving the ORIGINAL absolute risk and reward distances and their order; refuses (→ abort) when a valid geometry cannot preserve the trade's risk/reward meaning.
  * Rewritten bounded retry loop (max 3 attempts, 0.5s total budget): validate before every send; TRANSIENT → idempotency guard (_find_equivalent_pending preserved) + backoff 25ms/50ms + 10031 reconnect probe preserved; REPAIRABLE → at most ONE repair, and a repaired request identical to the failed one aborts instead of resending; HARD_REJECT or repair-impossible → structured HARD abort, return 0 WITHOUT any further broker send.
  * _log_recovery structured observability: symbol, order_type, retcode + official description, recovery_state (VALIDATION_FAILED|TRANSIENT_RETRY|REPAIRED|REPAIR_REJECTED|BROKER_HARD_REJECT|BROKER_TRANSIENT), recovery_reason, attempt, elapsed_ms, original entry/sl/tp, live bid/ask. No secrets.
...
- EVIDENCE ARTIFACTS: audit.db audit_signals ids 945046/945084/945220/945939/948020/951641 (2000.08 geometry, spread_usd=0.20); logs/info/2026/09/2026-09-03.* "[MODE] HOT_SWAP_REQUESTED source=WEB_UI old=PAPER new=LIVE" at 11:11:29+03:30; user-captured live quotes Bid 4442.65/Ask 4443.14; MQL5 official trade return codes table (mql5.com/en/docs/constants/errorswarnings/enum_trade_return_codes).
- RESIDUAL RISKS: (a) live MT5 smoke NOT executed (no live order placement during verification, per safety contract) — behavior validated offline via fake-mt5 harness + static analysis; (b) engine-level hot-swap proposal revalidation is NOT implemented here (HANDOFF: live_engine owner must drop/revalidate in-flight proposals whose tick provenance predates a PAPER→LIVE/SHADOW swap, and stamp proposals with tick source/spread signature); (c) PREDICTIVE_LIMIT producer (audit_signals 945041, entry 2000.02) also emitted 2000-relative geometry — separate explainability path already tracked in test_predictive_limit_explainability_bug226.py (different BUG-226, unrelated ID collision noted).
- COMMITS: this fix commit (Nexus-Recovery: cause-aware pending-order recovery + retcode-map fix (BUG-231)). Ledger row: this row.

## BUG-210 RESOLUTION (2026-09-03, Hermes-DevOps TASK-NX-BUG210-FLAKY release-owner referral): test_release_hardening::test_old_bug_fails_before_runtime_identity_rejected hardened against xdist worker CWD/package-resolution drift; production semantics untouched

### Reproduction attempt (2026-09-03, .venv python -m pytest, repo venv)
- Solo target test: 3x PASS (directive minimum) + 5x more PASS = 8/8 green.
- Full critical-suite manifest (79 files from tests/critical_suite.txt, comments filtered): 3 full iterations RC=0 - xdist worksteal x2 (~9.2min, ~8.1min) + loadgroup x1 (~3.4min). Original flake NOT reproduced (machine-state dependent; consistent with the once-in-N worker-scheduling trigger). Evidence logs: %LOCALAPPDATA%/Temp/bug210_run{A,B,C}*.log (pre-fix), bug210_runD_postfix_worksteal.log (post-fix).

### Root-cause analysis (vector proven by construction; flake vector class)
- The test's premise 'no build-info.json anywhere reachable' depends on WORKER CWD hygiene. metadata.get_build_info_file() frozen candidates end with Path.cwd()/build-info.json AND a package-relative repo-root escape hatch (__file__ parent-walk); the dev machine carries an UNTRACKED gitignored repo-root build-info.json (9.0.3 stamp, commit 53317de) satisfying both.
...
- Part-11 test: metadata.__file__ pointed at a shadow copy inside tmp_path -> package-relative walk deterministically returns tmp_path (no pyproject.toml there), cutting the repo-root escape hatch for BOTH leak directions; premise assert get_build_info_file() is None pinned BEFORE the identity verdict (stronger precondition, allowed fix class). Production candidate list/order and release-pipeline/runtime-identity semantics UNCHANGED.
- Verification: py_compile + ruff check + ruff format + mypy clean; solo 3x green post-fix; full release-hardening file 15 passed + 1 env-skip; post-fix full-manifest xdist worksteal iteration RC=0 (bug210_runD_postfix_worksteal.log). Red-before semantics preserved: the test still guards the BUG-174 unstamped-frozen contract (Commit None / runtime-timestamp detection).

Status: FIXED (test-layer hardening; flake vector closed by construction; production code untouched). Dev-machine note: the stale repo-root build-info.json remains on this machine (gitignored; deleting it is the machine-owner's call - the hardened test no longer cares).

## BUG-232 - Launcher/engine mode desynchronization: PAPER adapter under a LIVE badge (stale 2000 seed price in every panel; paper-geometry proposals reached the real broker — BUG-231's upstream cause) + LIVE choice did not survive restarts (2026-09-03, Nexus-Main; root cause PROVEN, fixes runtime-verified end-to-end)
...
- SYMPTOM CHAIN (all reproduced): (1) boots landed in a hybrid state — launcher log "configured_mode=PAPER" then engine "[MODE] runtime_mode=LIVE" with the PAPER simulator bound (LIVE CONNECTED balance=10000.0 login=9990001); (2) PAPER market showed ~2000 with ±0.02 dead-walk ticks while real XAUUSD traded 4480-4500; (3) switching to LIVE left every price panel on the stale 2000 seed; (4) console error storm once real prices replaced paper state ('MT5 Adapter is not connected' hot-path spam 18:50-18:51, 11.5k-line error log with 16 paper-era tracebacks); (5) the user's saved LIVE choice did not take effect on the next boot without re-asking.
- ROOT CAUSE (PROVEN, four independent defects):
  (a) LAUNCHER MODE FLOW: NexusTradingForexBot.py loaded only configs/live.yaml (no execution.mode key -> base.yaml PAPER safe default) and bound the adapter BEFORE the engine applied the settings-DB precedence (BUG-148). Engine then re-bound execution.mode=LIVE (persisted, version 75, source WEB_ENGINE_MODE) but align_adapter_to_boot_mode only corrected PAPER<-real boots — a LIVE boot that found a paper adapter returned it UNCHANGED. Result: PAPER adapter + LIVE badge.
  (b) PAPER SEED: PaperMT5Adapter hard-coded _current_price=2000.00 for metals — a dead, unrealistic market ~2400 USD away from the real instrument; every paper-geometry proposal was invalid against any real reference (BUG-231's 10016 INVALID_STOPS producer).
  (c) HOT-SWAP STATE LEAK: set_execution_mode swapped ONLY the adapter; signal-policy price/direction caches, engine staged ticks, and the cached account snapshot survived — stale paper state flowed into the new mode (BUG-231 handoff left OPEN).
  (d) PERSISTENCE UX: the launcher asked "Confirm LIVE trading? Type YES" on EVERY boot even though the UI save already persisted the operator's confirmed choice.
- FIX:
  * (a) launcher resolves the EFFECTIVE mode BEFORE adapter binding (explicit --mode > settings DB execution.mode > YAML), binds the matching adapter, and passes mode_override=resolved to LiveEngine so launcher and engine cannot diverge again.
  * (b) align_adapter_to_boot_mode gained the mirror guard: LIVE boot + paper adapter -> replace with DirectMT5Adapter (win32) / RemoteMT5GatewayAdapter fallback, BEFORE the first tick.
  * (c) PaperMT5Adapter._seed_price(): per-instrument baselines (XAUUSD 4400, XAGUSD 28, EURUSD 1.085, GBPUSD 1.27, USDJPY 150) with NEXUS_PAPER_SEED_<SYM> env override; get_last_tick walk now has instrument-scaled volatility (gold 5-45c bursts, 25-45c random spread) and mean-reverts to the seed (2% band). set_execution_mode paper branch now passes the ACTIVE symbol (was silently EURUSD conventions on XAUUSD engines).
  * (d) _invalidate_cross_mode_state() on every cross-boundary hot-swap: bumps _mode_session_generation (init 0 in __init__), clears signal-policy last_order_price/time/_last_active_direction/_last_executed_price, drops staged ticks/proposals, resets the tick-stagnation clock, and drops the cached _account_snapshot (which was serving the paper account under LIVE). PAPER<->SHADOW swaps share the simulation boundary and do not invalidate.
  * (e) launcher LIVE confirmation prompt now fires ONLY for a fresh CLI --mode live override; a persisted settings-DB LIVE is treated as the operator's prior confirmation (persistence without re-asking). UI-typed confirmation (CHG-0043/BUG-194) unchanged.
  * (f) /api/status now carries data_source (PAPER_SIMULATION|MT5_LIVE|MT5_DISCONNECTED), adapter_class, mode_source_mismatch; the runtime-mode badge renders LIVE·MT5_LIVE only as green, a LIVE/PAPER_SIMULATION combination as a hard red MISMATCH warning, and the header price panels carry a SRC: watermark that turns red+animated on mismatch. UI cannot display a paper price as live without an explicit visible flag.
...
- RUNTIME VERIFICATION (real app, isolated env DBs, port 8099):
  * PAPER boot: mode=PAPER, data_source=PAPER_SIMULATION, PaperMT5Adapter, bid 4400.00->4401.18 moving over 5 probes, spread positive, account PAPER_SIMULATION/10000.
  * POST /api/engine/mode LIVE: runtime LIVE, DirectMT5Adapter, data_source=MT5_LIVE, REAL demo ticks 4492.22/4492.48 moving (4492.28/4492.09/4491.90), account BROKER_NATIVE login=10011755849 balance=30896.70.
  * Reverse swap: PAPER became authoritative again (PaperMT5Adapter/4400.45/PAPER_SIMULATION/10000.0), then LIVE again (4485.64/MT5_LIVE/10011755849/30896.70). Zero cross-mode contamination across all four transitions; no error storm during either state.
...
- FIX (DECISION, MLFix §3 option-b variant — CALLER-DEPTH alignment on the shared contract): added `HTF_HISTORY_BARS = 4000` as the SINGLE SOURCE OF TRUTH in `features/scalp_features.py` (the feature layer owns the semantic, no feature-name/order change, schema hash `235b8fccc96b7e0e` PRESERVED). Rationale vs alternatives: (a) window-normalizing HTF inside `compute_from_bars` (option c) would silently change values computed with <4000 bars of history and would re-write the engine contract mid-incident with a retrain already queued — rejected; (b) passing a live-equivalent depth to the train builder (option b, chosen) is the smallest correct layer: the live aggregator cap is ALREADY 4000 (`live_engine.py:3618`, post-BUG-058 resync seeds 3500), so the only divergent caller was the TRAIN builder (and the replay path `replay_70d_vector` which sliced `base_window = bars[-min_bars:]` = 55). Live-side wiring required: NONE — `live_engine.py` cap 4000 == `HTF_HISTORY_BARS` by construction (documented in code comment; the constant is the contract the engine cap already implements).
- CHANGED FILES: `features/scalp_features.py` (+`HTF_HISTORY_BARS=4000` shared contract), `model_generation/schema_v2.py` (compute_70d_frame + compute_60d_frame pass the bounded window; `LIQUIDITY_HISTORY_LIMIT` aliased to the shared constant), `model_generation/schema_v2_incremental.py` (compute_70d_frame_fast same window — fast/canonical byte-identity re-proven), `model_generation/replay.py` (replay_70d_vector now slices `bars[-HTF_HISTORY_BARS:]` instead of 55 — restores dataset==replay parity), `tests/unit/test_htf_train_live_parity.py` (NEW invariant suite), `scratch/mlpower_parity_corpus_probe.out.txt` (post-fix MATCH evidence).
- BEFORE/AFTER EVIDENCE (repo venv): `scratch/mlpower_parity_htf_window_diag.py` depth grid 55→(0.000,0.0) 120→(3.000,0.0) 240→(3.000,1.0) 4000→(3.000,1.0) (unchanged — engine math untouched); `scratch/mlpower_parity_htf_live_train_callgrid.py` pinned the divergent call sites; `scratch/mlpower_parity_corpus_probe.py` BEFORE: `PARITY VERDICT: MISMATCH` (feat41 train 0.0 / live 3.0; feat42 train 0.0 / live 1.0; hash 235b8fccc96b7e0e both sides) — AFTER: `PARITY VERDICT: MATCH (train==live, all 70 dims)`, max_delta 0.0, hash `235b8fccc96b7e0e` stable.
- TESTS (repo venv, all green post-fix): `tests/unit/test_htf_train_live_parity.py` 5 passed + 1 documented-skip (full-vector train==live invariant, fast==canonical identity, replay==dataset row, contract-single-source pins); `test_70d_replay_parity_task3.py` 9 passed (p08 dataset==replay bit-exact restored — was failing at idx42 under the 55-bar replay window); `test_liquidity_engine_causality.py` 17 passed; `test_70d_bug106_incremental_phase19.py` 2 passed; extended parity family (70d_parity/contract/dataset_parity + temporal_phase20) 79 passed.
...
## BUG-236 - Strategy Marketplace, Isolated Research DB & Adaptive Runtime (NX-MKT EPIC, CHG-0057)
...
- METRICS (all deterministic, read-only): logit std (mean per-class), probability diversity (mean max-prob on 64 seeded randoms + zeros/±3), directional sensitivity (±3 sweep BUY-SELL margin swing), single-feature/group sensitivity (recorded in calibration probe), WAIT mass (4-head), parameter movement frac.
- THRESHOLDS (empirically calibrated, no magic numbers; probe `scratch/ns_mlt5_behavioral_calibration.py` + evidence `scratch/ns_mlt5_behavioral_calibration_out.json`): logit_std_min 0.15, max_prob_floor 0.35, wait_mass_ceiling 0.30, sensitivity_floor 0.02, movement_frac_min 0.10. Calibration table (fresh/epsilon/degraded/50d_main/70d_news/champion) recorded in behavioral_health.py docstring.
...
- WIRING: CHECK-MDL-04 check_model_behavioral_health() in forensics/checks_features.py + engine Model group (CHG-0057 carrier); promotion-lane handoff: three_model.py / walk_forward_trainer / LiveEngine persist path must call assert_model_behaviorally_healthy() before CHALLENGER registration / atomic save (open, owned by promotion lane).
...
## BUG-233-RG (research-gate short-circuit; ID COLLISION NOTE: the ledger already carries a separate '## BUG-233' verify-release row from the Nexus-Main test-reduction pass — parallel-session ID reuse; this row documents the NX-MKT A2-coder BUG-233 claim per FORENSIC_REPORT NX-MKT-AUDIT-20260903 4) (2026-09-03/04, Nexus-Coder A2 backend)
...
- ROOT CAUSE (forensic-proven, NX-MKT-AUDIT): validate_candidate ran ALL gates unconditionally — 3766 OOS-failed runs in audit.db also executed (and even PASSED) the robustness gate. Pure wasted compute + 'robustness PASSED' analytics pollution on dead candidates.
...
## BUG-237 - `nexus update` FAILED "artifact is not a valid zip" on the Inno setup.exe payload (2026-09-04, Nexus-Main)
...
## BUG-238 - shadow promotion_verdict noise-band branch raised NameError: VERDICT_INSUFFICIENT (lazy-import aliasing missed one branch) (2026-09-05, Nexus-Main Agent-9 70D lane)
...
## BUG-246 - dataset builder MAX_GAP_US drifted from the canonical temporal SSoT (2026-09-05, Agent-2 dataset forensics, TASK-AGENT2-DATA-FORENSICS)
...
## BUG-247 - Execution pipeline residual hedge/reversal entry gates (Agent-12 forensic follow-up, CHG-0058)
...
- SYMPTOM (found by the deep forensic sweep, parallel subagents, 2026-09-05): every broker-write call-site census proved single-authority (all outside calls inside OrderManager), but inside the manager two entry legs bypassed the primary-path risk gates:
  (P1) `order_manager.py:5257` `adapter.place_pending_order(...)` inside the AI-direction-flip fast-reversal chain bypassed the `SAFE_MODE` circuit (only a `HARD_MAX_LOTS` clamp ran). The protective CLOSE leg correctly fires in SAFE_MODE (risk-reducing), but the follow-up pending entry is itself an entry and must honor the circuit. Previously `execute_order:686` + `dispatch_order:836` both gated on SAFE_MODE while this direct pending did not.
  (P2) `order_manager.py:703` `adapter.send_order(order)` inside `execute_order` (the hedge entry path called from `live_engine.py:_evaluate_hedging_policy:4806`) bypassed the unconditional `HARD_MAX_LOTS=10.0` clamp (`order_manager.py:77/808`). The normal hedge flow calls `RiskEngine.evaluate_proposal -> get_clamped_position_size` which clamps, but a direct `TradeOrder(volume=100)` would bypass the 10-lot ceiling.
...
- FIX (fail-closed, same ordering as dispatch_order: SAFE_MODE -> exposure -> clamp): (P1) the fast-reversal follow-up now checks `if global_state == "SAFE_MODE": log WARN + return True` before any pending write (close leg already ran; the entry is suppressed). (P2) `execute_order` now runs `clamped_vol = _clamp_dispatch_volume(order.volume)` before the broker call: `clamped_vol <= 0` -> refuse + WARN, otherwise `order = order.model_copy(update={"volume": clamped_vol})` (frozen domain model) on mismatch, then `adapter.send_order(order)`. The normal clamped flow is byte-identical (no behavior change); an oversized direct volume is clamped down to 10.0.
...
## BUG-248 - walk-forward per-fold gate ignored the fold's own OOS window; walk_forward_folds still defaulted purge/embargo to 0.0 (2026-09-05, Agent 5 research-gate forensic; fix commit b1eb5b94 originally titled BUG-244 - ID COLLISION NOTE: Agent 15's bar-mode replay defect consumed BUG-244 first in a parallel commit; this row renumbers the research-gate defect per the ledger collision rule; history not rewritten)
...
- SCOPE NOTE: Agent 17 (TASK-AGENT17-OOS-GATE, CHG-0063) owns the broader OOS/temporal-holdout mission and already landed BUG-245 (empty-context silent widening). The deeper question "should per-fold OOS windows overlap the next fold's validation window" (walk-forward block layout) is left to that owner.


## BUG-249 - Research methodology fail-open trio: negative purge/embargo silently disabled leakage guards + all-non-finite OOS window PASSED the hard gate + research_runs.config recorded DEFAULT purge/embargo instead of effective values (2026-09-05, Agent 13 research-methodology forensic; fix commit 4a46fdd7; NOTE: commit title said BUG-244 - ID COLLISION per ledger rule, Agent 15 consumed BUG-244 first; this row is the authoritative Agent 13 entry)
...
- RESIDUAL (owner: research/context_contract): extract_context_contract collapses regime {name, require} pairs to regime_states=[require], losing name; phase27-style DSL hits CONTEXT_CONTRACT_EMPTY_POPULATION - fail-loud, wrong population resolution documented.

## BUG-249-A6 (execution clamp NaN/inf leak; ID SUFFIX to avoid collision with the two BUG-249-class research rows) - OrderLifecycleManager._clamp_dispatch_volume propagated NaN/inf volume into the broker request (2026-09-05, Agent 6 execution wave-2 QA; fix commit 60b1f42f)
...
- VERIFICATION (offline, no MT5): nan clamp -> 0.0; inf clamp -> 0.0; misbehaving-clamp NaN -> 0.0 post-clamp; ruff check+format clean; mypy clean; py_compile clean; scoped execution suites green (pending_recovery, exit_bugs, execution_quality, BUG-212/232/226 paper boundary).
...
## BUG-250 - Exit-classification false-promoted UNKNOWN to MANUAL_CLOSE on bare deal_reason==0 when profit!=0 (Agent-12 Wave-2, CHG-0058 exit-classification hardening, INV-012)
...
- EVIDENCE (deterministic harness, repo venv): narrow spread 0.20 vs ATR 1.5 (ratio 0.133) passes to FINAL_DECISION; wide spread 0.90 (ratio 0.60) blocks at SPREAD_ATR_GATE with reason SPREAD_ATR_RATIO_EXCEEDED; ruff clean, mypy clean, 3/3 regression tests in tests/unit/test_agent5_bug249_251_252.py::TestBug249SpreadAtrGate.
...
## BUG-251 - AI-reversal close/flip gate consumed RAW probabilities while the entry confidence gate is trained-class normalized (confidence-scale inversion; Agent-5 inference/decision/risk forensics, 2026-09-05, Nexus-Main)
...
- FIX (behavior-preserving on strong signals, semantics-aligned on the scale): `_evaluate_ai_reversal` now takes `no_trade_prob` (passed from the call site sanitized `prob_no_trade`), normalizes the relative bias over the trained mass `max(0,buy)+max(0,sell)+max(0,no_trade)` (degenerate mass falls back to the raw directional share, never manufactures conviction), and stamps the reversal CLOSE_POSITION proposal confidence in the SAME trained-class scale (clamp >= 0, no manufacturing). Thresholds (ai_flip_relative_bias_threshold 0.60, ai_flip_min_delta 0.10) and structural-confirmation requirements UNCHANGED.
- EVIDENCE (deterministic harness, repo venv): sell=0.55/buy=0.10/no_trade=0.10 -> bias 0.733 >= 0.60 fires with confidence 0.733 (was raw 0.55); sell=0.30/buy=0.20/no_trade=0.40 -> trained bias 0.333 < 0.60 and delta 0.10 NOT > 0.10 -> NO false reversal (old raw bias 0.60 >= 0.60 would have fired). ruff clean, mypy clean, py_compile clean.
...
## BUG-253 - 70D inference permanently blocked by stale liquidity snapshot: tick_pipeline duplicate aggregator.process_tick clobbered is_new_bar, killing the new-bar liquidity re-warm (live forensics 2026-09-08 19:15, Hermes)
...
- SYMPTOM (production log 2026-09-08T19:15:25): `[INFERENCE] 70D assembly failed - inference blocked for this tick` twice per tick (once in-trade via manage-active-positions, once in the policy slot), interleaved with REGIME TRANSITION + FRESHNESS_GATE BLOCKED_BY_STALE lines. Liquidity snapshot decision_at 15:39 vs feature timestamp 15:45 (age 6+ min > 4x90s causal window -> STALE), yet FEATURE_CALCULATION_OK at 19:09 and 21:17 proves the compute itself was healthy - the RE-WARM TRIGGER never fired between those runs.
...
- FIX (fail-closed direction preserved - liquidity is NEVER fabricated): (a) removed the duplicate process_tick; caller is_new_bar is authoritative (documented in-place). (b) BUG-169 latency contract preserved: re-warm stays new-bar-cadence; added a THROTTLED (15s) retry ONLY when the retained snapshot is already STALE so a transient compute failure cannot strand the governor permanently. (c) both inference call sites in run_pre_policy_stages gate on `effective_feature_dim==70 -> liquidity_governor.causal_state()=="VALID"` (_liq_ok): degraded states yield probs=None (positions still managed, protective stops never pause) instead of raising; 50D contracts are untouched; genuine model defects still propagate and feed the circuit breaker (fail-loud preserved for real failures).
- EVIDENCE (scratch/bug253_harness.py over the REAL TickPipeline code, repo venv): A: STALE governor -> infer NOT called, throttled re-warm fired once (no per-tick recompute - BUG-169 regression class guarded); B: VALID governor -> inference served normally; C: 50D contract -> inference never liquidity-gated; D: is_new_bar=True now reaches _on_new_bar + re-warm (was clobbered). Test parity: test_htf_warmup_gate/test_bug125/test_fidelity/test_runtime_70d_warmup/test_fault_injection failure set byte-identical to the pre-change baseline (all pre-existing/foreign; zero new reds).
...
## BUG-252 - RiskEngine drawdown penalty was unreachable dead code (AccountInfo has no peak_equity field) (Agent-5 inference/decision/risk forensics, 2026-09-05, Nexus-Main)
...
- EVIDENCE (deterministic harness, repo venv): equity=8000, peak=12000 -> `Drawdown Penalty Active: Scaling trade risk % by 0.20x due to 33.33% drawdown`, volume 0.14 -> 0.02 (0.2x scaled); peak=None -> baseline volume 0.14 unchanged. ruff clean, mypy clean, py_compile clean.
...
## BUG-256 - audit_executions UNIQUE-index bootstrap crashed startup on every pre-release database: bare CREATE UNIQUE INDEX against legacy duplicate rows (systemic multi-environment startup crash, 2026-09-11, Hermes)
...
- ROOT CAUSE (chain): (1) c2662e31 correctly identified (order_id, status) as the redelivery identity for dispatch attempts and made the writer `ON CONFLICT DO NOTHING`; (2) but the schema bootstrap created the enforcing index with NO legacy-data reconciliation — unlike its sibling `idx_orders_execution_idempotency` in the SAME file, which already follows the detect→repair→create pattern; (3) any database from the un-deduplicated plain-INSERT era (every pre-09-10 install) necessarily violates the new constraint; (4) the index DDL runs inside `_create_sqlite_tables` (construction path), so the crash IS the startup — no migration gate ordering, no repair, no fail-loud-without-crash. Secondary findings: no `PRAGMA user_version`/schema marker on the audit DB (the migration-engine schema_meta at version 7 predates and does not cover app-bootstrap DDL), and construction sites (engine + web + CLI) can run concurrently against one file with no repair-level lock.
...
- RECONCILIATION POLICY: duplicates here are redelivery artifacts of the same dispatch attempt (dispatch.py logs exactly one log_execution per order_id; the production dupes are byte-identical payloads with different timestamps), NOT legitimate distinct states — earliest row survives, later exact-identity rows move to the reconciled archive (preserved for forensics, never hard-deleted; TIER_1 never_delete semantics honored via preservation, not retention in the live table).
...
- INVARIANTS: INV-005/006 (duplicate-event idempotency — now actually enforced), INV-007 (historical rows immutable — superseded rows preserved, never rewritten/deleted), INV-001 (no hot-path change: repair is boot-path only, clean-DB cost is two indexed reads), DB-MIGRATION contract (additive, idempotent, fail-loud).
...
## BUG-257 - Champion model.pt HASH_MISMATCH boot refusal (second launcher fatal of 09-11): the 2026-09-07 ungoverned fine-tune drift (bb1f0afe) re-landed on the deployed champion path and P0-2 trust anchor correctly refused to serve (logs/critical 2026-09-11 14:31, Hermes)
...
- RISKS / OPEN: (1) the writer that re-published bb1f0afe at 09-11 04:13 is unidentified — any code path holding a reference to the old drift (or an operator/tool copying from the quarantine dir) can re-land it; monitor mtime on the deployed model.pt; (2) the engine's async-retrain persist path rewrites model.pt IN PLACE and refreshes sidecar digests atomically (BUG-235/236 persist-decision + sidecar refresh) — a crash BETWEEN the weight write and sidecar refresh would self-inflict this failure class; both refresh points log MANIFEST_REFRESH_REFUSED fail-loud.
...
## BUG-259 - Hold-score Time-in-Loss penalty derived its holding-duration denominator from the HOST WALL CLOCK (datetime.now) instead of the tick-threaded timestamp (G3 clock-domain residue, NSE-Swarm role 6, 2026-09-11)
...
## BUG-263 - Rollback restored .previous-* snapshots WITHOUT integrity re-verification: tampered/corrupted snapshot bytes were silently activated over the live app tree (P0 trust-chain bypass; tracked as BUG-262 during the recovery audit, renumbered after the swarm consumed 262 for close-time evidence; 2026-09-12, Hermes A-1)
...
- VERDICT CODES: SNAPSHOT_NO_MANIFEST | SNAPSHOT_INTEGRITY_FAILED (granular packaging reason kept in integrity_reason: MANIFEST_UNREADABLE/MANIFEST_MALFORMED/MANIFEST_TRAVERSAL/FILE_MISSING/HASH_MISMATCH/SUMS_MISMATCH/IDENTITY_MISMATCH).
- REGRESSION: spec battery tests/unit/test_agent9_rollback_integrity.py (5, byte-exact pinned fixture, kept md5-stable via [tool.ruff.format].exclude — assertions untouched) + tests/unit/test_bug263_snapshot_integrity_gate.py (25: both manifest shapes incl. release-root portable/ remap spellings, planted-shadow anti-steering, malformed JSON / empty entry set / non-hex digest / absolute / traversal fail-closed matrix, pure-read gate proof, SHA256SUMS cross-check, identity forgery, byte-identical live tree on refusal, no-partial-restore, orchestrator refusal wiring, manifest_records + escape-detector units). Pre-fix 4/5 spec RED (re-proven via stash probe on merged tip); post-fix 30/30 + update-engine battery green.
- INVARIANTS: version-aware user-data preservation unchanged; update state-machine observability preserved (refusals observable, never mis-reported); Web/, model, trading surfaces untouched; --json CLI purity preserved (stdlib logger). No production manifest format changed — both existing shapes are accepted by the canonical reader.
- OPEN (reported, not guessed): the spec fixture's {"manifest_version","files":{rel:sha}} embedded shape has NO producer anywhere on main (git log -S over all history: the pipeline embeds the release-root `artifacts` manifest verbatim into the portable tree, or portable-rooted `artifacts` locally via scripts/build/update_helpers.py). The gate accepts BOTH shapes so the contradiction is dissolved without weakening either side; a future portable-root `files` producer plugs in without code change.



## BUG-264 - Maintenance-window test pins stepped 180 minutes past an 181-minute inclusive window: dispatch idempotency/safe-mode batteries flake RED once a day at a fixed wall-clock minute (NSE-Swarm role 11 CI/GitOps, 2026-09-12)

- SEVERITY: MEDIUM (CI determinism: the push-gate critical suite fails tests/unit/test_capital_protection_a15.py::TestDispatchIdempotency::test_primary_path_duplicate_request_id_blocked on EVERY platform whose runner clock crosses 16:30:00..16:30:59 UTC; proven red on windows-latest CI run 1085 at 88a5fa15 16:30:01.944Z while the identical main run 34705056299 stayed green (its pytest phase started 16:24, before the band). Trading path unaffected — the MAINTENANCE_WINDOW guard itself is correct.) | STATUS: FIXED
...
- NOTE (pre-existing, fixed in passing): test_agent11 TestBug242ManualActionAuthority::test_web_layer_never_calls_adapter_directly 401s since WEB-AUTH-P0 (proven red at pristine cb90e174) — now carries the documented NSE_WEB_AUTH_DISABLE=1 fixture opt-out (test_rule_matrix::test_api_endpoints precedent).
...
## BUG-265 - Integration Heavy-CI lanes carried the BUG-264 wall-clock pin class + stale p0e contract mirror + unauthenticated WEB-AUTH client (NSE-Swarm role 12 Reviewer/Gatekeeper, 2026-09-13)

- SEVERITY: MEDIUM (CI determinism only; no src/ change. Three pre-existing reds in the Heavy-CI `integration` matrix (ci.yml:646-697), which runs tests/integration/ on every main push but is NOT push-gating critical_suite): (1) tests/integration/test_experience_execution_boundary.py::_proposal used raw datetime.now(UTC) into dispatch_order -> ENTRY_BLOCKED by MAINTENANCE_WINDOW for the whole UTC 19:30..22:30 band (same class as BUG-264, ledger'd as BUG-264-FOLLOWUP-1); (2) the same file's REST test probed create_app() without a token -> WEB-AUTH-P0 401 (also FOLLOWUP-1, proven 22:48Z locally at df275c16); (3) tests/integration/test_end_to_end_closed_loop_bug140.py STAGE-2 pinned eligibility contract_version 'p0e-bug140-1' while the SSOT stamp moved to 'p0e-bug185-1' in 5073b865 (src/nexus_scalp/research/dataset.py:150) — the unit twin (tests/unit/test_lifecycle_bug140.py:574) was updated in that commit, the integration mirror was missed, so the closed-loop E2E was red at STAGE 2 on EVERY HEAD since (ledger'd as BUG-185V140 OPEN-ROUTED; swarm resolved it as a stale-mirror test fix, pinning the SHIPPED value, no assertion relaxation). | STATUS: FIXED
...
## BUG-266 - Paper REPLAY mode shipped but NEVER wired: every PAPER boot/hot-swap/re-alignment silently locked to the synthetic AR(1) world (audit K2) (NSE-Swarm role 1 Architect, 2026-09-13)

- SEVERITY: HIGH (learning-substrate truth; no money path at default config). The adapter-level REPLAY market-data mode landed in a5f38467 (P0 phase 4, tests/unit/test_paper_replay_mode.py) and `ReplayTickSource` loads real recorded ticks/bars, but grep at b179d323 proved ZERO production construction ever passed `replay_source=`: engine_boot.py:455, NexusTradingForexBot.py:641, live_engine.py:4206 (align_adapter_to_boot_mode), runtime_mode.py:102 (hot-swap) all built `PaperMT5Adapter(symbol=...)` directly. Audit line K2 ("wire paper to real recorded data") was therefore UNREACHABLE in production regardless of configuration — every PAPER experience row, every paper-trained candidate and every paper-vs-demo comparison ran on a fictional AR(1) walk around the hardcoded `_SEED_BASELINES` price, while the code advertised two explicit modes. The #1 failure shape (working class with no production caller). | STATUS: FIXED
...
- INVARIANTS: no gateway-adapter/client-contract change; no `.github/workflows/*`; tick pipeline never blocked (the factory runs at construction time only); domain models untouched; ExecutionMode semantics untouched — PAPER/SHADOW/LIVE boundaries behave exactly as before, only the DATA substrate became configurable; default configuration (no paper_data block, or SYNTHETIC) produces the identical adapter the old code built.

## BUG-267 - Both web consoles dead-end UNAUTHORIZED on first navigation; React /alt never bootstraps; doctor probes a dead :8080 / phantom /api/health (Hermes, 2026-09-13; ID renumbered from 266 — the paper-REPLAY lane #173 consumed it during this PR's window, same precedent as BUG-263's renumber)

- SEVERITY: HIGH (operator lockout, LIVE machine). On the running engine (127.0.0.1:8081 after the port auto-incremented away from 8080, which NVIDIA Broadcast holds): (1) the alt console and http://127.0.0.1:8081/ both rendered the raw {"ok":false,"error":{"code":"UNAUTHORIZED"}} JSON in a fresh browser tab — Chrome MCP reproduction on the live server: GET /alt/ tokenless = 401, and the /alt SPA itself (client.ts) only authenticates from ?token= / sessionStorage, which no first visit ever has: the WEB-UI-BOOTSTRAP cookie rides ONLY /app.js + /api_client.js, but the browser's FIRST request is the document, and "/" + "/alt" were gated — an app.js that never loads can never issue the cookie it depends on (circular bootstrap). (2) Even the ?token= path was undocumented-dead: the DPAPI token resolution failure branch MINTS a fresh random token every start (secret-store errors are swallowed), and no surface ever showed the enforced value (OBS-TRACE-2 removed the log line), leaving no documented recovery. (3) `nexus doctor --fix` reported the web plane unreachable forever: check_a2a_gateway probes /api/health, a path on the auth PUBLIC allowlist with NO route registered (404 — verified live), while the real route is /health; and it probes 8080 while the server bound 8081+. (4) scripts/serve_alt_ui.py (#171, port 8088) + the vite dev proxy default to :8080 — refused connections the operator read as another UNAUTHORIZED ghost. | STATUS: FIXED
- FIX: auth.py — "/", "/index.html", "/alt", "/alt/" and the /alt/ static-prefix (trailing slash pinned; /alternative-api stays gated) join PUBLIC_PATHS as static shells carrying NO state and NO credentials (all console data flows through /api/**, which keeps full WEB-AUTH-P0 enforcement — 401 without credential pinned); the HttpOnly bootstrap Set-Cookie now also rides those document paths (COOKIE_BOOTSTRAP_PATHS), so both consoles self-authenticate on first navigation and every later /api/SSE call rides the cookie; headers (?token=/Bearer/X-NSE-Token) keep winning; NSE_WEB_AUTH_COOKIE_DISABLE opt-out now also disables ACCEPTANCE (it always gated issuance). WebAuthMiddleware._is_public delegates to is_public_path (the shadow copy skipped the traversal guard — layer-divergence closed). auth_boot.py (NEW): server-boot handoff — resolve-without-generate (env > store > middleware's in-process token via web_auth_token_in_process()), export NSE_WEB_AUTH_TOKEN + NSE_WEB_ACTUAL_PORT to the process env and persist both to the gitignored repo-root .env (docker-compose's existing NSE_WEB_AUTH_TOKEN contract keeps working; the compose MAPPING key NSE_WEB_PORT is never overwritten); the token VALUE is never printed/logged. Launcher: starts at the recorded port (stable across restarts) + publishes at boot. doctor: probes /health on the resolved actual port, and a 503 verdict now reports "reachable, non-READY" instead of "unreachable". serve_alt_ui + vite dev: backend defaults read the recorded actual port; /app.js + /api_client.js are proxied so the cookie bootstrap survives the standalone-host lane. React client.ts: explicit credentials:"same-origin" + ONE-SHOT throttled 401 self-heal (refetch /app.js, retry once, GET only — mutations never replay).
...
- INVARIANTS: WEB-AUTH-P0 fail-closed DATA contract untouched — every /api route (incl. SSE) still 401s without a credential (pinned); public surface widened only to static document/asset shells; constant-time comparison unchanged; LIVE-mode auth enforcement unchanged (cookie is first-party HttpOnly SameSite=strict + 127.0.0.1 bind, opt-out honored for both issuance AND acceptance); no token value ever printed/logged/committed; engine hot path untouched (zero diffs under application/|features/); DEC-0002 honored (no Node at runtime — dist stays a static artifact); token-generation authority stays the middleware (auth_boot never mints).

## BUG-268 - Minimum-loss optimization grace gate still fell back to the HOST WALL CLOCK when no tick timestamp was threaded (clock-domain sweep residue, NSE-Swarm role 2 Backend/Domain, 2026-09-13)

- SEVERITY: MEDIUM (money-path exit gating; same class as G3/BUG-259/BUG-260). The clock-domain sweep fixed `_arbitrate_decision` (15d39552), `_calculate_hold_value_score` (7b7aeaa2) and `_update_mfe_mae`/`_position_performance` (BUG-260) but MISSED the fourth hold-age consumer: `PositionScoringEngine._evaluate_minimum_loss_optimization` (src/nexus_scalp/execution/lifecycle/scoring.py:513-514 pre-fix) kept `now_ref = datetime.now(UTC) if entry_time.tzinfo else datetime.now()` for its 60-second Spread Overcome Grace Period whenever the manage loop did not thread `now`. The gate decides whether the whole MIN_LOSS exit cluster (EV breach / deep drawdown / weak recovery) may fire at all: a host wall clock AHEAD of the broker/tick domain inflates the derived duration past 60s on a freshly-opened position (exit cluster can fire instantly, defeating the "let the trade breathe" guard); the opposite skew keeps the gate shut for the true grace PLUS the skew (minimum-loss exits suppressed — the production `Age: -10781.6s` direction). The naive fallback branch ALSO re-opened the mixed naive/aware subtraction crash class G3 retired. | STATUS: FIXED
...
## BUG-269 - Docker starter provisioner mints UNTRAINED weights the P0 serving gate refuses: fresh-volume deployment is a MODEL_LOAD_REJECTED restart loop (NSE-Swarm role 3 ML/Training, 2026-09-13)

- SEVERITY: HIGH (deployment truth — directly hits the one-click-install goal). The P0 serving refusal (60b785d3, #154: fresh-init canary + behavioral anti-degenerate probe wired into `application/live/model_bundle_store.py::_load_or_create_bundle`, lines 100-135) runs BEFORE any serving load, fail-closed. But `docker/provision_model.py` — the entrypoint's first-run starter (docker/entrypoint.sh:124, added in the 2026-09-09 stack repair 50df3a1e, BEFORE the gate existed) — kept minting `ScalpNet(70,3)` at ambient random init and blessing it with a matching manifest digest. The bundle classifies VERIFIED (`verify_artifact_integrity` — the digest is self-referential, it only proves weights==sidecar) and then dies on `check_model_behavioral_health` (probe, this run: `DEGENERATE:logit_std 0.073 < 0.15`, sensitivity ~0.008) -> ArtifactIntegrityError -> launcher fatal -> container restart -> identical failure forever. The provisioner exists precisely to prevent that loop (entrypoint comment: "MODEL_LOAD_REJECTED -> container restart loop") — the serving-gate wave updated the CI twin (runtime_gate.py:705-807 uses the seed-999 30-step AdamW TRAINED mint + fails loud at the seam, BUG-154) and missed the docker twin: the two provisioners drifted. RED-before reproduced live: old recipe output -> integrity VERIFIED + behavioral FAIL. | STATUS: LANDED-MERGED (PR #182 squash baf737ab 2026-09-13; ALL post-merge main gates green at baf737ab: CI, Security, OSV, DepLock, QA-Deep, JS, Docs, Tests OS Matrix win+mac)
...
- INVARIANTS: serving gate NOT relaxed (the fix provisions models the gate ACCEPTS — the gate itself byte-unchanged); no .github/workflows touch; no gateway/client contract touch; trained-champion volumes preserved byte-for-byte (champion-preservation contract of the skip path, pinned); CI runtime_gate + smoke paths unchanged (already servable-recipe compliant).

## BUG-270 - Alt-UI standalone host echoed client/upstream header bytes raw: 9 CodeQL py/http-response-splitting alerts on main (Hermes, 2026-09-13)

- SEVERITY: MEDIUM (CodeQL security-severity 6.1, precision high; alerts #1086-#1094 on refs/heads/main, tool 2.27.0). The stdlib email parser that ``http.server`` uses PRESERVES obs-fold continuations inside a header VALUE (``X-Request-ID: abc\n\tSet-Cookie: ...`` parses to ``"abc\n\tSet-Cookie: ..."`` — probe on CPython 3.11), and ``BaseHTTPRequestHandler.send_header`` buffers ``"%s: %s\r\n" % (kw, val)`` and latin-1-encodes with ZERO validation (unlike h11/uvicorn or ``http.client.putheader`` which rejects CR/LF). serve_alt_ui.py echoed the parsed request-id (9 sinks incl. static/relay/error paths) and relayed UPSTREAM names+values (SSE + buffered loops, incl. the HEAD content-length echo) straight into response headers — a fold-bearing value splits the response. | STATUS: FIXED (PR open; alerts auto-close on the next CodeQL run that reports the paths clean)
...
- INVARIANTS: correlation contract preserved (legitimate ids still honored/echoed/forwarded — pinned by existing tests); hop-by-hop stripping, fail-closed 502 envelope, traversal guard, log redaction, SSE passthrough semantics untouched; no src/ or workflow change; the request side already validated (http.client.putheader raises on CR/LF) — upstream could never receive a folded value even pre-fix.

## BUG-271 - Governed in-place serving-artifact persist (async retrain / collapse recovery / hot-swap) orphans the CHAMPION registry fingerprint: the P0-2 boot trust anchor refuses the NEXT cold boot — trading dies permanently after one legitimate retrain (NSE-Swarm role 4 Research/Backtest, 2026-09-13)

- SEVERITY: P0 (trading-path kill-switch hidden behind a SUCCESS log). The P0-2 ARTIFACT TRUST ANCHOR (`application/live/model_bundle_store.py::_verify_champion_registry_binding`, ~305-395: serving sha16 vs the newest `lifecycle_status='CHAMPION'` row's `artifact_fingerprint`, fail-closed `ArtifactIntegrityError` on mismatch) runs on EVERY cold load. But every GOVERNED in-place persist path — async-retrain success (`live_engine.py:3759` `_save_model_weights_atomic` -> `:3797` `_register_active_model(replaced=True)`), collapse recovery (`live/model_health.py:161-174`), promotion/rollback activation (`live_engine.py:1927/1950`), hot-swap (`live/hot_swap.py:180`) — re-registers provenance under the NEW fingerprint via `ModelRegistry._persist`'s ON CONFLICT INSERT, which lands with the column DEFAULT `lifecycle_status='CANDIDATE'`; the CHAMPION stamp stays on the OLD row forever. Next boot: serving bytes (new) != governed fingerprint (old) -> CRITICAL CHAMPION_BINDING_MISMATCH -> MODEL_LOAD_REJECTED -> the engine never serves again, with NO self-heal path (re-runs of the persist make it worse: every new row is CANDIDATE too). The boot-time `_sync_champion_registry_state` (live_engine.py:1568) never gets a chance — the refusal fires at `_load_or_create_bundle` (line 1536), BEFORE it. BUG-257's detection gap is therefore WORSE than attribution: the system cannot even distinguish its own governed retrain from an attack — it kills itself on the good path. | STATUS: LANDED-MERGED (PR #190 squash 5a9e36e3 2026-09-13 18:41Z; all 13 post-merge main check-runs success incl. Py Tests windows+macOS)
- ROOT CAUSE (reproduced live pre-fix, scratch probe at HEAD): champion registered fp=5267fecb4b5a5722 -> anchor PASS; in-place persist fp=6c12684d400a98c1 + provenance re-register (exactly the retrain sequence) -> rows = CHAMPION@old + CANDIDATE@new -> anchor REFUSE `ArtifactIntegrityError: HASH_MISMATCH` (trading dead after boot 2). The trust anchor and the provenance writer were shipped in different waves with no supersession contract between them — the same provisioner-drift failure class as BUG-269, one seam later.
...
- OPERATOR REPAIR (installs already orphaned by a PRE-fix retrain/hot-swap — boot refuses before any code heals): `sqlite3 artifacts/audit.db "UPDATE experience_model_registry SET lifecycle_status='ARCHIVED' WHERE lifecycle_status='CHAMPION'"` (engine stopped) -> the anchor reads NO_CHAMPION_ROW and stays INERT, boot proceeds, and the boot-time `_sync_champion_registry_state` BOOTSTRAP re-registers the serving artifact as the truthful CHAMPION with its live fingerprint. Deliberately manual: an automatic boot-time heal would also heal ATTACKER bytes.

## BUG-272 - Champion-drift sentinel: the P0-2 trust anchor fires at BOOT only, so an out-of-process rewrite of the serving artifact while the engine runs is served silently until the next refusal (BUG-257 remainder; NSE-Swarm role 5 Data/Ingestion, 2026-09-13)
...
- REGRESSION: tests/unit/test_bug257_champion_drift_sentinel.py (11, gated in critical_suite.txt): pure verdict table, real-DB probe match/drift/INERT postures, DRIFT<=>anchor-REFUSE equivalence through the real ModelBundleStore helper, wiring pin (probe call inside run_cycle + engine drives run_cycle), None-sentinel first-due, throttle, 2-sighting alarm discipline + clear, failure isolation, end-to-end on a real WAL audit DB, import-light pin.
...
- LEDGER: closes the BUG-257 "REMAINING: alerting between boots for OUT-OF-PROCESS writes" row; LEARNFIX1-PROVENANCE-PREEXISTING-RED CLOSED (root cause: b1eb5b94 BUG-244 smoke-scoped the emission gate the tests' `pytest.raises` asserted — the two pins needed smoke=False; NOT a host-env artifact, they were red in CI too because the file is not in the critical manifest, only unrun).

## BUG-273 - HOLD_AGE_FALLBACK rate limiter used the `_last=0.0` vs time.monotonic() throttle pattern: the FIRST warning is silently suppressed on any host with uptime < 300s (fresh CI runner / just-restarted engine) — proven red on PR #192 CI (NSE-Swarm role 5, 2026-09-13)
...
## BUG-274 - Bound-seam wrapper-state leak class: 13 `getattr(self, ...)` reads in P1-seam wrapper classes target attributes that only exist on the composition root — silently permanent-default (throttles never engage, hourly log gates fire per-bar, two subsystems unreachable) (NSE-Swarm role 6 Runtime/Linux, 2026-09-14)
...
- SYMPTOM (behavioral): (1) RuntimeLoop: the PERF-02 5s account-refresh + snapshot throttles NEVER engage — the loop performs the per-tick gateway RPC (`get_account_info` + `get_account_snapshot` + `_update_runtime_mode`) the throttle exists to eliminate, on the remote-gateway shape that is exactly the tick-path stall risk; the companion test test_perf02_account_refresh_throttle.py pins a hand-copied MIRROR of the block on a `_FakeEngine` that re-creates the wrapper-local pattern, which is why shipped code drifted while CI stayed green. (2) BarHandler: the once-per-hour BUG-185 width-split CRITICAL and the once-per-hour DISABLED_BY_CONFIG INFO short-circuit their own rate gates (missing stamp -> falsy -> `not None/0.0` is True every call): with the retrain counter past the interval (never resets while fine-tune is disabled by default) they log on EVERY completed M1 bar — the EXEC_TRACE/BUG-273 volume class again. (3) BarHandler candle-intel + MSLIE bar feeds can never run even when the operator enables the subsystems (permanently None through the wrapper); the radar payload's `news_state`/`decision_reason` are permanently null. (4) TickPipeline live chart-overlay refresh (server_state gate) never runs from the tick path — UI visuals freeze to bar-close/manual-sync cadence. (5) ProtectionEngine VOLATILITY_EXPANSION breakeven-breach check can never observe a price (`{}` on the wrapper) — the protective market-close branch is dead.
...
- REGRESSION: NEW tests/unit/test_bug274_wrapper_state_leak.py (10, gated): RED-BEFORE PROVEN 10/10 at baseline fd95a7b0 in a clean worktree (worktree copy probe), 10/10 GREEN post-fix. Behavior pins: hourly CRITICAL/INFO fire exactly once across 3 bars + land on the ENGINE stamp; self-disabled trainer takes the width-guard early return; radar news_state/decision_reason flow from engine doubles; engine-mounted mslie_engine receives the bar feed; md7-style source pins for the loop/pipeline/protection reads; CLASS-WIDE AST guard over the 6 bound seams (bar_handler, runtime_loop, tick_pipeline, protection, maintenance, reconciliation) rejecting any `getattr(self, X)` where X is never assigned in-file (allowlist-by-construction: legitimate wrapper-local state is pinned too).
...
- LEDGER: the AST sweep also triaged the remaining `self.X` reads in the UNBOUND seams (inference/runtime_mode/model_bundle_store/model_health: methods invoked as `Cls.method(engine, ...)`, self IS the engine) — NOT leaks, contract documented in their docstrings. ModelBundleStore._load_or_create_bundle dual-mode getattr guards (allow_legacy/unbound audit/_verify anchor) are explicit by-design. No cron-scope action: NIGHTLY-E2E(Docker) has had NO scheduled run since 09-13 07:36Z (the ~09-14 07:30Z occurrence never fired; schedule gap is operator-visible, the dcc57826 seed fix stays OPEN-MONITOR until a nightly actually runs on it).

## BUG-275 - DB-hygiene cadence clock-domain mix: RuntimeCleanupScheduler stamped _last_light/_last_deep with time.monotonic() while its ONLY production caller (MaintenanceCycle) compares against its wall-clock now_t -> is_deep_due PERMANENTLY True: the deep index-scan branch ran on EVERY 30-min light cycle instead of once per 6h (NSE-Swarm role 6 Runtime/Linux, 2026-09-14)
...
## BUG-276 - Baseline-skeleton CONSTRAINT shadow (PERF-DEADLETTER remainder): gate-first fresh DBs have no UNIQUE indexes for 15 of the 17 ON CONFLICT producers — every idempotent upsert dead-letters FOREVER (NSE-Swarm role 7 QA/Tests, 2026-09-14)

- LIVE ACCEPTANCE (2026-09-15, role 12): first POST-FIX scheduled Nightly Client E2E (run 34944422367, 07:58Z, core.log artifact e2e-evidence-9, 500 lines from a fresh gate-first volume): ZERO 'ON CONFLICT clause' dead-letter lines, ZERO 'Audit batch insert failed' lines, accounting/intelligence worker cycles flowing — the ~19 rows/s guard-telemetry flood from the pre-fix log is GONE. CLOSED (the run's overall red is the independent NIGHTLY-E2E-CURLF readiness shape: container Up (healthy) while curl -f eats the 503 DEGRADED body).
...
- FIX: (1) `APP_UNIQUE_TARGETS` (app_columns.py, stdlib-only, machine-readable copy of the producers' ON CONFLICT targets); (2) engine baseline builds the unique indexes at gate time (tables are empty inside the same transaction — fail-loud, `BASELINE_UNIQUE_HEALED` event); (3) `AuditRepository._ensure_unique_constraint_heal` runs at the END of `_create_sqlite_tables` on every boot, repairing EXISTING poisoned databases (the field shape: DB already built by the pre-fix engine): PRAGMA-gated zero-churn on healthy DBs (app-first boot adds ZERO indexes — pinned), recreate-when-dead on skeleton-shadowed DBs, and when existing rows genuinely violate the constraint it logs `BUG-276 skeleton-shadow heal FAILED ...` and continues (never crash-boot, never silent).
- REGRESSION: NEW tests/unit/test_bug276_unique_constraint_shadow.py (27 gated, added to critical_suite): 15 parametrized gate-first target-resolution pins + 6 real-producer-SQL replay pins (double-execute = dedup/aggregation semantics asserted, zero dead letters) + the exact log_signal(TICK_DUPLICATE_SUPPRESSED) worker-path pin (count aggregates to 2, dl=0) + poisoned-DB boot-heal (pre-heal raises the exact E2E error, post-heal upserts work) + genuine-duplicate survival (boot does not crash, conflicted table skipped, others healed) + healthy-DB zero-churn + baseline-heal stability across re-boots + CLASS GUARD: regex scan of every `INSERT ... ON CONFLICT(...)` in src/ against a gate+bootstrap database (scanner must match >=18 targets or the guard fails loudly itself).
- EVIDENCE (this run): battery 27 passed RC=0; test_perf_deadletter_skeleton_repro 48 passed RC=0 (column half unaffected); database-platform/BUG-254-idempotency/dead-letter-split/retention lanes 38 passed RC=0; experience/research/telemetry/registry -k lane 250 passed RC=0; scripts/ci/check_migration_safety.py OK RC=0 (audit/news/candle reach expected version on disposable DBs); ruff check + format repo-wide clean; mypy Success on the 3 touched src files.
- NOTE (separate, OPEN-OPERATOR): the same E2E run's hard failure message is "engine did not become ready within 300s" while ps.txt shows the container Up (healthy). The readiness loop in .github/workflows/nightly-e2e.yml:72-77 uses `curl -fsS` — curl -f suppresses the body on 503, and /health answers DEGRADED with 503 (docker/healthcheck.sh parses the 503 body precisely for this reason). A DEGRADED-but-operational engine can NEVER pass the workflow probe. Workflow files are out of this job's write scope; proposed one-line fix: `body="$(curl -sS --max-time 3 "$NSE_E2E_BASE/health/ready" ...)"` or parse the 503 body like healthcheck.sh does. Tracked as NIGHTLY-E2E-CURLF.


## BUG-277 - `nexus doctor` WARNING-actions filter matched the RENDERED label "WARN" against the producer vocabulary "WARNING" — the "USER ACTION / NEXT" operator-guidance surface was permanently dead (NSE-Swarm role 9 Observability, 2026-09-14)
...
- SYMPTOM (deterministic, host-independent): any DEGRADED install whose guidance lives in WARNING entries — `DATABASE` "schema 8 behind expected 9 → Run `nexus db migrate`", `NEWS` disabled, `GPU` NVIDIA-without-CUDA, `LOGGING`/`SHADOW`/`MEMORY` suggestions — prints `USER ACTION: 0` and `NEXT: nexus start   (paper mode by default)`. On a migration-behind ledger that is precisely the BUG-254/276 dead-letter pre-condition, told to the operator by the tool designed to catch it. The `--fix` path is unaffected (FAIL-derived), JSON output unaffected (raw dicts shipped).
...
- EVIDENCE (this run): battery 8 passed RC=0 (post-fix), baseline worktree 4/8 RED pre-fix incl. both behavior shapes + both source pins; neighbor lanes RC=0: release_system + perf_health_probe_cache + bug275 + bug276 = 64 tests; cli e2e -k "doctor or health" 5 passed (incl. the >=19 checks-count pin); uvx ruff@0.16.6 check . + format --check . clean repo-wide (1993 files); mypy Success on doctor.py. Pre-existing host reds in test_runtime_truth_hardening (3) BASELINE-PROVEN identical at be1842d2 in a clean worktree (K3 data/raw absent — 14th check).
- LEDGER: `check_worker_liveness` and `check_a2a_gateway` are defined but NOT in run_all()'s checks list (health.py:518/:573 vs :1160-1185) — by-design or drift UNKNOWN; left untouched, flagged for a future role-9 decision because wiring them changes the /health verdict surface (DEGRADED-adjacent, NIGHTLY-E2E-CURLF interaction). Dead-letter observability wave result: counters ARE wired (debug_snapshot.py:970-978 + the store's per-event WARNING + retention cap) — no gap found; /health 503 body DOES surface per-category reasons+state (`checks` array), so the CURLF adjacent ask (operator-visible DEGRADED reason) is already served once the workflow probe stops discarding the body.


## BUG-285 - Durable financial-overflow was WRITE-ONLY (perf-wave R2): an overflowed audit row never returned to the ledger and the directory grew unbounded (2026-09-15, NSE-Swarm role 10 Accounting/Forensics; ID NOTE: reserved as BUG-278 mid-cycle, renumbered after the Hermes wave took 278 for the test-harness artifact clobber while PR #203 was in flight — same swarm-collision class the wave documented, keep both entries)
...
- EVIDENCE (this run): RED-BEFORE proven at ddf5c946 baseline worktree (battery fails on the absent API), GREEN at HEAD: bug278 16+2 RC=0; neighbor lanes RC=0: runtime_safety+persistence+dead_letter_retention+store_split 46; bug276+bug277+perf_deadletter_skeleton 72; debug_snapshot_phase20 + qa_deep_observability + observability_guardrails RC=0 (2 pre-existing HOST reds in phase20 TestDebugApi BASELINE-PROVEN identical at ddf5c946 — web-auth 401 env shape, not this change). `ruff check .` + `ruff format --check src tests` repo-wide clean; mypy Success on both touched src files; critical manifest: 120 entries, zero missing.
## BUG-278 - Repo-root pytest CLOBBERED the production champion bundle + contaminated the live model registry with CHAMPION rows (Hermes wave 2026-09-14; ID NOTE: executed under 'BUG-276', renumbered after role7's PR #196 took BUG-276 mid-wave — classic swarm ID collision, keep both entries)
...
- VERIFIED: serving sha16 c9982ddde1755591 before/after the full verification battery (byte-stable); 0 new production-DB contamination rows during runs (audit_signals 1469 stable, registry max id stable); tests/unit/test_bug278_test_harness_artifact_isolation.py (8, critical_suite-pinned) incl. quarantine migration behavior tests; neighbors bug271/272/257/269/agent3-sync/bug223/latency 50 passed; mypy src clean, ruff clean; pre-existing env-reds (WEB-AUTH 401 + bug247 source-pin + fidelity 4-head vs 3-class artifact drift) re-verified IDENTICAL on clean 9431edd2 worktree — not introduced by this fix.
...
## BUG-280 - Order-frequency throttle was a HARDCODED 60s floor that pre-empted the configured cooldown: 96:1 evaluation swallow (78,798 throttled vs 1,469 decisions), entry cap invisible to operators (Hermes wave 2026-09-14)
...
## BUG-281 - HIGH_SPREAD_CHOP chop boundary calibrated on a stale era froze NORMAL live spreads: guardian FREEZE_ALL killed 24% of decisions (median killed spread $0.30 = routine) (Hermes wave 2026-09-14)
...
## BUG-282 - RSS pubDate (RFC-822) silently fabricated to ingest now(): 88.1% of news rows carry a fake event time AND the 60-s publish bucket in the article-hash re-minted identity per poll, defeating dedup structurally (233 MB news.db, 26,161 byte-identical rows) (Hermes wave 2026-09-14)
...
- NOTE (SCOPE): the hunter layer remains research/UI-scoped (no live order authority) - this fix makes its DETECTION/labeling honest, it does not enable a new live path (INV-001 unchanged). Dead-API cleanup (hunter_gate_frame/build_hunter_frame/best_strategy_for/validate_setup_type zero-caller set), phantom hunter_pullback_v1 hint, stop_distance unit leak (M4) and the AND-wall -> compositional entry scoring remain taskboard follow-ups; recorded in docs/audit/wave_20260914/03_setup_intel.md §7.

## BUG-284 - Phase-09 suitability REJECT fired on 1-4 sample families: evidence contract INVERTED (zero evidence passes, ONE loss blocks) - 473/1373 decisions (34.5%) died at SUITABILITY_BELOW_THRESHOLD, experience-paralysis (Hermes wave 2026-09-14)
- SYMPTOM (VERIFIED lane-08 §4, production ledger): the largest single rejection source after the policy/confidence gates was the soft suitability gate, driven by micro-samples. _suitability_score starts 0.5, clamps expectancy contribution to -0.30, adds -0.10 for n below 5, -0.20 for DEGRADED; min_suitability_to_qualify=0.40. A family with ONE closed losing trade (expectancy -1R, n=1) scores ~0.15 (below 0.40) -> REJECT -> NO_TRADE + confidence 0.0. But ZERO evidence routes to Phase-08 INSUFFICIENT_EVIDENCE which PASSES the proposal bit-identical. So a strategy was punished harder at n=1-2 than at n=0, and the rejections emit NOT_DISPATCHED outcomes that never count as samples -> the family could not accumulate the 5+ samples needed to escape the penalty it was rejected for. Self-limiting trap ("lost twice -> stop trading"), beyond the designed risk gates (3-loss 1h freeze / survival mode are separate).
...
- NOTE (SCOPE, deliberate non-changes): (a) Phase-08 experience DEGRADED/PENALIZE (x0.70 confidence) untouched — that is a real recency signal, not a micro-sample artifact. (b) The gate is still hardcoded enabled=True at live_engine construction (no config kill-switch, lane-08 §2 governance gap) — a SEPARATE open item, NOT silently changed here because flipping it live needs an operator decision. (c) The deeper architecture fix the wave calls for (compositional entry scoring replacing the AND-wall, lane-03 §5) is a larger design change tracked as a follow-up; this fix removes the worst single paralysis source with the smallest reversible seam. (d) The double Phase-08 evaluation (gate.evaluate re-calls evaluate_proposal after tick_pipeline already did — lane-08 §2 side-observation, cosmetic counter double-count) left as-is to keep the seam minimal; candidate for a follow-up.

## BUG-285 - Corrupted-price boundaries failed OPEN: TickData accepted +/-inf (pydantic gt/ge pass inf: ask=inf -> spread inf, both-inf -> spread NaN), BarData accepted NaN/inf, and get_historical_bars forwarded NaN/inf/inverted-OHLC broker rows into the canonical series (validate_ohlc_bars was report-only) (Hermes wave 2026-09-14)
- SURFACE (lane-11 L11-1/L11-2, probes VERIFIED at 0c855019): TickData(bid=1.0,ask=inf) ACCEPTED -> spread_points=inf; (inf,inf) ACCEPTED -> spread NaN; volume/last=inf ACCEPTED. NaN was rejected only as a side-effect of `gt` (nan>0 False). On the history path, providers.validate_ohlc_bars computes a full report but get_rate_history only LOGS invalid=N; the BarData mapping loop in mt5_adapter.get_historical_bars filtered None fields only -> non-finite / high<low / non-positive rows reached BarData and from there the feature windows, warmup reseed, replay and training (the 2026-09-02 EUR-scale-on-XAUUSD contamination in candle_intel.db, 84+13 rows, is this class).
...
## BUG-286 - Review follow-up on merged #199 (code-review, 2026-09-15; ID NOTE: reserved 285 mid-flight but the wave's price-boundaries fix took BUG-285 at 116f7efd while this work was uncommitted — renumbered to 286, same collision class as 276->278): BUG-279 weekend predicate RESTATED the calendar window + a second sentinel-file pair reached main ('the', 'min_samples_to_reject' tracked by 94ecf2c7 INSIDE the wave)
- FINDING-1 (threshold-ownership, same class BUG-281 pinned for spread): LiveEngine._feed_stall_in_weekend inlined its own Fri-22..Sun hour arithmetic while claiming "same semantics as accounting/market_calendar". A sweep over all 168 hour-slots of a full week proved the time-classification AGREES exactly (delegation is behavior-preserving; no Sun 21:00-22:00 escalation hole as the review first suspected — the calendar's own hour-check is equally conservative, only its next_open/reason math says 21:00). The DEFECT is the restatement itself: two sources of truth for one physical fact, rotting the day the calendar recalibrates. FIX: delegate to market_calendar.market_state(...) == WEEKEND (the production-proven seam market_entry_gate already uses); stale "next Monday's first stall pass re-evaluates" comment corrected (escalation resumes at Sun->Mon midnight if reopen was unhealthy; a healthy reopen closes the episode via the fresh-tick stamp).
...
## BUG-288 - Audit worker ADOPTS a rebound queue: the "deterministic" BUG-140 stall test is a scheduling coin flip (windows-latest CI red at b1fe0137, run 34925973236; NSE-Swarm role 12 Reviewer/Gatekeeper, 2026-09-15)
...
## BUG-289 - telegram_notify CLI rejected the call shapes the CI workflows actually emit: OS-leg and JS-lane finish notifications NEVER delivered (found by the same windows run log, NSE-Swarm role 12, 2026-09-15)
...
- ACCEPTANCE PARTIAL (PR #209 CI, second cause disclosed): the js lane now PARSES (no more usage error) but still dies there for an INDEPENDENT pre-existing reason — js-tests.yml installs no Python deps, so `from nexus_scalp.observability...` raises ModuleNotFoundError: structlog (job 104297888448 log 07:49:19Z; hid behind the usage error before, same || true cover). That half needs a workflow edit (add the python setup or drop the step) = OPEN-OPERATOR. tests-os.yml legs install the full env, so os-finished --os is fully effective there (acceptance = next windows/mac leg shows no 'unrecognized arguments').
- TEST-SHAPE LESSON (self-inflicted, fixed in 25afc95e): the first pin asserted the unconfigured-Telegram payload, but CI injects TELEGRAM_BOT_TOKEN/CHAT_ID into EVERY job -> the reporter attempted a real send (SEND_FAILED red). CLI grammar pins must spy the DISPATCH seam (class-level monkeypatch of notify_run_finished) and assert the os_name tag, never depend on ambient config.

## BUG-290 - Perf-wave R7+R9: the engine's in-process guard maps grew unbounded for the process life (`_processed_orders` 50k+/day never pruned; `_ai_flip_warn_times` carried a dead "cleaned up with the ticket" docstring with NO pop site) (2026-09-15, NSE-Swarm role 1 Architect)
...
- SAFETY CASE (why eviction is correct, not a dedup hole): the in-memory guard is a SAME-SESSION fast pre-broker filter only; cross-boot duplicate protection is durable via the `idx_executions_order_status` UNIQUE identity (adapters/database/executions_idempotency.py, BUG-254 lineage) + the INV-006 outcome collapse pinned by test_recon_duplicate_deal_across_restart.py. Policy request_ids are per-tick, so same-boot replay of an entry older than 50k dispatches (~weeks of scalping) is not a live threat shape.
...
## BUG-291 - Perf-wave R8: AccountingCore._report_cache never evicted — every distinct period key ever browsed stayed resident forever (2026-09-15, NSE-Swarm role 1; sibling of BUG-290, shared fix seam)
...
## BUG-292 - Perf-wave R6: the C3 session spread-percentile gate did SQLite I/O INSIDE policy evaluation — one connection + same-day scan per spread-positive candidate, on the event-loop thread (2026-09-15, NSE-Swarm role 2 Backend/Domain)
- SYMPTOM (documented in the perf wave, unbounded only by cadence): `LiveEngine._session_spread_percentile_provider` (live_engine.py:1659-1698 pre-fix) opened `audit._connect_sqlite(5.0)` and ran `broker_history.session_spread_percentile` (a `SELECT spread FROM audit_paper_executions WHERE symbol/ts/status` scan + sort) on EVERY call. `SignalPolicy` calls the provider from `policy.py:772` inside `evaluate_probabilities`, i.e. on the loop thread, for every candidate that is live-spread-positive — the tick path paid connection churn plus a scan whose cost grows with the day's fill count. `docs/audit/wave_20260914/10_performance.md:127` states it plainly: "documented, bounded, but still loop-thread I/O + per-call connect", prescribed fix: "maintain the same-day spread-percentile sketch incrementally off-loop (audit worker or maintenance tick), expose RAM read; refresh cadence <= 60 s."
- RED-BEFORE (executed probe, not a claim): /tmp-style harness calling the REAL provider three times against a stub whose `_connect_sqlite` counts calls — `CONNECTS_PER_3_EVALUATIONS=3` at bae2dc19, `=0` at the fix. Battery pins the same shape three ways: a `sqlite3.connect` bomb, an `AuditRepository._connect_sqlite` spy, and a source-pin class guard (forbids `_connect_sqlite` / `sqlite3.connect` / `session_spread_percentile(` inside the provider body — the BUG-285 "comment says bounded, code says per-call" class).
...
- PARITY IS THE CONTRACT (this is a money path): the sketch reads the SAME durable `audit_paper_executions` copy with the SAME row-selection semantics (FILLED, spread IS NOT NULL AND spread > 0, exact symbol, ISO-STRING window bounds compared exactly as SQLite compares TEXT, same-UTC-day-clamped trailing window) and shares ONE interpolation implementation — `interpolated_percentile` lives in `broker_history` (the lower layer) and the provider now calls it too, so the formula cannot drift between surfaces (a second copy = the MIRROR-TEST-CLASS defect). Pinned by a 20-combination parametrized equality test (percentile x reference instant, incl. the day-floor-crossing window) against the live SQL function on a deliberately messy fixture.
- HONEST-UNKNOWN preserved on three levels: thin sample -> None (never 0.0 fail-open), missing substrate (LIVE-only install, no paper table) -> counted pass, no alarm, no fake number, and a DEAD refresh -> None after `MAX_REFRESH_FAILURE_STREAK`/`max_age_sec` instead of defending a frozen distribution forever. `snapshot_debug()` + `debug_snapshot._policy_section["spread_session_sketch"]` make "gate off because the session is thin" distinguishable from "gate off because maintenance died" — previously indistinguishable.
...
## BUG-293 - Lane-11 input-validation fail-open boundaries: corrupt paper_state.json was PARTIALLY APPLIED before failing, and gateway order payloads silently defaulted volume 0.01 / price 0.0 straight to the broker (2026-09-15, NSE-Swarm input-validation lane; ID NOTE: the lane brief preferred BUG-290, taken at base by the perf-wave guard-maps entry (bae2dc19) — renumbered to 293, same swarm-collision class as 276->278 and 285->286)
...
- FIX (L11-4): `_load_state` is now candidate-then-swap — `_build_state_candidate` parses+validates the ENTIRE payload (balance/equity finite + non-negative, ticket counter + closed tickets non-negative ints, EVERY position row must validate — a bad row rejects the whole file, never a silent drop) into a detached dict; engine fields are assigned only after full success. On rejection: state untouched, exactly ONE loud structured WARNING `PAPER STATE LOAD REJECTED` with `corruption_class` naming the defect (balance_negative / equity_not_finite / unreadable_or_invalid_json / payload_not_object / position_row_invalid[i] / ...), file left in place for the next healthy persist, never crash-boot (may-degrade tier honored).
- FIX (L11-6): NEW single-owner seam `src/nexus_scalp/gateway/order_validation.py::validated_order_request` — mirrors `_validate_pending_request`'s audited `(ok, reasons)` error shape; required fields must be present + non-null (missing volume fails closed, it is NEVER invented as 0.01), numerics bool-rejected + finite + in range (volume > 0, price >= 0, sl/tp >= 0; legacy TradeOrder path keeps its domain gt=0 contract incl. no more 1.0 placeholder coercion), symbol non-empty, order_type never defaulted. Server rejects BEFORE any adapter dispatch with HTTP 200 + `{"status": "FAILED", "message": "invalid order request", "reasons": [...]}` — the wire shape the client already maps to its typed REJECTED tri-state; HMAC auth untouched; client signing untouched.
...
- NEIGHBORS: paper persistence/replay/provenance/parity/lineage/launcher/data-wiring + gateway e2e/secrets/remote-client suites green at fix (see PR CI); ruff check/format, mypy Success on all three touched modules.
- REVIEW FOLLOW-UP (lead on #217): ROOT-ANCHORED the whole shell-redirect sentinel block in .gitignore (/PAPER, /LIVE, /the, the '/278)' paren literal etc.) — unanchored single-segment patterns match at ANY depth and, case-insensitively on Windows, 'PAPER' shadowed src/nexus_scalp/adapters/paper/ directory-wide (the git add -f incident above is the evidence). Leading-slash patterns are root-relative and can never eat source trees. Aligned the BUG-286 class-guard pin test_known_offenders_ignored to REQUIRE the anchored form (a future re-unanchor fails CI). A dedicated ignore-anchoring guard script is a disclosed follow-up, not this round.

## BUG-294 - Perf-bounds lane (NSE master wave): the bounded-runtime fixes (BUG-287 R9 print / BUG-290 R7 / BUG-291 R8) were pinned only STRUCTURALLY — no test drove the eviction or the stdout hazard END-TO-END through the real seams (2026-09-15, NSE-Swarm role perf-bounds; ID NOTE: lane preferred BUG-288, taken by #209 queue-adoption race; 290/291 taken by role 1 at bae2dc19 while this lane scoped; 292/293 reserved by the concurrent zero-state-bootstrap and fail-open-boundaries lanes (branch-name convention, ledger not yet written) — took the next free number 294, same swarm-collision class as 276->278 / 285->286; convergence NOTE also in docs/agent_handoffs)
...
- FIX: tests-only. `tests/unit/test_bug294_bounded_runtime_behavior.py` (6 pins): A1a inspect-scopes the UNKNOWN if-block (comment-stripped) for `print(` absence — tighter than the file-wide R9 regex in test_obs_trace_chain.py; A1b runs `log_signal` on an UNKNOWN proposal with `sys.stdout` a raising object (logger double per the OBS-TRACE routing ruling) and asserts the structured warning still fires and the row persists — stdout is write-invisible to the decision path; B1 shrinks the manager's OWN guard instance (env-free caps; instance is the deterministic lever, same technique om-2 used on a probe) and drives limit+N distinct request_ids through the REAL `om.dispatch_order`: size capped, oldest evicted, newest retained, retained id dedupes with ZERO broker sends, EVICTED id re-accepted — the documented BUG-290 trade-off made visible (durable guards: executions UNIQUE + broker request-id idempotency); B2 pins `in`-does-not-touch through dispatch writes; C1 browses 6 DAY periods on a cap-4 instance through the public API: residency capped, evictions counted, evicted period recomputes with warm-IDENTICAL derived truth (cache advisory); C2 dict-parity identity serving through `period_report`.
- REGRESSION: the new file IS the regression net (no src diff to regress). Gates: 6/6 new + neighbors 53/53 (test_order_lifecycle + test_execution_architecture + test_accounting_advanced_metrics + test_execution_quality_contract + test_bug290_bounded_guard_maps + test_obs_trace_chain), ruff/check-format/mypy clean on the new file. RED-BEFORE structural by design (comments in file): at 65d3cb50 A1a/A1b fail on the live print; at 2e53fd04 B1/C1 cannot bind (no BoundedLRUMap).

## BUG-295 - Hygiene SAFE_CLEAN was permanently inert on the worker-state / news / lifecycle rules: four policies named columns that do not exist in the production DDL and one filtered a never-stored event_type — the planner skipped every one SILENTLY (2026-09-15, NSE-Swarm hygiene-rules lane; ledger ID reserved as 295 by the master mission)
- SYMPTOM (lane-04 DBF-004, VERIFIED against the production artifacts, docs/audit/wave_20260914/04_db_forensics.md §3.1/§4/§7): 417 recorded hygiene cycles (archive/_hygiene_state/hygiene_state.db) with deleted=0, archived=0, bytes_freed=0 in EVERY row; news.db (233 MB) was 100 % undeletable via SAFE_CLEAN even with apply; audit.db shrank only through BUG-054's own in-repository purge, never through the hygiene worker.
...
- FIX (all in hygiene/worker.py + registry mirror in retention.py; every existing protection semantic preserved): (1) worker-state rules repinned to `last_cycle_at` (single-row upsert checkpoints — age-bounded DELETE of an idle checkpoint matches the existing STALE_TEMP intent; producers recreate via ON CONFLICT); (2) `news_health` moved to a NEW `ts_cols` list semantics — a row qualifies ONLY when every timestamp carrying evidence (IS NOT NULL AND not '') is older than the window AND at least one carries evidence: either-timestamp-fresh = live source, survives; no-evidence (NULL or ''-defaults) = keep-when-uncertain (spec §73). (3) `event_type` now accepts a scalar OR a list (`event_types`); position_lifecycle_events keeps the legacy MOVING key AND adds the four real churn classes; the durable BUG-054 purge in audit_repository.py stays MOVING-only, untouched (money-path twin with its own tests). (4) NEW `news_analysis_runs` rule (started_at, 30 d, run_id TEXT PK) — lane-04 §3 measured 26,160 rows / 4.0 MB at 1,189/day of pure bookkeeping with zero cleanup paths. (5) LOUD SKIP: a rule whose ts_col(s) are absent now appends a dead-rule entry to plan.blocked AND logs `HYGIENE_DEAD_RETENTION_RULE` at WARNING (event name uppercase-snake so the BUG-141b redactor constant-shape guard exempts it — a lowercase 24+ char event name gets entropy-redacted out of its own log line; pinned). (6) planner COUNT and executor DELETE now share ONE `_retention_where(cfg, cutoff)` builder so the two paths cannot diverge; single-ts_col rules stay bit-identical (` WHERE col < ?`). (7) RetentionEngine registry: purpose text names the real age columns; `news_analysis_runs` flipped never_delete to a WARM-30d STALE_TEMP mirror (rebuildable=False — nothing rebuilds a run manifest) per lane-04 §7 "WARM 30 d DELETE".
- REGRESSION: tests/unit/test_bug295_hygiene_dead_rules.py (NEW, 10 pins; hygiene family is NOT critical-suite-registered — kept out of tests/critical_suite.txt per mission): fixtures use EXACT production DDL column sets copied from audit_repository.py / news/db_schema.py (NOT task11's old fake shape); RED-before executed twice — collection ImportError pre-fix plus a behavioral probe on stashed-out source (pre-fix planner on production-shaped stale rows: candidates 0, blocked 0, no warning — silent skip reproduced). Pins: healed worker-state rules find stale rows / fresh+NULL never; dead rule = blocked entry + caplog WARNING (configure_logging rebind after caplog per BUG-140 lesson); shipped-rule class guard (zero dead blocks against production-shaped fixtures); news_health seven-shape truth table incl. ''-defaults; lifecycle vocabulary sweep with legacy MOVING still matched and CREATED/OPENED/EXITED never; analysis_runs TEXT-PK plan+executor delete; single-key regression (audit_signals/candles counts unchanged + exact WHERE string pin); end-to-end SAFE_CLEAN worker run (stale checkpoint/health/run rows gone, live rows survive, verification PASS) + DRY_RUN zero-mutation. tests/unit/test_database_hygiene_task11.py fixtures reshaped to the production worker-state/news_health shapes (its hyg04/hyg08/hyg12 pins now prove STALE_TEMP deletion on REAL columns; 13/13 green unmodified in intent). Neighbors RC=0: storage_policy + storage_lifecycle_wiring + logging_retention_runtime + perf_deadletter_skeleton_repro (56) + hygiene-adjacent cadence/guardrails/operational-log suites (61); whole-battery 140 passed. ruff/format (full repo, 2045 files) + mypy clean on touched files.
- ID NOTE: this entry was reserved as 295 by the master mission because 292 landed on main while 293/294 are CONCURRENT in-flight lanes (their ledgers exist only on origin/fix/bug293-* / fix/bug294-* branches at cut time); an all-remote-branch ledger scan at push time found no 295+ claim. If one appears at merge time, this is the documented collision class — renumber to the next free ID.


## BUG-296 - Z-B1 zero-state bootstrap: NO supported non-docker model provisioning existed — fresh clone `nexus start` died MODEL_LOAD_REJECTED while health/doctor hints advertised a `nexus repair --model` flag that DID NOT EXIST; plus Z-B5 `nexus db migrate` FileNotFoundError on missing artifacts/ and W12 cp1252 pre-bind panel crash (2026-09-15 NSE-Swarm ZERO-STATE lane; ID NOTE: brief preferred BUG-289 — taken at base by role-12's telegram_notify entry (agents/bugs.md:8689); renumbered to 292 — taken on origin/main by perf-wave R6 (fa34fccd) while this lane worked; final 296, free on origin/main; same swarm-collision class as 276->278 / 285->286 / 286)
- SYMPTOM (lane-05 E1/E4/E6/E8/E9 + wave-12 step-2/3/4): (1) `git clone -> pip install -> nexus start` crashed with a raw ArtifactIntegrityError traceback rc=1 — the P0 serving gate fail-closes a missing artifact (correct), but there was NO documented non-docker path to provision the starter bundle; `nexus repair --model` returned typer rc=2 "No such option" even though three HealthEngine hints (MODEL/MODEL_CONTRACT/FEATURE_SCHEMA) printed exactly that command; (2) a bare/force_fresh mint without sidecars BRICKS the next boot as LEGACY_UNVERIFIED (E8) and force_fresh clobbers digest-VERIFIED bytes without ceremony (E9); (3) `nexus db migrate` on a workspace without artifacts/ died FileNotFoundError on the migration LOCK path (os.open does not create parents); (4) under redirected stdout (Windows cp1252 default) the "Migrations applied" success panel's U+2713 and the welcome panel's U+25CF raised UnicodeEncodeError inside `_run_engine` BEFORE uvicorn bound — the boot died on its own status output (reproduced: exact charmap crash).
- FIX (Z-B1): NEW single-owner seam `src/nexus_scalp/release/model_bootstrap.py` owns the BUG-269 seed-999 TRAINED starter recipe (mint_trained_starter), the integrity sidecar stamping (manifest.json model_sha256 + scaler_sha256 BOUND + provisioner marker, model.meta.json with model_sha256, model.scaler.npz — E8: sidecar-less mint forbidden), the servability probe, and `provision()` refusal semantics: governed registry CHAMPION (same row query as the P0-2 trust anchor) -> REFUSED never clobberable; digest-VERIFIED servable foreign bundle -> KEPT byte-for-byte even under --force (E9); declared starter -> SKIPPED idempotent (--force re-mints); digest-valid but DEGENERATE -> re-mint (BUG-269 silent-skip trap kept); LEGACY_UNVERIFIED/HASH_MISMATCH/absent -> MINT. `nexus repair --model` (cli/doctor.py, + `--force`, JSON `model` section) provisions the CONFIGURED artifact resolved with engine-boot precedence (runtime-config snapshot > user yaml > defaults — BUG-136 parity). docker/provision_model.py is now a thin wrapper over the same seam (no forked recipe; BUG-269 source-contract test re-pointed at the owning module). engine_boot catches ArtifactIntegrityError on the LiveEngine construction and renders the fail-closed operator panel whose hint names `nexus repair --model` (gate NOT weakened — still raises + Exit(1), surfaced not traceback); health hints now true; README quickstart gained the one-line bootstrap step (Z-B8 fold-in).
- FIX (Z-B5): database/engine.py `_Lock.__enter__` now `path.parent.mkdir(parents=True, exist_ok=True)` before os.open(O_CREAT|O_EXCL). FIX (W12-1): styling panels ASCII-safe (`OK` prefix, `*`/`o`/`#` mode dots, `!!` LIVE warning, `>` hint arrow); rich box glyphs already fall back via its encoding machinery.
...
- OUT-OF-SCOPE DISCLOSED (lane-05 still open): Z-B2 conftest model-path isolation (root cause of the §0 production clobber — separate lane owns test isolation), Z-B3 force_fresh seam generalization, Z-B4 gateway default-creds/URL gap (blocks LIVE on non-Windows), Z-B6 champion-sync class-defaults row (E10), Z-B7 portability items, packaging latent (execution_costs parents[3]). Bare `nexus start` on a zero state STILL fails closed by design (correct per P0 gate) — it now says exactly what to run; the lane's clone-and-run acceptance ran repair -> repair --model -> boot-loop green in a redirected-LOCALAPPDATA tmp workspace.

## BUG-297 - Perf-wave R5 (the lane-10 §3.1 item-1 twin of BUG-292): the evaluator's derived-registry reads opened a RAW per-call sqlite3.connect on the pre-trade tier-3 fallback — connection churn per entry-candidate proposal on the event-loop thread, plus a bypass of the repository's single-connect URI contract (2026-09-16, NSE-Swarm hot-path-sqlite lane; ID NOTE: brief preferred 297, confirmed free at base 3df969bb — 293/294/295/296 consumed by sibling lanes)
- SYMPTOM (VERIFIED reachability map, origin/main fa34fccd, one line per site): `get_registered_strategy_score` (evaluator.py:603-621, raw `sqlite3.connect(self.audit_repo._db_path, timeout=5.0)`) <- `intelligence._get_score` TIER 3 (the budget-exhausted fallback, by design reachable from `evaluate_proposal` <- `tick_pipeline.run_post_policy_stages:71` <- `LiveEngine._process_tick_pipeline` <- `runtime_loop.py:440` — EVENT-LOOP THREAD, per entry-candidate proposal whenever the <=4/s inline-refresh budget is starved; lane-10 §3.1 item 1 stated it: "genuine per-proposal DB read on the loop thread... plus per-call connect churn"; also reached off-loop from accounting/core.py:692/:932); `list_registered_scores` (:623, same 5.0 shape) — zero callers today, public surface, any thread; `_clear_registry` (:523, connect+DELETE, 10.0) — self-heal only (startup `asyncio.to_thread` / web routes), off-loop-safe. Wave-perf's closeout named R5 "the most concrete remaining regression risk"; the policy.py sibling pre-trade read moved off-loop in #213 (BUG-292) — this is the evaluator twin. WORLD: PRE-TRADE-REACHABLE (mission option 2), not offline-only.
- SECOND DEFECT (same sites, silent): every raw connect bypassed `AuditRepository._connect_sqlite` — the repository's ONE connect site that owns the `file:` URI contract ("when the resolved path is a file: URI, sqlite3.connect MUST receive uri=True", audit_repository.py:2063-2074, the 2026-09-09 junk-CWD-file disk-leak fix). Pre-fix, a shared in-memory audit repo (`file::memory:?cache=shared`) made these readers read/create a LITERAL FILE named "file::memory:?cache=shared" in the process CWD: wrong database + disk leak. Pinned now by test_bug297_e through the `_connect_sqlite` spy.
...
- HONEST SCOPE NOTE: the SELECT statements themselves (indexed PK lookup) stay synchronous on the loop thread at tier 3 — removing the churn, not the read; eliminating the read entirely needs the lane-10-prescribed TTL'd in-memory registry copy owned by the accounting worker, a cross-lane design (its §5 R5 prescription) this seam makes cheap to add later. The ledger's own `_connect` (intelligence tier-2 path, budget-gated) is a DIFFERENT owner's seam, untouched.
- REGRESSION: tests/unit/test_bug297_evaluator_hot_path_reads.py (NEW, critical_suite-pinned): a) stats correct through the REAL ledger write API + async persist queue; b) connect-count CONSTANT over 10 lookups + listing + two clear tiers (RED-BEFORE EXECUTED twice: verbatim pre-fix body = 10 connects/10 lookups; full battery against origin/main evaluator = 13 of 17 sites red, growth-per-call exactly as lane-10 described); c) degradation parity — poisoned path (None/[]/no raise), dead-handle eviction + reopen, closed-repo one-shot delegation + file deletable; d) source pins — no sqlite3.connect/_connect_sqlite inside the three former readers (BUG-285/292 class-guard lesson), exactly ONE module connect site (the stub fallback inside `_open_conn`), no check_same_thread/locking escape; e) production opens route through the repository seam per tier then stop; f) END-TO-END TIER-3: six budget-starved `engine._get_score` calls (real retriever context, real family id, TTL cache dropped, `_last_inline_refresh` poisoned) read the persisted VALIDATED-tier score for 1 connect total; g) cross-thread independence (worker thread gets its own handle, main cache untouched); h/i) close() determinism + repo-swap staleness immunity. Neighbors RC=0 after rebase onto 3df969bb: test_experience_intelligence + test_experience_provenance_contract + integration/test_experience_execution_boundary + test_intelligence_phase09 + test_bug284_suitability_micro_sample_floor + test_accounting_core + test_obs_trace_chain (194), bug156 memory-URI + bug292 spread sketch + perf_safety_contract_pins + e2e smoke stage-16 experience (60+), ruff check/format repo-wide clean, mypy clean on the three touched files. PRE-EXISTING RED disclosed, base-proven identical by stash-check at 3df969bb: integration test_end_to_end_closed_loop_bug140::test_complete_evidence_loop_0_to_100 (STAGE 12 CAUSALITY_REJECTED — mock-adapter clock skew in that E2E's own fixture, untouched by this lane; the wave's known-CURLF-shape class).
...
## BUG-298 - Packaged fresh-install START crash: a clean GitHub-release install died on first launch `ArtifactIntegrityError: LOAD_REJECTED: artifact missing or empty (model.pt)` -> `[PYI-...ERROR] Failed to execute script 'packaged_main'`; the release lanes NEVER launched the built EXE in a start shape, and the ecosystem had no honest model-ACQUISITION path (2026-09-16, user-reported on v9.0.12; renumber note: brief said "BUG-293" — taken on origin/main by the lane-11 fail-open entry (3df969bb) before this lane wrote its ledger; sibling BUG-296 (78c77e03) owns the SOURCE-clone `repair --model` starter seam — this entry owns the PACKAGED install + the first-run acquisition architecture, same swarm-collision class as 276->278 / 285->286)
- SYMPTOM (user log, 2026-09-15 21:22, Windows 11, v9.0.12 setup.exe): banner + migrations OK, then `CANONICAL_SPREAD_BAND_UNAVAILABLE` + three `CANONICAL_COSTS_UNAVAILABLE` warnings (configs/execution_assumptions.json resolved to `<install>/configs/...` but PyInstaller onedir extracts `--add-data configs` into `_internal/configs/` — nothing mirrored it), then MODEL_LOAD_REJECTED on model.pt and a hard EXE abort. The SAME defect class as BUG-296's fresh-clone death, but on the release-payload path: nothing in release.yml / build_release.ps1 / the installer ever minted or shipped a serving bundle, and no code anchored the packaged relative config path (double-click CWD is arbitrary).
- ROOT CAUSES (three, all verified against the shipped 9.0.12 layout): (1) PATH GAP — no first-run model acquisition existed for an INSTALLED release (mint lived only in docker/runtime_gate; BUG-296 productized the CLI-repair half; the boot half stayed broken for releases); (2) PACKAGED PATH — `model_artifact_path` / `CANONICAL_PATH` resolve relative to CWD/repo-parents which are wrong under frozen onedir (live_engine 1167/1571, execution_costs 36); (3) RELEASE-GATE GAP — every smoke launched only `version` / `health` (non-engine commands); a release whose first double-click crashes could and did ship GREEN.
- FIX (this lane; builds ON the BUG-296 seam, never forks it): NEW `release/bootstrap.py` — packaged anchors: `resolve_model_path` (workspace-anchor relative artifact paths), `anchor_workspace` (frozen chdir to bundle dir + runtime subdir skeleton), `ensure_packaged_config_dir` (mirror `_internal/configs` -> `<root>/configs`, copy-if-missing), `bundle_status` (never-raising classification through the EXACT engine load gates), `mint_starter_bundle` DELEGATING to `model_bootstrap.provision` (single mint recipe, champion/refusal tables inherited, no forked bytes; local pre-check keeps the never-clobber-what-cannot-be-judged contract). `execution_costs`: single `resolve_canonical_path()` (env > repo-root > packaged candidates; source behavior byte-identical, fail-closed unchanged). `live_engine`: config artifact path anchored at BOTH resolution sites (incl. the BUG-136 rehydrated snapshot). `engine_boot`: first-run model gate — workspace anchoring + `FirstRunCoordinator.ensure_serving_model` BEFORE LiveEngine construction: PAPER prefers signed OFFICIAL download, falls back to the explicitly-labeled DEV STARTER (loud yellow panel, install-state origin DEV_STARTER, never presented as production); SHADOW/LIVE REFUSE without a verified bundle; LIVE never auto-provisions. `model_bundle_store` rejection log names the remedies.
- ACQUISITION ARCHITECTURE (operator redesign directive, 2026-09-16): NEW package `model_provisioning/` — states (missing/downloading/verifying/training/validating/validation_failed/candidate/verified/ready/rejected StrEnum) + service (`classify_serving_slot` honest origins OFFICIAL/USER_TRAINED/GOVERNED/DEV_STARTER/UNKNOWN with provisioner-marker + starter-note precedence, install-state sidecar `install-state.json` keyed by artifact sha256, disk-disagreement demotes to UNKNOWN, champion binding via registry fingerprint, governed install-over-starter-only helper, provisioner_state.json for UI resume) + PATH A `official.py` (`nexus_model_bundle_v1` signed-manifest schema; verification chain download -> per-file SHA256 -> Ed25519 over canonical payload against the EMBEDDED updater trust root -> schema/geometry/architecture bindings -> integrity probe -> atomic manifest-last install; unverified never installs; Google-Drive and HTTPS-directory layouts) + PATH B `pipeline.py` (CSV/Parquet/MT5 import through canonical `normalize_bars_frame` with REAL diagnostics — duplicates/invalid/gaps/time-range/chronological-tail selection; the CANONICAL purged walk-forward trainer shared with official training; honest ProgressEvents — fraction/metrics exist only where measured; cancel at epoch boundary via new `progress_cb`/`cancel_event` seams on WalkForwardTrainer (epoch-hook additions only, dormant when unconfigured — every existing caller unchanged); zero-network privacy invariant; install only over empty/starter slot, governed promotion untouched). CLI (`provision_commands.py`): `model-setup` ceremony (PATH A/PATH B/DEV STARTER choice screen), `model-official`, `model-train-local`, `model-provision --status|--starter`, `train-once` kept as deprecated compat alias (MT5 data-fetch -> same pipeline). Web: `provisioning_routes.py` (/api/provisioning/{status,environment,official,train/start,train/progress,train/cancel}) + `Web/first_setup.html` wizard (public entry document, same BUG-267 cookie-bootstrap contract; all data routes token-gated; import path allowlist via NEXUS_IMPORT_ROOTS — arbitrary paths refused). Publisher tool `scripts/release/build_official_bundle.py` (signs with the release trust root and REFUSES to publish anything a client would reject).
- RELEASE-GATE CLOSE (the user's "if fail not push to github"): NEW `scripts/build/start_smoke.ps1` launches the BUILT onedir EXE bare (= double-click parity) from a clean workspace for 90s and fails on early-exit OR any crash marker (ArtifactIntegrityError / MODEL_LOAD_REJECTED / Failed to execute script / [PYI- / Traceback). Wired as a BLOCKING step in release.yml build-windows-x64 ("Fresh-install START smoke", after EXE smoke tests, before the installer) — the publish job needs build-windows-x64, so a broken first-start can no longer produce a GitHub release. The clean-install lifecycle (silent install -> version/health -> START smoke against the INSTALLED layout -> re-install idempotency -> uninstall -> user-data preservation, `clean_install_test.ps1 -StartSmoke`) was only ever run from the local orchestrator — it is now a required release step too, and build_release.ps1 passes -StartSmoke.
...
- HONEST LIMITS: the starter is a 30-step synthetic-data mint — PAPER-safe, gate-passing, explicitly labeled DEV STARTER; it is NOT the user's production model (that is PATH A once the operator hosts the first official bundle via build_official_bundle.py + NEXUS_OFFICIAL_MODEL_BASE_URL, or PATH B train-once / model-train-local). PATH A end-to-end against a LIVE host is exercised only through the fake-source pins until the first official bundle is published (the chain itself is real code; signature/SHA/geometry failures proven). The torch-absent PATH A install path defers the tensor probe to engine load (defense-in-depth layering, logged as TORCH_ABSENT, never a silent pass). The packaged smoke is Windows-only (the only platform that builds the EXE).

## BUG-299 - P0-3 gate-integrity (ID NOTE: brief said 298 — consumed by the sibling fresh-install lane mid-run, PR #234 933a9577; renumbered 299, free on origin/main at merge-base check, same swarm-collision class as 276->278/285->286): the canonical-costs research bridge silently truncated friction to the 5-tick model default, so every default-costs backtest paid 25% of calibrated cost and robustness GATE8 stress scenarios were EXACT no-ops that PASSed everything (2026-09-16, NSE-Swarm role4 Research/Backtest)
- SYMPTOM (VERIFIED at 2ae009c0, executable probes): configs/execution_assumptions.json canonical evidence converts to spread_ticks=15.0 + slippage_ticks=5.0 (execution_costs.py:239-240 to_research_assumptions), but the bridge never set max_slippage_ticks — it inherited the ExecutionAssumptions model default 5.0 (research/models.py:174, "guard against runaway"). research/metrics.py:143-144 models effective friction as min(spread+slippage, max_slippage_ticks) => 5 ticks, i.e. 25% of the calibrated 20-tick cost, on EVERY default-costs engine: ResearchPipeline wires to_research_assumptions into BacktestEngine/WalkForwardEngine/OOSGate/RobustnessEngine (pipeline.py:150-161) and LiveEngine wires the pipeline (live_engine.py:1101). Probe (risk 1.0, r=+0.10, canonical costs): pre-fix expectancy 0.0 vs cap=40 expectancy -0.3 — the truncation is exactly 15 ticks of hidden edge. WORSE: robustness STRESS_SCENARIOS perturb spread/sl by +1/+2 ticks (robustness.py) and _friction_sensitivity re-applies the same cap (metrics.py:289) — with base sum 20 already ABOVE cap 5, baseline AND every stressed bundle clamp to the SAME 5.0: all six stress expectancies byte-identical to baseline, max_degradation a constant 0.0, GATE8 (model_lifecycle/gates.py:212-229 PASS iff status==PASS) PASSed every candidate that reached it, and scoring rob term = 1.0 (scoring.py:196, marketplace/scoring.py:246). This is EXACTLY P0-3 handoff (docs/agent_handoffs/2026-09-09_p0-3_research_gate_integrity_handoff.md) acceptance criterion 3 "silent no-op forbidden", which listed latency only — the friction scenarios were dead too, via a path the handoff did not see. The economics lane had already reasoned out the same trap (economics.py:77-79 PRODUCTION_MAX_FRICTION_TICKS=40.0: "legacy frictionless baseline used max_slippage_ticks=5 for tiny stresses") — the canonical bridge never got it.
...
## BUG-301 - CodeQL first-run-surface remediation (#1096 path-injection + #1097 stack-trace-exposure) AND the OPTIONAL Training-Environment lifecycle the redesign directive demanded (2026-09-16, follow-up lane to BUG-298 #234; ID NOTE: this lane's brief said "BUG-299" — taken on origin/main by the P0-3 friction-cap lane; then 300 was taken by AI-triage lane; final BUG-301, free on origin/main; same swarm-collision class as 276->278 / 285->286 / 293->298)
- TRIGGER (two CodeQL alerts on the shipped main from #234 + operator architecture directive, Farsi+English, 2026-09-16): #1096 py/path-injection `provisioning_routes._allowed_import_path` built a Path from the request body; #1097 py/stack-trace-exposure `detect_ml_environment()["torch_error"] = str(exc)` flows to `GET /api/provisioning/environment`. The directive additionally rejected the shipped state as "environment detection + provisioning plumbing, not a real lifecycle": demanded APPLICATION SETUP vs OPTIONAL TRAINING SETUP split, DISCOVERY-only `/environment`, a TrainingEnvironmentManager owning the full lifecycle, training GATED on READY, canonical PINNED versions (not `pip install torch` guessed), CPU and CUDA as two REAL smoke-tested paths, existing-venv reuse, typed actionable failures (never hide the real error as `unexpected {category}`), and NO PyTorch at startup.
- #1096 FIX (defense-in-depth, also closes a REAL bug the original had): `_allowed_import_path` now (1) rejects empty/null-byte and any `..` segment (both separators) BEFORE resolving, (2) resolves, (3) admits ONLY via `Path.is_relative_to(root)` per-operator containment (the old `str.startswith` carried a sibling-prefix bypass — `data/rawx` passes `startswith("data/raw")` — now rejected; pinned). Read-only thereafter. Root itself + missing-under-root refused.
- #1097 FIX: `detect_ml_environment` carries `torch_error = type(exc).__name__` (CATEGORY only), full detail to the local log; the worker-crash path likewise surfaces `unexpected {category}` server-logged, and `OFFICIAL_BUNDLE_REJECTED` returns the safe step CODE (digests/URLs stay in provisioner_state). New web route never stringifies an exception to the client.
- TRAINING ENVIRONMENT CONTRACT (new `model_provisioning/training_env.py` + `configs/training_environment.json`): TrainingEnvironmentManager stage ladder PYTHON->VENV->PIP->BACKEND->TORCH->VERSION->SMOKE->READY, `status()`=DISCOVERY (proven read-only: no `-m venv`/`pip install` in subprocess spy, no training-env dir created), `install()`=the ONLY mutator (explicit). Frozen-EXE guard: `is_interpreter_process()` — a PyInstaller exe is never probed/spawned as a script interpreter (would re-launch the engine); current interpreter short-circuits to in-process facts (a packaged CPU torch no longer pretends to satisfy a cuda backend). resolve_python override>current>PATH python (supported-range gate), resolve_env managed>env>active-venv. VERSION gate rejects +cpu-vs-cuda via TORCH_LOCAL_TAG_MISMATCH. Two REAL smoke tests: CPU tensor op; CUDA allocation+matmul on cuda:0 (is_available alone is NOT READY). Cross-interpreter READY -> in_process_ready False + training_command (TRAINING_ENV_MISMATCH, never silent). ~20 EnvCodes each with an actionable remedy; `classify_pip_failure` maps network/index/version/permission.
- CANONICAL PINS: `configs/training_environment.json` (nexus_training_env_v1) — python range, venv, cpu{torch==2.13.0 @ whl/cpu} + cuda{torch==2.13.0 @ whl/cu126, min_driver 12.6}. torch version matches the shipped hidden-import build (2.13.0). CPU and CUDA are the two variants; a user's explicit --backend is honored (no auto-flip to a mismatched build).
- GATE WIRED: pipeline.train_local_model + `model-train-local` + web `train/start` all call `status()` first and return typed `TRAINING_ENV_BLOCKED`/`TRAINING_ENV_MISMATCH` (checklist + remedy) BEFORE any training work / thread — a not-ready env leaves the serving slot empty (pinned). Routes: `/environment` DISCOVERY-only, new `POST /environment/install` (single-flight `_INSTALL_ACTIVE`, never auto-trains). CLI: new `nexus model-train-env [--install] [--backend]`; `model-train-local` gains `--backend`; `model-setup` split into APPLICATION SETUP vs OPTIONAL TRAINING SETUP panels. Wizard `/first_setup.html`: real OK/-- envNote + backend `<select>` + "Install required dependencies" button; train blocked panel. cp1252 safe.
- REGRESSION (critical_suite-registered files from #234, +8 new): test_bug298_model_provisioning.py (+contract-parses, discovery-readonly spy, packaged VENV_NOT_FOUND/PYTHON_NOT_FOUND gate, local-tag cuda-vs-cpu gate, pip taxonomy 4 cases, train-blocked-leaves-slot-empty); test_bug298_provisioning_web_api.py (+import-guard sibling-prefix/traversal/null-byte/root/missing/absolute, torch_error category-no-secret via __import__ monkeypatch, environment discovery-only shape, backend validation, install route single-flight). GREEN: both batteries + bug298 bootstrap + bug296/bug293 failopen neighbors; runtime_gate L0-L9 CERTIFIED; repo-wide ruff check/format clean; mypy 597 files clean. HONEST LIMIT: the real install() pip ladder is proven only via the taxonomy + discovery spies (no torch install executed in CI); cuda smoke needs a real GPU (auto-skips where nvidia-smi absent); the +cu126 pin's exact driver compatibility is operator-host-time verifiable, not CI-provable here.

## BUG-305 - Audit G1 remainder: the runtime broker-offset override was only PARTIALLY adopted — native MT5 history/tick request windows and the LIVE maintenance-window entry guard still read the raw 180-min constant (2026-09-18, NSE-Swarm role 10 Accounting/Forensics)
- SYMPTOM (VERIFIED at origin/main 184fc857, executable probes): audit G1 (docs/agent_handoffs/2026-09-07_NSE_audit_findings_and_status.txt:281) flagged `BROKER_SERVER_UTC_OFFSET_MINUTES = 180` as a hardcode that shifts every timestamp and accounting day boundary for half the year (EET GMT+2 winter / EEST GMT+3 summer). A runtime override accessor was later added (`get_broker_server_utc_offset_minutes`, providers.py:57, env `NSE_BROKER_SERVER_UTC_OFFSET_MINUTES`, validated 0..1440) and wired into `broker_epoch_to_utc` (providers.py:149), `broker_history._epoch_utc` (broker_history.py:308-312) and `incidents.timebase.timebase_event_chain` (timebase.py:239-242). It was NOT adopted everywhere: `DirectMT5Adapter` built its default history/tick request windows from the raw constant (mt5_adapter.py:610/617 get_history_orders, 656/663 get_history_deals, 794/795 get_tick_history), and the LIVE directional-entry guard evaluated the maintenance window from the raw constant (execution/lifecycle/dispatch.py:326). An operator setting the env var got epoch OUTPUT conversion at the configured offset but INPUT request windows at 180min — a net skew equal to the difference on every reconcile-baseline history read — and, worse on the money path, an entry guard evaluated on a DIFFERENT clock than the rest of the timebase. Same class as BUG-303: a configurable seam with a bypass consumer.
- FIX (minimal, seam-consistent): all seven executable raw-constant reads route through `get_broker_server_utc_offset_minutes()`; the adapter's import is replaced by the accessor import; the dispatch guard caches the accessor result once per guard evaluation and logs it. With the env var unset the behaviour is byte-identical to the pre-fix constant read (pinned). No contract/model/threshold changed; `RemoteMT5GatewayAdapter` untouched.
...
## BUG-306 - Windows CI critical suite breakages: official_install._sync_file EBADF on "rb" fsync and test_model_studio.py charmap UnicodeDecodeError on Web/index.html (2026-09-18, NSE-Swarm role 11 CI/GitOps)
- SYMPTOM (VERIFIED in Actions run 35300190463 / job 105461019562 `Py Tests (windows-latest)` on main push b361357c):
  (1) `tests/unit/test_bug298_model_provisioning.py::test_official_path_a_end_to_end` failed with `OSError: [Errno 9] Bad file descriptor`. On Windows, Python's `os.fsync(fd)` delegates to CRT `_commit(fd)`. Microsoft CRT `_commit` documentation specifies: "If fd is an invalid file descriptor, or is open for read only, ... errno is set to EBADF." In `src/nexus_scalp/model_provisioning/official_install.py:225`, `_sync_file` opened files with `with path.open("rb") as stream: os.fsync(stream.fileno())`. Read-only descriptor triggered EBADF under Windows CRT `_commit`.
  (2) `tests/unit/test_model_studio.py::test_web_ui_assets_exist` failed with `UnicodeDecodeError: 'charmap' codec can't decode byte 0x81 in position 17444: character maps to <undefined>`. On Windows runners, `Path.read_text()` defaults to `cp1252`/`charmap`. `Web/index.html` contains multi-byte UTF-8 sequences that crash cp1252 decode when opened without explicit `encoding="utf-8"`.
- FIX:
  (1) `official_install._sync_file`: use `"r+b"` on Windows (`os.name == "nt"`) so the CRT file descriptor has write access, and wrap in `try ... except OSError: pass` to ensure filesystem sync limitations never abort an install transaction.
  (2) `tests/unit/test_model_studio.py`: specify `encoding="utf-8"` in all `.read_text()` calls.
- REGRESSION: `tests/unit/test_bug306_windows_ci_compat.py` (NEW, critical_suite-pinned, stdlib+repo only):
  - `test_sync_file_normal_file`: validates `_sync_file` on real payload.
  - `test_sync_file_windows_mode`: validates that when `os.name == "nt"`, `_sync_file` opens with `"r+b"`.
  - `test_sync_file_handles_ebadf_gracefully`: simulates CRT EBADF `_commit` failure, confirms graceful handling.
  - `test_sync_file_handles_missing_file_gracefully`: confirms inaccessible/missing paths don't crash caller.
  - `test_web_ui_assets_utf8_decoding_charmap_regression`: proves `Web/index.html` crashes `cp1252` decode and passes `utf-8` decode.
  - `test_model_studio_test_file_ast_read_text_encoding_guard`: AST scanner enforcing all `.read_text()` calls in `test_model_studio.py` declare explicit encoding.
  All 6 new tests + 19 model_studio tests + 17 model_provisioning tests passed (42/42 GREEN). Manifest 202 entries, 0 missing. repo-wide ruff check/format clean; mypy clean.

## BUG-307 - 70D shadow reported PHANTOM Champion-vs-Shadow disagreements for ticks where NO shadow inference ran: the neutral placeholder shadow_action="NO_TRADE" was fed into the disagreement taxonomy, so an IDLE runtime (no 70D candidate) told the operator "the shadow disagrees with the champion" on every tick (Hermes, 2026-09-21)
...
- ROOT CAUSE: `observe()` builds its invalid-path observation with the module-level neutral defaults `shadow_action="NO_TRADE"` / `shadow_confidence=0.0` and then calls `classify_disagreement(champion_action, shadow_action, ...)` UNCONDITIONALLY. With `champion=BUY_MARKET` and the placeholder `NO_TRADE`, the pure classifier correctly returns `CHAMPION_BUYS_SHADOW_NO_TRADE` — and the older code path collapsed to `NO_TRADE_DISAGREEMENT` for the not-READY / vector-rejected branches. So a row that never ran a model got a real disagreement label. Three follow-on effects compounded it:
  (1) `runtime.summary()["disagreements"]` counted those rows, so the Shadow panel's disagreement total included ticks with no comparison;
  (2) `store.disagreement_counts()` (CHG-0046 D9 had already moved it to `valid=1`) still counted a valid-but-timed-out row, and `list_observations(disagreement_only=True)` filtered on `agreement=0` alone, so the non-compared rows reached the spec-30 disagreement table;
  (3) the rows were stamped `sample_source="LIVE"` even though they are `smoke_24` fixtures from a synthetic smoke run, which is what made them indistinguishable from live evidence in the UI.

- FIX (fail-loud, no evidence deleted):
  (1) NEW `DisagreementClass.NOT_COMPARED` — distinct string, not an alias, so persisted old rows stay readable and the histogram separates them;
  (2) both invalid branches of `observe()` stamp NOT_COMPARED, and the classify call is now guarded by `valid` — a real class is only computed when an inference actually ran;
  (3) NEW `Shadow70Observation.compared` property (`valid and error_code == ""`) — the single truthfulness primitive; note it is NOT the same as `valid`, because a valid row can still carry `SHADOW_INFERENCE_TIMEOUT`;
  (4) `summary()` reports `not_compared` separately and no longer inflates `disagreements`;
  (5) `store.disagreement_counts()` defaults to `valid=1 AND error_code=''`, and `list_observations(disagreement_only=True)` uses the same condition — the two historical rows are still returned by `list_observations()` (no deletion), they just cannot be read as disagreements;
  (6) the `/api/models/shadow70/summary` projection carries `disagreement = error_code` for non-compared rows via the new `_shadow70_verdict()` helper, plus `valid` / `error_code` / `sample_source` / `confidence_delta` so the UI can render the distinction;
  (7) the UI cell renders a non-compared verdict in the error color with a tooltip stating no shadow inference ran — it never reads as a trade decision;
  (8) the two `smoke_24` rows in `artifacts/audit.db` were relabeled `sample_source='SMOKE'` (they were never live ticks).

- EVIDENCE (executed, not described): reproduced at HEAD with a bare `Shadow70Runtime()` (IDLE, no candidate) — pre-fix `disagreement=NO_TRADE_DISAGREEMENT`, `summary.disagreements=1`; post-fix `disagreement=NOT_COMPARED`, `summary.disagreements=0`, `not_compared=1`. Store probe against the REAL `artifacts/audit.db`: `disagreement_counts()` = `{}` (was `{"NO_TRADE_DISAGREEMENT": 2}` under the old `valid=1` filter), `disagreement_counts(valid_only=False)` still reports both rows, `list_observations(disagreement_only=True)` = 0 rows, `list_observations()` = 2 rows unchanged. Tests: NEW `tests/unit/test_bug278_shadow70_not_compared.py` 5/5 (ID NOTE: named for the observed class, not the bug id — BUG-278 was already taken by the test-harness champion-clobber lane; keep both entries); shadow70 suites green: test_shadow70_runtime 58, test_shadow70_safety, test_shadow70_health_drift, test_shadow_hardening_chg0046 18, test_shadow_replay_evidence_chg0047, test_shadow_phase11, test_bug276_unique_constraint_shadow, test_live_shadow_outcome_resolution 20, test_schema_70d_reconciliation (2 env skips: champion artifact + real 70D dataset absent on this host), test_shadow70_news_family. `frontend/` tsc -p tsconfig.json --noEmit = 0 errors. No goldens weakened; no contract changed — only the label on rows that never compared.
