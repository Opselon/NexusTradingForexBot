...
## BUG-249 - Spread-gate parameter `max_spread_atr_ratio` was plumbed into the constructor but NEVER enforced (dead guard; Agent-5 decision-path forensics, 2026-09-05, Nexus-Main; ID-COLLISION NOTE: Agent-13's research row + Agent-6's -A6 suffix already consumed `BUG-249`; this row uses the same base ID per the collision rule with a distinct title - the ledger keeps both, history not rewritten)

- SEVERITY: MEDIUM (documented spread guard silently inert: an ATR-normalized cap meant to fail-closed entries when the broker take dominates the M1 scalp window was unreachable) | STATUS: FIXED
- SURFACE: `src/nexus_scalp/signals/policy.py` (compute + wiring)
- SYMPTOM: `SignalPolicy.__init__(max_spread_atr_ratio=0.18)` stored the limit but NO code path ever consulted it (inspect confirmed 0 uses in evaluate_probabilities); a candidate with spread 0.90 (ratio 0.60 vs ATR 1.5, 3.3x the cap) proceeded past every gate. Only RiskEngine max_spread_points (60 points) and regime HIGH_SPREAD_CHOP protected the path - neither ATR-normalized.
- FIX: guard wired fail-closed: `spread_atr_ratio = current_spread/atr` (zero-spread degenerate path never blocked by this gate), blocked at decision_stage=SPREAD_ATR_GATE / blocked_by=SPREAD_ATR_RATIO with reason SPREAD_ATR_RATIO_EXCEEDED (e.g. `0.60 > 0.18`); evidence stamped into risk_checks (`spread_atr_ratio`, `max_spread_atr_ratio`) on both NO_TRADE and FINAL_DECISION paths (INV-018).
- EVIDENCE (deterministic harness, repo venv): narrow 0.20 (0.133) passes to FINAL_DECISION; wide 0.90 (0.60) blocks at SPREAD_ATR_GATE; ruff/mypy clean; 3/3 regression tests.
- REGRESSION: `tests/unit/test_agent5_bug249_251_252.py::TestBug249SpreadAtrGate` (wide-block, narrow-pass, zero-spread-non-block; xdist-safe).
- INVARIANTS: INV-018; threshold value 0.18 unchanged.
...
- SEVERITY: MEDIUM (decision-consistency defect: the reversal gate and the entry gate measured the same model output on two different scales, so flips could over-fire or under-fire relative to the calibrated threshold's meaning) | STATUS: FIXED
...
- SYMPTOM: since CHG-0042 the entry confidence gate measures `max(buy,sell)/(buy+sell+no_trade)` (trained-class normalization, WAIT excluded). The AI-reversal gate still computed relative bias as `buy/(buy+sell)` over RAW directional probs and stamped the RAW prob as proposal confidence. With a large NO_TRADE mass (production serving head is near-uniform ~0.3-0.35/class) a conviction that PASSES the entry gate could FAIL the reversal bias or vice versa: sell=0.30/buy=0.20/no_trade=0.40 -> entry-scale 0.333 but raw bias 0.60 (would pass rel_threshold 0.60); sell=0.55/buy=0.10/no_trade=0.35 -> raw 0.846 passes, trained 0.550 fails the same threshold.
- ROOT CAUSE: CHG-0042 normalized the entry gate but did not propagate to `_evaluate_ai_reversal` (no `no_trade_prob` input at all).
- FIX: `_evaluate_ai_reversal` takes `no_trade_prob` (passed from the call site's sanitized `prob_no_trade`), normalizes the relative bias over the trained mass `max(0,buy)+max(0,sell)+max(0,no_trade)` (degenerate mass falls back to the raw directional share, never manufactures conviction), and stamps the reversal CLOSE_POSITION proposal confidence in the SAME trained-class scale (clamp >= 0). Thresholds (ai_flip_relative_bias_threshold 0.60, ai_flip_min_delta 0.10) and structural-confirmation requirements UNCHANGED.
- EVIDENCE (deterministic harness, repo venv): 0.55/0.10/0.10 -> bias 0.733 >= 0.60 fires with confidence 0.733 (was raw 0.55); 0.30/0.20/0.40 -> trained bias 0.333 < 0.60 AND delta 0.10 NOT > 0.10 -> NO false reversal (old raw bias 0.60 >= 0.60 would have fired). ruff/mypy/py_compile clean.
- REGRESSION: `tests/unit/test_agent5_bug249_251_252.py::TestBug251ReversalConfidenceSemantics` (3: large-NO_TRADE suppression, genuine-conviction normalized stamp, degenerate-mass fallback no-crash).
- INVARIANTS: INV-018 respected; no threshold changed; CHG-0042 semantics now uniform across entry AND reversal gates.
...
- SYMPTOM: `peak_equity = getattr(account, "peak_equity", account.equity)` ALWAYS resolved to `account.equity` because the frozen `AccountInfo` domain model (domain/models.py) has NO `peak_equity` field -> drawdown_pct == 0 -> the `max(0.2, 1-drawdown*0.2)` penalty unreachable on every live path. The authoritative peak lives in `LiveEngine._peak_equity` (restored from the audit DB at startup, withdrawal-adjusted, drives survival mode / max-DD halt) but was never forwarded to the risk engine.
- ROOT CAUSE: contract seam - the risk engine expected an account field the domain model never carried; nothing wired the engine's peak tracker into sizing.
- FIX: `RiskEngine.evaluate_proposal` gains `peak_equity: float | None = None` (precedence: explicit kwarg > account attr if a future model adds it > honest fallback account.equity; bool/NaN/inf/<=0 rejected -> fallback, never fabricated); the LiveEngine hedge path forwards `peak_equity=getattr(self, "_peak_equity", None)`. No threshold changed (penalty formula and 1.0% DD trigger preserved).
- EVIDENCE (deterministic harness, repo venv): equity=8000, peak=12000 -> `Drawdown Penalty Active: Scaling trade risk % by 0.20x due to 33.33% drawdown`, volume 0.14 -> 0.02 (0.2x scaled); peak=None -> baseline 0.14 unchanged. ruff/mypy/py_compile clean.
