...
- FIX: layered, stdlib-only, no contract loss. (1) source: correlation id charset-allowlisted at ``_dispatch`` (``_REQUEST_ID_RE`` = ``[A-Za-z0-9_.:-]{1,64}`` fullmatch; anything else => fresh ``req_`` id, never echoed); (2) relay names: upstream header names must match the RFC 9110 field-name token regex ``_FIELD_NAME_RE`` or are dropped (production uvicorn/h11 never emits non-token names, so relay parity is unaffected); (3) every relayed/echoed name+value passes ``header_safe()`` (CR/LF strip) — the sanitizer shape CodeQL models as a barrier; (4) choke point: handler overrides ``send_header`` to strip CR/LF from BOTH args, so no future echo regresses the invariant. Verified with CodeQL CLI 2.27.0 + the exact codeql/python-queries HeaderInjection.ql run LOCALLY: pre-fix file reproduced all 9 alerts at the alert lines; post-fix file = 0 alerts.
- REGRESSION: tests/unit/test_alt_ui_standalone_server.py TestHeaderInjectionDefense (7): header_safe unit, id-charset unit, send_header choke-point (raw _headers_buffer inspection), folded request-id never echoed (raw-socket obs-fold request; upstream receives only the clean minted id), spacey id mints replacement, upstream fold + illegal-name never split the relayed head (new stub /api/smuggle literal route, line-level wire assertions), HEAD relay echoes only a numeric content-length. RED-before (fix reverted via git show HEAD file): 6/7 FAIL; post-fix 48/48 green — the 41 existing echo/relay/traversal/SSE/fail-closed/redaction pins untouched and still green.
