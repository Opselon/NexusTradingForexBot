...
## BUG-278 - Durable financial-overflow was WRITE-ONLY (perf-wave R2): an overflowed audit row never returned to the ledger and the directory grew unbounded (2026-09-15, NSE-Swarm role 10 Accounting/Forensics)
...
- REGRESSION: NEW tests/unit/test_bug278_overflow_drain.py (16, gated) — end-to-end producer-write -> drain -> ledger replay; duplicate-replay no-op; None-sentinel first pass due; cadence throttle (re-stamp both directions); bounded batch; 5-shape parametrized poison -> dead-letter + retire; per-row isolation (poison does not block the good row in the same pass); writer cap routes to dead-letter; exact counter semantics (recovered+failed+pending == seen, no silent third bucket); wiring pins (drain called from `_process_queue_worker` idle branch; single path-resolution site — writer may not re-inline the workspace expression); NEW tests/unit/test_bug278_overflow_wiring.py (2, gated) — idle-branch wiring + INV-001 guard (no application/* module may call the drain directly).
