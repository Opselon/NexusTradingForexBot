...
| Correlation | incoming `X-Request-ID` honored **only if it matches `[A-Za-z0-9_.:-]{1,64}`** (else a fresh `req_<10 hex>` is minted), forwarded upstream, echoed on every response, present in every log line and error envelope |
| Header-injection defense (BUG-270) | stdlib `send_header` does NOT validate; this host does: every emitted name/value passes `header_safe()` (CR/LF stripped), the handler overrides `send_header` as a choke point, relayed upstream NAMES must be RFC 9110 tokens, and a HEAD relay echoes `Content-Length` only when it is a clean integer |
