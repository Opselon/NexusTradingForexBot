...
| Proxy | `/api`, `/health`, `/healthz` (+ subpaths) → `--backend` (default: the launcher-recorded `NSE_WEB_ACTUAL_PORT` from the repo `.env`, else `http://127.0.0.1:8080`; `NSE_API_ORIGIN` overrides). BUG-267: `/app.js` + `/api_client.js` are proxied too — they carry the backend's bootstrap `Set-Cookie`. Methods GET/HEAD/POST/PUT/PATCH/DELETE/OPTIONS, body (1 MiB `Content-Length` guard), query string |
...
| Correlation | incoming `X-Request-ID` honored **only if it passes `request_id_is_safe` (charset `[A-Za-z0-9_.:-]`, max 64)** (else a fresh `req_<10 hex>` is minted), forwarded upstream, echoed on every response, present in every log line and error envelope |
| Header-injection defense (BUG-270) | stdlib `send_header` does NOT validate; this host does: every emitted name/value passes `header_safe()` (CR/LF stripped), the handler overrides `send_header` as a choke point, relayed upstream NAMES must pass `field_name_is_safe` (RFC 9110 tchar set, linear check — no regex on attacker bytes), and a HEAD relay echoes `Content-Length` only when it is a clean integer |
