...
def register_diagnostics_state_routes(
    ...
    async def toggle_engine(req: ToggleRequest) -> dict[str, Any]:
        # BUG-239: MUST stay ASYNC - START schedules engine.run_loop() via asyncio.create_task.
        # A sync route executes in the anyio threadpool (no loop) -> RuntimeError -> HTTP 500.
        engine = app.state.engine
        if not engine:
