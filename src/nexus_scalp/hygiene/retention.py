...
AUDIT_RETENTION: dict[str, RetentionRule] = {
    ...
    # CHG-0046 D10: shadow70 telemetry is the only UNGOVERNED per-tick
    # shadow surface (up to ~10^5-10^6 rows/day when ACTIVE). Policy:
    # bounded raw telemetry + durable research summaries. Comparison rows
    # (aggregated evidence) are never deleted; raw per-tick observations /
    # events / health snapshots roll off after 30 days; drift ALERTS are
    # governance evidence and are kept.
    "shadow70_observations": RetentionRule(
        database="audit",
        table="shadow70_observations",
        tier=DataTier.TIER_2_RESEARCH_EVIDENCE,
        purpose="70D shadow per-tick observations (bounded raw telemetry)",
        minimum_retention_days=7.0,
        delete_after_days=30.0,
        cleanup_class="STALE_TEMP",
        owner="Shadow70Runtime",
        rebuildable=False,
    ),
    "shadow70_events": RetentionRule(
        database="audit",
        table="shadow70_events",
        tier=DataTier.TIER_2_RESEARCH_EVIDENCE,
        purpose="70D shadow structured events",
        minimum_retention_days=7.0,
        delete_after_days=30.0,
        cleanup_class="STALE_TEMP",
        owner="Shadow70Runtime",
    ...
    "shadow70_feature_health": RetentionRule(
        database="audit",
        table="shadow70_feature_health",
        tier=DataTier.TIER_3_MODEL_METADATA,
        purpose="70D shadow feature-health snapshots",
        minimum_retention_days=7.0,
        delete_after_days=30.0,
        cleanup_class="STALE_TEMP",
        owner="Shadow70Runtime",
    ...
    "shadow70_drift_alerts": RetentionRule(
        database="audit",
        table="shadow70_drift_alerts",
        tier=DataTier.TIER_2_RESEARCH_EVIDENCE,
        purpose="70D shadow drift alerts (governance evidence)",
        never_delete=True,
        owner="Shadow70Runtime",
    ),
    "shadow_runs": RetentionRule(
        database="audit",
        table="shadow_runs",
        tier=DataTier.TIER_2_RESEARCH_EVIDENCE,
        purpose="PHASE-11 shadow run identity (append-only history)",
        never_delete=True,
        owner="ShadowEngine",
    ),
    "shadow_decisions": RetentionRule(
        database="audit",
        table="shadow_decisions",
        tier=DataTier.TIER_2_RESEARCH_EVIDENCE,
        purpose="PHASE-11 paired decision records (promotion evidence)",
        never_delete=True,
        owner="ShadowEngine",
    ),
    "shadow_comparisons": RetentionRule(
        database="audit",
        table="shadow_comparisons",
        tier=DataTier.TIER_2_RESEARCH_EVIDENCE,
        purpose="PHASE-11 aggregated comparisons (promotion evidence)",
        never_delete=True,
        owner="ShadowEngine",
    ),
    "shadow_promotions": RetentionRule(
        database="audit",
        table="shadow_promotions",
        tier=DataTier.TIER_2_RESEARCH_EVIDENCE,
        purpose="PHASE-11 promotion evaluations (governance audit trail)",
        never_delete=True,
        owner="ShadowEngine",
    ),
