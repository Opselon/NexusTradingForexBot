...
class BarHandler:
    ...
    def on_new_bar(self, tick: TickData, fv, last_bar) -> None:
            ...
            if self.om._governance_reference_vector is None:
                _ref = self.om._validate_50d_tensor(fv.to_tensor_input(), context="governance_reference")
            ...
            try:
                ...
                if "feat_0" not in radar_rec:
                    radar_rec = (
                        self.om._rolling_feature_records[-1] if self.om._rolling_feature_records else None
                    )
